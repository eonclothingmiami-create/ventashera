...
(function initCajaLogic(global) {
  ...
  /** Antes del uuid fijo, algunas BD tenían la caja principal con id texto `caja_principal`. */
  const LEGACY_CAJA_ROW_ID = 'caja_principal';

  /**
   * Si coexisten la caja canónica (`AppId.CAJA_PRINCIPAL_ID`) y la fila legada, fusiona saldos en la canónica y elimina la legada del array.
   * @returns {{ merged: boolean, canonical?: object, legacyId?: string }}
   */
  function mergeLegacyCajaPrincipalDuplicate(state) {
    if (!state || !Array.isArray(state.cajas)) return { merged: false };
    const canonicalId = String(global.AppId?.CAJA_PRINCIPAL_ID || 'f7c2b8e0-4a1d-4f3e-9c8a-2b6e1d4a7f00');
    const list = state.cajas;
    const li = list.findIndex((c) => c && String(c.id).toLowerCase() === LEGACY_CAJA_ROW_ID);
    const ci = list.findIndex((c) => c && String(c.id) === canonicalId);
    if (ci < 0 || li < 0 || ci === li) return { merged: false };
    const canonical = list[ci];
    const legacy = list[li];
    normalizeCaja(canonical);
    normalizeCaja(legacy);
    BUCKET_KEYS.forEach((k) => {
      canonical.saldosMetodo[k] =
        (parseFloat(canonical.saldosMetodo[k]) || 0) + (parseFloat(legacy.saldosMetodo[k]) || 0);
    });
    canonical.saldo = canonical.saldosMetodo.efectivo;
    if (legacy.estado === 'abierta') {
      canonical.estado = 'abierta';
      canonical.apertura = canonical.apertura || legacy.apertura;
    }
    if (!canonical.sesionActivaId && legacy.sesionActivaId) canonical.sesionActivaId = legacy.sesionActivaId;
    const bid = new Set([...(canonical.bodegaIds || []), ...(legacy.bodegaIds || [])]);
    canonical.bodegaIds = Array.from(bid);
    if (!canonical.proximaAperturaSaldos && legacy.proximaAperturaSaldos) {
      canonical.proximaAperturaSaldos = legacy.proximaAperturaSaldos;
    }
    list.splice(li, 1);
    return { merged: true, canonical, legacyId: LEGACY_CAJA_ROW_ID };
  }

  function remapLegacyCajaIdInCollections(state, canonicalId) {
    const cid = String(canonicalId || '');
    if (!cid) return;
    (state.tes_movimientos || []).forEach((m) => {
      if (m && String(m.cajaId).toLowerCase() === LEGACY_CAJA_ROW_ID) m.cajaId = cid;
    });
    (state.tes_cierres_caja || []).forEach((r) => {
      if (r && String(r.cajaId).toLowerCase() === LEGACY_CAJA_ROW_ID) r.cajaId = cid;
    });
  }
  ...
  global.AppCajaLogic = {
    ...
    normalizeCaja,
    normalizeAllCajas,
    mergeLegacyCajaPrincipalDuplicate,
    remapLegacyCajaIdInCollections,
    bucketFromMetodoId,
