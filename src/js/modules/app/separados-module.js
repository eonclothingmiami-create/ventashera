...
(function initSeparadosModule(global) {
  ...
  function fmtLineItems(items) {
    return (
      items
        .map((i) => `${i.nombre || i.name || 'Ítem'}${i.talla ? ' · T:' + i.talla : ''} ×${i.qty || i.cantidad || 1}`)
        .join('; ') || '—'
    );
  }
  ...
  function renderSeparados(ctx) {
    ...
    const rowsHtml =
      lista
        .map((v) => {
          ...
          const itemsCell = fmtLineItems(items);
      ...
      <td>${v.telefono || '—'}</td>
      <td style="font-size:11px;color:var(--text2);max-width:220px">${itemsCell}</td>
      <td style="color:var(--accent);font-weight:700">${fmt(v.valor)}</td>
      <td><span class="badge ${entregado ? 'badge-ok' : 'badge-warn'}">${v.estadoEntrega || 'Pendiente'}</span></td>
      <td>${
        !entregado
        ...
        .join('') ||
      '<tr><td colspan="9" style="text-align:center;color:var(--text2);padding:24px">Sin separados</td></tr>';
    ...
    <div style="display:flex;gap:10px;align-items:flex-end;flex-wrap:wrap;margin-bottom:16px;">
      <div class="search-bar" style="flex:1;min-width:180px;max-width:280px;margin:0"><span class="search-icon">🔍</span><input type="text" id="sep-search" placeholder="Cliente, tel, ref, guía..." value="${q}" oninput="renderSeparados()"></div>
      <div>
    ...
    <div class="card"><div class="card-title">🛍️ SEPARADOS (${lista.length} en tabla · ${separados.length} con filtros fecha/texto)</div><div class="table-wrap"><table><thead><tr><th>Fecha</th><th>Canal</th><th>Ref</th><th>Cliente</th><th>Teléfono</th><th>Artículos</th><th>Total</th><th>Estado</th><th>Acción</th></tr></thead><tbody id="sep-tbody">${rowsHtml}</tbody></table></div></div>`;
  }
