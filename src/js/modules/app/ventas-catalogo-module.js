  ...
  function openRegistrarExternoModal(ctx) {
    ...
    openModal(
      ...
      <div class="form-row-3" style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        <div class="form-group"><label class="form-label">Departamento</label>
          <input type="text" id="vc-reg-depto" class="form-control" placeholder="Antioquia"></div>
        <div class="form-group"><label class="form-label">Ciudad</label>
          <input type="text" id="vc-reg-ciudad" class="form-control" placeholder="Medellín"></div>
      </div>
      <div class="form-group"><label class="form-label">Dirección de envío *</label>
        <input type="text" id="vc-reg-dir" class="form-control" placeholder="Barrio, calle, número, referencia"></div>
    ...
    setTimeout(() => {
      ...
      btn.onclick = async () => {
        ...
        const envioDireccion = (document.getElementById('vc-reg-dir')?.value || '').trim();
        if (!envioDireccion) {
          notify('warning', '📦', 'Dirección', 'Indica la dirección de envío para registrar el pedido.', { duration: 4500 });
          return;
        }
        ...
        const row = {
          ...
          clienteDocumento: '',
          envioDepartamento: (document.getElementById('vc-reg-depto')?.value || '').trim(),
          envioCiudad: (document.getElementById('vc-reg-ciudad')?.value || '').trim(),
          envioDireccion,
          items: itemsParsed,
          totales: {},
          amountCop: monto,
