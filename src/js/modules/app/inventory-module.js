...
(function initInventoryModule(global) {
  ...
  /** Alineado con tesorería / `esMercCreditoTitulo`: evita que “Crédito” con acento no dispare refresco de Pagos proveedores. */
  function artEsMercanciaCredito(art) {
    if (!art || art.tituloMercancia == null || art.tituloMercancia === '') return false;
    return typeof global.esMercanciaCredito === 'function'
      ? global.esMercanciaCredito(art.tituloMercancia)
      : String(art.tituloMercancia).trim().toLowerCase() === 'credito';
  }
  ...
  function renderInvAjustes(ctx) {
          ...
          return lines
            .map((l, idx) => {
              ...
              const fechaHtml =
                `${formatDate(fecha)}` +
                (idx === 0
                  ? `<div style="font-size:9px;color:var(--text2);margin-top:3px">Lote · ${lines.length} línea(s)</div>`
                  : '');
              return `<tr>
                <td style="white-space:nowrap;vertical-align:top">${fechaHtml}</td>
                <td style="vertical-align:middle;max-width:min(280px,36vw)">${art?.nombre || '—'}</td>
                <td style="vertical-align:middle"><span class="badge ${pos ? 'badge-ok' : 'badge-pend'}">${l.tipo}</span></td>
                <td style="font-weight:700;text-align:right;vertical-align:middle;white-space:nowrap;color:${pos ? 'var(--green)' : 'var(--red)'}">${sign}${l.cantidad}</td>
                <td style="vertical-align:middle;font-size:11px;color:var(--text2)">${motivo}</td>
                <td style="vertical-align:middle;white-space:nowrap">${idx === 0 ? `<button type="button" class="btn btn-xs btn-danger" onclick="eliminarAjusteLote('${row.loteId}')">✕ Lote</button>` : ''}</td>
              </tr>`;
            })
            .join('');
        })
    ...
    <div class="table-wrap"><table class="inv-ajustes-table"><thead><tr><th>Fecha</th><th>Artículo</th><th>Tipo</th><th style="text-align:right">Cant.</th><th>Motivo</th><th></th></tr></thead><tbody>
    ${tbody}
    </tbody></table></div></div>`;
  }
  ...
  async function eliminarAjusteLote(ctx) {
    ...
    try {
      for (const a of lines) {
        ...
        if (a.tipo === 'devolucion' && !skipPagosProvMirror()) {
  ...
  function invArtComboLabel(meta) {
    if (!meta) return '';
    return `${meta.nombre || '—'} (Stock: ${meta.stock})`;
  }
  function syncInvArtRowFromPick(row) {
    const pick = row.querySelector('.inv-art-pick');
    const hid = row.querySelector('.m-aj-art');
    const search = row.querySelector('.inv-art-search');
    const id = pick && pick.value ? pick.value : '';
    if (hid) hid.value = id;
    const meta = (global._ajusteArtsMeta || []).find((x) => String(x.id) === String(id));
    if (search) search.value = meta ? invArtComboLabel(meta) : '';
  }
  function selectInvArtRow(row, artId) {
    const hid = row.querySelector('.m-aj-art');
    const search = row.querySelector('.inv-art-search');
    const pick = row.querySelector('.inv-art-pick');
    const meta = (global._ajusteArtsMeta || []).find((x) => String(x.id) === String(artId));
    if (hid) hid.value = artId || '';
    if (search) search.value = meta ? invArtComboLabel(meta) : '';
    if (pick) pick.value = artId || '';
  }
  function renderInvArtSuggestions(row, q) {
    const box = row.querySelector('.inv-art-sug');
    const list = global._ajusteArtsMeta || [];
    if (!box) return;
    ...
    if (hits.length === 0) {
      box.innerHTML = '<div class="inv-art-sug-empty">Sin coincidencias</div>';
    ...
    box.innerHTML = hits
      .map((a) => {
        ...
        const lab = escHtml(invArtComboLabel(a));
        return `<button type="button" class="inv-art-sug-item" data-id="${id}">${lab}</button>`;
      })
      .join('');
    box.style.display = 'block';
    box.querySelectorAll('.inv-art-sug-item').forEach((btn) => {
      btn.addEventListener('mousedown', (e) => {
        e.preventDefault();
        selectInvArtRow(row, btn.getAttribute('data-id'));
        box.style.display = 'none';
      });
    });
  }
  function initInvArtComboRow(row) {
    if (!row || row.dataset.invArtInit === '1') return;
    row.dataset.invArtInit = '1';
    const search = row.querySelector('.inv-art-search');
    const box = row.querySelector('.inv-art-sug');
    const pick = row.querySelector('.inv-art-pick');
    let tmr = null;
    if (pick) pick.addEventListener('change', () => syncInvArtRowFromPick(row));
    if (search) {
      search.addEventListener('input', () => {
        clearTimeout(tmr);
        tmr = setTimeout(() => renderInvArtSuggestions(row, search.value), 100);
      });
      search.addEventListener('focus', () => {
        if (search.value.trim().length >= 1) renderInvArtSuggestions(row, search.value);
      });
  ...
  function buildAjusteLineaRowHtml(artOptionsHtml) {
    return (
      '<div class="ajuste-linea" style="display:grid;grid-template-columns:minmax(200px,1.4fr) minmax(118px,0.85fr) 72px 34px;gap:8px;align-items:end;margin-bottom:10px;padding-bottom:10px;border-bottom:1px solid var(--border)">' +
      '<div class="form-group inv-art-combo-wrap" style="margin:0;position:relative;min-width:0">' +
      '<label class="form-label" style="font-size:10px">Artículo</label>' +
      '<input type="hidden" class="m-aj-art" value="">' +
      '<div class="inv-art-search-wrap">' +
      '<input type="text" class="form-control inv-art-search" placeholder="Buscar nombre o código…" autocomplete="off" spellcheck="false">' +
      '<div class="inv-art-sug" style="display:none" role="listbox"></div>' +
      '</div>' +
      '<select class="form-control inv-art-pick" style="margin-top:6px;font-size:11px">' +
      artOptionsHtml +
      '</select></div>' +
  ...
  function openAjusteModal(ctx) {
    ...
    global._ajusteArtsMeta = (state.articulos || []).map((a) => ({
      ...
      nombre: a.nombre || '',
      codigo: a.codigo || '',
      stock: getArticuloStock(a.id),
    }));
    const artOptionsHtml =
      '<option value="">' +
      escHtml('— Escribí arriba o elegí aquí —') +
      '</option>' +
      (global._ajusteArtsMeta || [])
        .map((a) => '<option value="' + escAttr(a.id) + '">' + escHtml(a.nombre) + ' (Stock: ' + a.stock + ')</option>')
        .join('');
    ...
    <p style="font-size:10px;color:var(--accent2);margin:-4px 0 10px;line-height:1.35">🔎 <b>Buscar</b>: escribí en el campo del artículo para filtrar; también podés usar el desplegable debajo.</p>
    ...
    <button class="btn btn-primary" style="width:100%" onclick="saveAjusteInv()" ${global.AppSyncGuard && global.AppSyncGuard.isBusy() ? 'disabled' : ''} title="${global.AppSyncGuard && global.AppSyncGuard.isBusy() ? global.AppSyncGuard.waitMessage : ''}">Guardar ajuste(s)</button>
  `);
    setTimeout(() => {
      try {
        global.document.querySelectorAll('#ajuste-lineas .ajuste-linea').forEach((row) => initInvArtComboRow(row));
      } catch (_) { /* noop */ }
    }, 0);
  }
  ...
  async function saveAjusteInv(ctx) {
    ...
    if (container) {
      container.querySelectorAll('.ajuste-linea').forEach((row) => {
        ...
        const artId = artSel && artSel.value ? artSel.value : '';
    ...
    if (typeof global.ensureAntiDesyncBefore === 'function') {
      const ad = await global.ensureAntiDesyncBefore('inventario');
      if (!ad.ok) return;
    }
    ...
    try {
      ...
      for (let li = 0; li < lineas.length; li++) {
        ...
        let newStock;
        try {
          newStock = await syncStockViaRpc(supabaseClient, artId, qtyFinal);
        } catch (prodErr) {
          await rollbackLoteSave(completed);
          throw prodErr;
        }
      ...
      try {
        if (typeof global.refreshCriticalSlice === 'function') {
          const rr = await global.refreshCriticalSlice('inventario');
          if (!rr.ok) {
            notify('warning', '📡', 'No se pudo sincronizar', 'El ajuste se guardó; reintenta para ver datos al día.', { duration: 5500 });
          }
        }
      } catch (_) { /* noop */ }
    } catch (err) {
