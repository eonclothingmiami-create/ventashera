...
(function initInventoryModule(global) {
  ...
  function esArticuloMercCredito(product) {
    if (!product) return false;
    return typeof global.esMercanciaCredito === 'function'
      ? global.esMercanciaCredito(product.tituloMercancia)
      : String(product.tituloMercancia || '').trim().toLowerCase() === 'credito';
  }

  async function removeAbonoInvEntradaIfAny(state, supabaseClient, ajusteId) {
    const marker = global.AppTreasuryModule?.CXPIV_ABONO_MARKER || '[cxp:inv_entrada]';
    const needle = `inv_ajuste_id:${ajusteId}`;
    const abn = (state.tes_abonos_prov || []).find(
      (x) => String(x.nota || '').includes(marker) && String(x.nota || '').includes(needle)
    );
    if (!abn) return;
    try {
      await supabaseClient.from('tes_abonos_prov').delete().eq('id', abn.id);
    } catch (e) {
      console.warn('[abono inv entrada delete]', e);
    }
    state.tes_abonos_prov = (state.tes_abonos_prov || []).filter((x) => x.id !== abn.id);
  }
  ...
  async function eliminarAjusteLote(ctx) {
    ...
    try {
      for (const a of lines) {
        ...
        await removeAbonoInvEntradaIfAny(state, supabaseClient, a.id);
        if (esArticuloMercCredito(art) || a.tipo === 'devolucion') anyCreditoOrDev = true;
      }
  ...
  async function eliminarAjuste(ctx) {
    ...
    try {
      ...
      await removeAbonoInvEntradaIfAny(state, supabaseClient, id);
      ...
      if (movIndex !== -1) state.inv_movimientos.splice(movIndex, 1);
      renderInvAjustes();
      if (esArticuloMercCredito(art) || a.tipo === 'devolucion') {
        if (typeof renderTesPagosProv === 'function') renderTesPagosProv();
        updateNavBadges();
      }
  ...
  function openAjusteModal(ctx) {
    ...
    <p style="font-size:10px;color:var(--text2);margin:-4px 0 0;line-height:1.45">📌 <b>Entrada</b> mercancía a <b>crédito</b> con proveedor y <b>precio de compra &gt; 0</b>: registra deuda en Pagos proveedores (abono negativo = CxP, costo × cantidad). <b>Devolución</b>: reduce deuda. <b>Salida</b>: solo stock.</p>
  ...
  async function saveAjusteInv(ctx) {
    ...
    async function rollbackLoteSave(completed) {
      for (let i = completed.length - 1; i >= 0; i--) {
        const c = completed[i];
        const { product, artId, tipo, cant, devId, abonoInvId } = c;
        if (product) {
        ...
        if (c.abonoInvId) {
          try {
            await supabaseClient.from('tes_abonos_prov').delete().eq('id', c.abonoInvId);
          } catch (_) { /* noop */ }
          state.tes_abonos_prov = (state.tes_abonos_prov || []).filter((x) => x.id !== c.abonoInvId);
        }
      }
      try {
        await supabaseClient.from('inv_ajustes').delete().eq('lote_id', loteId);
    ...
    try {
      ...
      for (let li = 0; li < lineas.length; li++) {
        ...
        if (tipo === 'devolucion' && esArticuloMercCredito(product) && product.proveedorId) {
        ...
        if (tipo === 'entrada' && esArticuloMercCredito(product) && product.proveedorId) {
          const costoUnit = parseFloat(product.precioCompra) || parseFloat(product.cost) || 0;
          if (costoUnit > 0) {
            const marker = global.AppTreasuryModule?.CXPIV_ABONO_MARKER || '[cxp:inv_entrada]';
            const prov = (state.usu_proveedores || []).find((p) => String(p.id) === String(product.proveedorId));
        ...
        if (esArticuloMercCredito(product) || tipo === 'devolucion') anyCreditoOrDev = true;

        completed.push({ product, artId, tipo, cant, devId, abonoInvId });
