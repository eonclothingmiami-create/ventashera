...
(function initInventoryModule(global) {
  ...
  function openAjusteModal(ctx) {
    ...
    <div class="form-row">
      <div class="form-group"><label class="form-label">TIPO</label><select class="form-control" id="m-aj-tipo"><option value="entrada">📥 Entrada</option><option value="salida">📤 Salida</option><option value="devolucion">↩️ Devolución (+stock y, si a crédito con proveedor, −deuda)</option></select></div>
  ...
  async function saveAjusteInv(ctx) {
    ...
    try {
      ...
      if (tipo === 'devolucion' && product.tituloMercancia === 'credito' && product.proveedorId) {
        ...
        if (dErr) console.warn('[devoluciones_prov]', dErr.message);
        else {
      ...
      notify('success', '✅', 'Ajuste guardado', `${tipo === 'entrada' || tipo === 'devolucion' ? '+' : '−'}${cant} uds · Stock: ${product?.stock || 0}${tipo === 'devolucion' && product.tituloMercancia === 'credito' && product.proveedorId ? ' · Deuda proveedor actualizada' : ''}`, { duration: 3500 });
    } catch (err) {
      showLoadingOverlay('hide');
