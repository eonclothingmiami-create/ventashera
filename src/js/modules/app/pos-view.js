        ...
        <div class="pos-cart-footer">
          ...
          <div id="pos-shipping-fields" style="display:${posFormState.canal !== 'vitrina' ? 'flex' : 'none'}; flex-direction:column; gap:8px; margin-bottom:12px; background:var(--bg3); padding:10px; border-radius:10px; border:1px solid var(--border)">
             ...
             <div style="border-top:1px solid var(--border);padding-top:8px;margin-top:2px">
               ...
               ${posFormState.tipoPago === 'contraentrega' ? `<div style="font-size:10px;color:var(--yellow);margin-top:4px">⚠️ <b>Ingreso en caja y totales del día:</b> fecha de <b>esta venta</b>. Cobros pendientes es seguimiento; al liquidar <b>no</b> se suma de nuevo en caja.</div>` : `<div style="font-size:10px;color:var(--green);margin-top:4px">✅ Pago inmediato — registra factura y caja con la fecha de hoy.</div>`}
             </div>
          </div>
          ...
          <div class="form-group" style="margin-bottom:10px; display:flex; gap:8px;">
            ...
            ${posFormState.metodo === 'transferencia' ? `<select class="form-control" id="pos-cuenta" style="padding:8px 12px">${(global.CUENTAS_BANCARIAS || []).map((c) => `<option value="${c}" ${posFormState.cuenta === c ? 'selected' : ''}>${c}</option>`).join('')}</select>` : ''}
          </div>
