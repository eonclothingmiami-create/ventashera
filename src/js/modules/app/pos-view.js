...
(function initPosView(global) {
  ...
  const INTER_METODOS_CONTADO = [
    ...
    { id: 'transferencia', nombre: '🏦 Otra transferencia bancaria' },
  ...
  function normalizePosCanalMetodo(posFormState) {
    ...
    if (posFormState.canal === 'inter') {
      ...
      } else if (posFormState.tipoPago === 'contado' && !interIds.includes(posFormState.metodo)) {
  ...
  function renderPOSLayout(ctx) {
    ...
    const showCuentaBancaria = global.AppCajaLogic?.bucketFromMetodoId
      ? global.AppCajaLogic.bucketFromMetodoId(posFormState.metodo, state.cfg_metodos_pago) === 'transferencia' || String(posFormState.metodo || '') === 'mixto'
      : ['transferencia', 'mixto'].includes(String(posFormState.metodo || ''));
    ...
    <div class="pos-layout">
      ...
      <div class="pos-cart">
        ...
        <div class="pos-cart-footer">
          ...
          ${((posFormState.canal === 'local' && flete > 0) || (posFormState.canal === 'inter' && posFormState.applyFlete && posFormState.flete > 0)) ? `
          <div style="display:flex;justify-content:space-between;margin-bottom:6px">
            <span style="color:var(--text2);font-size:12px">Flete</span>
            <span style="font-size:12px">${fmt(flete)}</span>
          </div>` : ''}
          ...
          ${posFormState.metodo === 'mixto' ? (() => {
            const efe = parseFloat(posFormState.mixtoEfectivo) || 0;
            const trf = parseFloat(posFormState.mixtoTransferencia) || 0;
            const delta = (efe + trf) - total;
            const msg = Math.abs(delta) < 0.01 ? 'Cuadra exacto' : (delta > 0 ? `Sobra ${fmt(delta)}` : `Falta ${fmt(Math.abs(delta))}`);
            const col = Math.abs(delta) < 0.01 ? 'var(--green)' : 'var(--red)';
            return `<div style="background:var(--bg3); padding:12px; border-radius:10px; border:1px dashed var(--border); margin-bottom:12px;">
              <div style="font-size:11px;color:var(--text2);margin-bottom:8px">🧾 Pago mixto: distribuye entre efectivo y transferencia.</div>
              <div style="display:flex;gap:8px;flex-wrap:wrap">
                <div style="flex:1;min-width:160px">
                  <label class="form-label" style="margin:0 0 4px;font-size:10px">💵 Efectivo</label>
                  <input type="number" class="form-control" id="pos-mixto-efectivo" value="${efe || ''}" min="0" step="0.01" placeholder="0">
                </div>
                <div style="flex:1;min-width:160px">
                  <label class="form-label" style="margin:0 0 4px;font-size:10px">🏦 Transferencia</label>
                  <input type="number" class="form-control" id="pos-mixto-transferencia" value="${trf || ''}" min="0" step="0.01" placeholder="0">
                </div>
              </div>
              <div style="margin-top:8px;font-size:11px;color:${col};font-weight:700">${msg}</div>
            </div>`;
          })() : ''}
          ...
          <div class="form-row" style="gap:8px; margin-bottom:8px">
            <input type="text" class="form-control" id="pos-comprobante" placeholder="Comprobante (ref. pago, nota)" style="padding:8px 12px" value="${escAttr(posFormState.comprobante)}" autocomplete="off" spellcheck="false" onpaste="event.stopPropagation()">
          </div>
          <div class="form-row" style="gap:8px; margin-bottom:8px">
          ...
          <div class="form-row" style="gap:8px; margin-bottom:10px">
            ...
            <input type="text" class="form-control" id="pos-telefono" placeholder="${phTel}" style="padding:8px 12px" value="${escAttr(posFormState.telefono)}" inputmode="tel" autocomplete="tel" onpaste="event.stopPropagation()">
          </div>
