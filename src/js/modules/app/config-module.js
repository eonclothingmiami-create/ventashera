...
(function initConfigModule(global) {
  ...
  function forceMonthReset(ctx) {
    if (ctx.confirm('⚠️ ¿Estás seguro? Esto archivará todas las ventas actuales y reiniciará el progreso de la meta mensual. Esta acción no se puede deshacer fácilmente.')) {
      ctx.state.ventas = (ctx.state.ventas || []).map((v) => ({ ...v, archived: true }));
      if (typeof global._erpResetMonthlyMetaBadges === 'function') global._erpResetMonthlyMetaBadges();
      ctx.state.currentMonth = null;
      ctx.state.erpCalendarMonth = null;
      ctx.checkMonthReset();
      ctx.saveConfig('game', ctx.state.game);
      ctx.saveConfig('erp_calendar_month', ctx.state.erpCalendarMonth);
