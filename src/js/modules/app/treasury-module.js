...
(function initTreasuryModule(global) {
  ...
  function renderTesPagosProv(ctx) {
    ...
    <div style="display:flex;gap:6px;margin-bottom:10px;flex-wrap:wrap;align-items:center">
      ...
      <button type="button" class="btn btn-secondary btn-sm" onclick="backfillStockMovesVentaPos()" title="Rellenar movimientos POS">POS</button>
    ...
    <div class="card" style="margin:0 0 12px;padding:10px 12px;font-size:12px;line-height:1.5;border-color:rgba(255,255,255,.08)">
      <div style="display:flex;flex-wrap:wrap;gap:8px 14px;align-items:baseline">
        ...
        <span style="color:var(--border)">|</span>
        <span><b>${fmt(totalAbonos)}</b> <span style="color:var(--text2)">abonos caja</span></span>
        <span style="color:var(--border)">|</span>
        <span><b>${fmt(totalEstimacion)}</b> <span style="color:var(--text2)">estim.</span></span>
      </div>
    </div>
    ${
            ...
            .map((p) => {
              ...
              const ventasHtml =
                ventasLines.length === 0
                  ? '<div style="font-size:11px;color:var(--text2)">Sin ventas POS.</div>'
                  : `<ul style="margin:0;padding-left:16px;font-size:11px;line-height:1.45;color:var(--text2);max-height:140px;overflow:auto">${ventasLines
                      .map((ln) => {
                        ...
                        return `<li>${formatDate(ln.fecha)} · ${ln.art.nombre || ln.art.codigo || ''}: <b>${ln.uds}</b> uds × ${fmt(cost)} = <b>${fmt(sub)}</b> · ${ln.referencia}</li>`;
                      })
                      .join('')}</ul>`;
              const artsRows = (p.articulos || [])
                .map((a) => {
                  ...
                  return `<tr>
                    ...
                    <td style="font-size:15px;font-weight:800;font-variant-numeric:tabular-nums">${a.stock ?? 0}</td>
                    <td style="font-size:12px">${uv}</td>
                    <td style="font-size:15px;font-weight:800;font-variant-numeric:tabular-nums;color:var(--accent)">${fmt(a.precioCompra || 0)}</td>
              ...
              <span><b>${fmt(p.refOperativaTotal)}</b> <span style="color:var(--text2)">ref</span></span><span style="color:var(--border)">|</span>
              <span><b style="color:var(--yellow)">${fmt(p.saldoOperativoEstimado)}</b> <span style="color:var(--text2)">estim</span></span><span style="color:var(--border)">|</span>
              <span><b style="color:var(--green)">${fmt(p.abonos)}</b> <span style="color:var(--text2)">abonos</span></span><span style="color:var(--border)">|</span>
              <span><b style="color:var(--accent)">${fmt(p.devolucionesDeuda || 0)}</b> <span style="color:var(--text2)">devol</span></span>${libroExtra}
              </div>
              <details style="margin-bottom:6px"><summary style="font-size:11px;color:var(--accent);cursor:pointer;font-weight:600">POS</summary><div style="margin-top:6px">${ventasHtml}</div></details>
              <details open style="margin-bottom:8px"><summary style="font-size:12px;cursor:pointer;font-weight:700">Artículos</summary>
              <div class="table-wrap" style="margin-top:6px"><table><thead><tr><th>Ref</th><th>Artículo</th><th>Stock</th><th>Vend.</th><th>Costo u.</th><th>Ref. deuda</th><th></th></tr></thead><tbody>${artsRows || '<tr><td colspan="7" style="text-align:center;color:var(--text2)">—</td></tr>'}</tbody></table></div>
              </details>
    ...
    ${
      abonosRecientes.length > 0
        ? `<div class="card"><div class="card-title" style="font-size:13px">Abonos recientes</div><div class="table-wrap"><table><thead><tr><th>Fecha</th><th>Prov.</th><th>$</th><th>Mét.</th><th>Nota</th><th></th></tr></thead><tbody>${abonosRecientes
            .map(
              (ab) =>
    ...
    ${
      compromisosRecientes.length > 0
        ? `<div class="card"><div class="card-title" style="font-size:13px">Notas libro</div><div class="table-wrap"><table><thead><tr><th>Fecha</th><th>Prov.</th><th>$</th><th>Lín.</th><th>Ref.</th><th>Nota</th><th></th></tr></thead><tbody>${compromisosRecientes
            .map(
              (c) => {
                const nl = getLineasMovimiento(c).length;
                const lineasBrief = nl > 0 ? `${nl} artículo(s)` : '—';
                return `<tr><td>${formatDate(c.fecha)}</td><td style="font-weight:700">${c.proveedorNombre || '—'}</td><td style="color:var(--accent);font-weight:700">${fmt(c.valor || 0)}</td><td style="font-size:11px;color:var(--text2)">${lineasBrief}</td><td style="color:var(--text2);font-size:11px">${c.referencia || '—'}</td><td style="color:var(--text2);font-size:11px">${c.nota || '—'}</td><td><button class="btn btn-xs btn-danger" onclick="eliminarCompromisoProv('${c.id}')">✕</button></td></tr>`;
              }
            )
            .join('')}</tbody></table></div></div>`
        : ''
