...
(function initTreasuryModule(global) {
  ...
  function renderTesPagosProv(ctx) {
    ...
    <details class="card" style="margin:0 0 10px;border-color:rgba(255,255,255,.06);padding:0">
      <summary style="padding:8px 12px;cursor:pointer;font-size:11px;font-weight:600;color:var(--text2);list-style-position:outside;margin-left:12px">Fase 1 — Checklist validación visual (Tonina · Moda Latina)</summary>
      <div style="padding:0 12px 12px;font-size:11px;line-height:1.45;color:var(--text2)">
        ...
        <ul style="margin:0 0 10px;padding-left:18px">
          <li>Primer nivel: solo saldo CXP grande, vend. últ. abono, sugerido, pagos, acciones, tabla — <b>sin pend. est.</b> en la franja.</li>
          <li>Detalle: bloque <b>Libro CXP</b> = cargos / créditos / saldo (igual al saldo grande); <b>Legacy</b> muestra reg. créd. alto sin confundirlo con pagos caja; traza <b>moves</b> vs fallback acorde a base (aquí suele pesar moves).</li>
        </ul>
        <p style="margin:0 0 6px"><b>MODA LATINA</b> (fallback facturas POS; ajustes ± que se anulan)</p>
        <ul style="margin:0 0 10px;padding-left:18px">
          <li>Primer nivel: misma calma; sugerido = <code>min(vend. últ. abono, saldo)</code>; tope útil solo en tooltip o en detalle.</li>
          <li>Detalle: si moves = 0 y hay uds/$ en <b>facturas POS (fallback)</b>, cuadra con Supabase; ajustes por línea visibles en tabla (±).</li>
        </ul>
        <p style="margin:0;font-size:10px;opacity:.85">Siguiente: <b>Fase 2</b> (badges/alerts y alineación). <b>Fase 3</b> inventariará lectores de <code>invoices.items</code> — sin migración aún.</p>
      </div>
    </details>
    <details class="card" style="margin:0 0 12px;border-color:rgba(255,255,255,.08);padding:0">
      <summary style="padding:10px 12px;cursor:pointer;font-weight:600;font-size:12px;list-style-position:outside;margin-left:12px">Totales · <b style="color:var(--yellow)">${fmt(totalSaldo)}</b> Σ saldo · <b style="color:var(--green)">${fmt(totalAbonosPagados)}</b> Σ pagos · ${provConDeuda.length} prov.</summary>
      <div style="padding:0 12px 12px;font-size:11px;line-height:1.5;color:var(--text2)">
        <div style="font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.04em;color:var(--text2);margin-bottom:6px">Desglose global (libro + operativo + legacy)</div>
        <div style="display:flex;flex-wrap:wrap;gap:8px 14px;align-items:baseline;margin-bottom:8px">
          <span><b style="color:var(--red)">${fmt(totalCxpCargo)}</b> CXP cargos</span><span style="color:var(--border)">|</span>
          <span><b style="color:var(--green)">${fmt(totalCxpCredito)}</b> CXP créditos</span><span style="color:var(--border)">|</span>
          <span title="Ref. operativa (inv.+vendido+ajuste mód.)"><b>${fmt(totalDeudaOperativa)}</b> ref.</span><span style="color:var(--border)">|</span>
          <span title="Solo valores &gt; 0 en tes_abonos_prov"><b>${fmt(totalAbonosPagados)}</b> pagos caja</span>${
            totalAbonosRegCredAbs > 0.5
              ? `<span style="color:var(--border)">|</span><span title="Abonos negativos / inv_entrada_credito legacy; no es efectivo caja"><b style="color:var(--orange)">${fmt(totalAbonosRegCredAbs)}</b> reg. créd.</span>`
              : ''
          }<span style="color:var(--border)">|</span>
          <span title="ref. − pagos caja − devol."><b>${fmt(totalEstimacion)}</b> pend. est.</span><span style="color:var(--border)">|</span>
          <span title="Σ vendido desde último abono &gt; 0"><b style="color:var(--accent)">${fmt(totalVendidoDesdeUltimoAbono)}</b> Σ vend. últ. abono</span>
        </div>
        <div style="font-size:10px;opacity:.88;line-height:1.4">Cada proveedor desglosa CXP vs operativo en <b>Operativo, libro y reconciliación</b>. Vendido POS / fallback factura: detalle por tarjeta. Auditoría: <code>__erpAuditVendidoProv</code> y SQL <code>audit_proveedor_vendido_pos_vs_facturas</code>.</div>
      </div>
    </details>
    ${
  ...
  function openAbonoProvModal(ctx) {
    ...
    <button type="button" class="btn btn-primary" id="ab-btn-guardar" style="width:100%" onclick="guardarAbonoProv()">💳 Guardar Abono</button>`);
    if (provId) {
      setTimeout(() => {
