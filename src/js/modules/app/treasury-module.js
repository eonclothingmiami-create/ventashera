...
(function initTreasuryModule(global) {
  ...
  function renderTesPagosProv(ctx) {
    ...
    el.innerHTML = `
    <div class="card" style="margin:0 0 16px;border-color:rgba(0,229,180,.25);padding:20px 24px">
      <div class="card-title" style="margin-bottom:8px">Pagos proveedores</div>
      <p style="font-size:15px;font-weight:700;margin:0 0 8px;color:var(--accent)">Módulo en reconstrucción</p>
      <p style="font-size:13px;color:var(--text2);margin:0;line-height:1.5">La información histórica está preservada mientras se audita y reconstruye la lógica. No uses los flujos antiguos de cargo, abono o sincronización POS desde aquí.</p>
      <p style="font-size:12px;color:var(--text2);margin:10px 0 0;line-height:1.45">Plan de respaldo y auditoría: <code>docs/pagos-proveedores-rebuild-plan.md</code> y <code>supabase/audits/pagos_proveedores_audit.sql</code> (solo lectura; ejecutar en Supabase SQL Editor, no desde la app).</p>
    </div>
