...
(function initTreasuryModule(global) {
  /**
   * Deuda operativa (a costo) = stock actual a costo + vendido POS (histórico). Solo baja con abonos.
   * Ajustes de salida (p. ej. devolución al proveedor) bajan la deuda porque bajan el stock; no se suman al vendido.
   * inv_ajustes se usa solo como desglose informativo (coherencia con «en stock»).
   */
  function calcDeudaProveedor(state, provId) {
    ...
    for (let j = 0; j < ajustes.length; j++) {
      ...
      if (!artAj || artAj.tituloMercancia !== 'credito') continue;
      if (esSinEspecificar) {
        if (artAj.proveedorId) continue;
      } else if (String(artAj.proveedorId) !== String(provId)) continue;
  ...
  function renderTesPagosProv(ctx) {
    ...
    <div style="display:flex;gap:10px;margin-bottom:12px;flex-wrap:wrap;align-items:center">
      ...
      <div style="margin-left:auto;font-size:11px;color:var(--text2);max-width:520px;text-align:right">
        <b>Deuda</b> = stock + vendido POS a costo. <b>Salidas</b> (dev. al proveedor) bajan el <b>stock</b>; <b>abonos</b> bajan el saldo. <b>Saldo</b> = deuda − abonos.
      </div>
    </div>
    <div class="card" style="margin-bottom:14px;padding:10px 12px;font-size:11px;color:var(--text2);line-height:1.5">
      📌 <b>Total deuda</b> = stock a costo + vendido POS a costo. Una <b>salida</b> por devolución al proveedor <b>reduce el stock</b>, así que <b>baja la deuda</b> (no se “sume” aparte en vendido). <b>Solo los abonos</b> reducen el saldo pendiente. Si un ajuste no cambió el stock, recarga la página o revisa que el artículo exista en sesión.
    </div>
      ...
      provConDeuda.length === 0
        ...
        : provConDeuda
            .map((p) => {
              ...
              <div style="font-size:10px;color:var(--text2);margin:6px 0 8px">Desglose: en stock ${fmt(p.valorInventarioCosto)} · ya vendido (POS a costo) ${fmt(p.costoVendidoHist)} (${p.unidadesVendidasHist} uds) · ajustes salida hist. ${fmt(p.ajustesSalidaCosto || 0)} (${p.ajustesSalidaUds || 0} uds, ya reflejados en stock si el ajuste se aplicó bien). Refs. artículos: ${p.articulos.length}</div>
  ...
  function verAbonosProv(ctx) {
    ...
    <div><span style="color:var(--text2)">Abonado:</span> <b style="color:var(--green)">${fmt(d.abonos)}</b></div>
    <div><span style="color:var(--text2)">Saldo por pagar:</span> <b style="color:var(--yellow)">${fmt(d.saldo)}</b></div></div>
    <div style="font-size:11px;color:var(--text2);margin-bottom:12px;line-height:1.45">Desglose: en stock ${fmt(d.valorInventarioCosto)} · ya vendido (POS a costo) ${fmt(d.costoVendidoHist)} (${d.unidadesVendidasHist} uds) · ajustes salida (hist.) ${fmt(d.ajustesSalidaCosto || 0)} (${d.ajustesSalidaUds || 0} uds, ya en stock)</div>
