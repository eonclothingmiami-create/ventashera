...
(function initTreasuryModule(global) {
   ...
   * `tes_abonos_prov`: **positivos** = pagos; **negativos** = reconocimiento de deuda (ej. `CXPIV_ABONO_MARKER`). Entran en `abonos` y en `saldoOperativoEstimado`; no se rebaja otra vez del inventario base (evita doble descuento frente al stock real).
   * Devoluciones operativas = `tes_devoluciones_prov` + N/C CXP sin espejo de esas devoluciones.
   * Saldo por pagar: con libro CXP usa saldo oficial; si no, `refOperativaTotal − abonos − devoluciones`.
   */
