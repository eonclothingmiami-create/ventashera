...
(function initTreasuryModule(global) {
  ...
  function renderTesPagosProv(ctx) {
    ...
    <div class="card" style="margin:0 0 12px;padding:10px 12px;font-size:12px;line-height:1.5;border-color:rgba(255,255,255,.08)">
      <div style="display:flex;flex-wrap:wrap;gap:8px 14px;align-items:baseline">
        ...
        <span style="color:var(--border)">|</span>
        <span title="Suma por proveedor: ref. − pagos caja (&gt;0) − devoluciones (sin CXP). Sin doble conteo con reg. créd."><b>${fmt(totalEstimacion)}</b> <span style="color:var(--text2)">pend. est.</span></span>
        <span style="color:var(--border)">|</span>
        <span title="Suma de vendido a costo desde el último abono positivo por proveedor (ver tarjeta)"><b style="color:var(--accent)">${fmt(totalVendidoDesdeUltimoAbono)}</b> <span style="color:var(--text2)">Σ vend. últ. abono</span></span>
      </div>
    </div>
    ${
            ...
            .map((p) => {
              ...
              const artsRows = (p.articulos || [])
                .map((a) => {
                  ...
                  const deltaHint =
                    deltaU !== 0
                      ? `<div style="font-size:9px;font-weight:400;color:var(--text2);line-height:1.35">ajuste mód. ${deltaU > 0 ? '+' : ''}${deltaU} uds → <span style="color:var(--red);font-weight:700">${adjMon >= 0 ? '+' : ''}${fmt(adjMon)}</span> <span style="opacity:.85">(Δ×costo)</span></div>`
                      : '';
                  ...
                  return `<tr>
                    ...
                    <td class="tes-pp-num" style="font-size:12px" title="Solo movimientos POS/facturas; el ajuste ± del módulo no suma aquí">${uv}</td>
                    <td class="tes-pp-num" style="font-size:15px;font-weight:800;color:var(--accent)">${fmt(a.precioCompra || 0)}</td>
                    <td class="tes-pp-num" style="color:var(--red);font-weight:700" title="Total referencia = stock×costo + vendido×costo + ajuste mód.×costo. El número grande es ese total; debajo, solo el importe Δ×costo (igual que el modal/resumen).">${fmt(linea)}${deltaHint}</td>
              ...
              <div style="margin-bottom:10px;padding:10px 12px;border-radius:8px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1)">
                <div style="font-size:9px;font-weight:700;text-transform:uppercase;color:var(--text2);letter-spacing:.04em;margin-bottom:6px">Resumen deuda / ventas (calcDeudaProveedor)</div>
                <div style="display:flex;flex-wrap:wrap;gap:8px 14px;font-size:12px;line-height:1.45;align-items:baseline">
                  <span><span style="color:var(--text2)">ref.</span> <b>${fmt(p.refOperativaTotal)}</b></span>
                  <span style="color:var(--border)">·</span>
                  <span><span style="color:var(--text2)">pend. est.</span> <b style="color:var(--yellow)">${fmt(p.saldoOperativoEstimado)}</b></span>
                  <span style="color:var(--border)">·</span>
                  <span><span style="color:var(--text2)">inv. a costo</span> <b style="color:var(--accent)">${fmt(p.valorInventarioCosto)}</b></span>
                  <span style="color:var(--border)">·</span>
                  <span><span style="color:var(--text2)">vendido total</span> <b>${fmt(p.costoVendidoHist)}</b> <span style="font-size:10px;color:var(--text2)">(${p.unidadesVendidasHist || 0} u)</span></span>
                  <span style="color:var(--border)">·</span>
                  <span><span style="color:var(--text2)">💰 vend. últ. abono</span> <b style="color:${colorVendDesdeAb}">${fmt(vendDesdeAbNum)}</b> <span style="font-size:10px;color:var(--text2)">(${p.unidadesVendidasDesdeUltimoAbono || 0} u)</span></span>
                </div>
                <div style="font-size:10px;color:var(--text2);margin-top:6px">Último abono (ref.): <b style="color:var(--text1)">${fechaUltAbEsc}</b> · <span style="opacity:.9">Verde si vend. últ. abono ≥ saldo (${fmt(saldoNum)})</span></div>
              </div>
              <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:8px;margin-bottom:8px">
                ...
                <div style="border-left:3px solid rgba(251,191,36,.85);padding:8px 10px;background:rgba(251,191,36,.07);border-radius:8px">
                  <div style="font-size:9px;font-weight:700;text-transform:uppercase;color:var(--yellow);margin-bottom:2px">Vendido de este proveedor</div>
                  <div style="font-family:Syne;font-size:22px;font-weight:800;line-height:1.1">${fmt(p.costoVendidoHist)}</div>
                  <div style="font-size:12px;font-weight:700;margin-top:2px">${p.unidadesVendidasHist || 0} uds <span style="font-weight:600;color:var(--text2)">a costo</span></div>
                  <div style="font-size:9px;color:var(--text2);margin-top:4px;line-height:1.35">Cruce <b>ventas POS</b> (y facturas sin move en estado) × artículos <b>crédito</b> de este proveedor. Incluye vitrina.</div>
                  <div style="margin-top:8px;padding-top:8px;border-top:1px solid rgba(255,255,255,.08)">
                    <div style="font-size:9px;font-weight:700;text-transform:uppercase;color:var(--green);margin-bottom:2px">Tope útil (a costo)</div>
                    <div style="font-family:Syne;font-size:18px;font-weight:800;line-height:1.1;color:var(--green)" title="min(pend. est., vendido a costo). Referencia conservadora; no sustituye ver caja ni margen.">${fmt(p.topePagoRespaldadoVentas || 0)}</div>
                    ${
                  ...
                  <div style="margin-top:8px;padding-top:8px;border-top:1px solid rgba(255,255,255,.08)">
                    <div style="font-size:9px;font-weight:700;text-transform:uppercase;margin-bottom:2px;color:var(--text2)">💰 Vendido desde último abono</div>
                    <div style="font-family:Syne;font-size:18px;font-weight:800;line-height:1.1;color:${colorVendDesdeAb}" title="Ventas a costo posteriores al último pago (valor &gt; 0) registrado para este proveedor. Sin abonos positivos = mismo total que vendido histórico.">${fmt(vendDesdeAbNum)}</div>
                    <div style="font-size:10px;color:var(--text2);margin-top:4px;line-height:1.35">Último abono (ref.): <b style="color:var(--text1)">${fechaUltAbEsc}</b></div>
                    <div style="font-size:9px;color:var(--text2);margin-top:2px">Comparado con saldo mostrado arriba (${p.usaCxp ? 'CXP' : 'estim.'}).</div>
                  </div>
                </div>
              </div>
              ...
              <span title="max(0, ref. − pagos caja (&gt;0) − devoluciones). Los abonos negativos (reg. créd.) no entran aquí: la ref. ya refleja ese inventario. Debe cuadrar ≈ ref. − pagos − devol."><b style="color:var(--yellow)">${fmt(p.saldoOperativoEstimado)}</b> <span style="color:var(--text2)">pend. est.</span></span><span style="color:var(--border)">|</span>
              <span title="min(pend. est., vendido a costo) — referencia para pagar sin apalancar solo con mercancía ya vendida (a precio compra)"><b style="color:var(--green)">${fmt(p.topePagoRespaldadoVentas || 0)}</b> <span style="color:var(--text2)">tope vendido</span></span><span style="color:var(--border)">|</span>
              ...
              <div class="table-wrap tes-pp-table-wrap" style="margin-top:6px"><table class="tes-pp-table"><thead><tr><th class="tes-pp-ref">Ref</th><th>Artículo</th><th class="tes-pp-num">Stock</th><th class="tes-pp-num" title="Solo unidades vendidas POS/facturas (sin ajuste módulo)">Vend.</th><th class="tes-pp-num">Costo u.</th><th class="tes-pp-num">Ref. deuda</th><th class="tes-pp-actions"></th></tr></thead><tbody>${artsRows || '<tr><td colspan="7" style="text-align:center;color:var(--text2)">—</td></tr>'}</tbody></table></div>
              </details>
  ...
  function verAbonosProv(ctx) {
    ...
    <div style="font-size:11px;color:var(--text2);margin-bottom:12px;line-height:1.45">Desglose: en stock ${fmt(d.valorInventarioCosto)}${
      (d.invEntradaCredAbonoDedup || 0) > 0.5
        ...
        : ''
    } · ya vendido (POS a costo, <b>incluye vitrina</b>) ${fmt(d.costoVendidoHist)} (${d.unidadesVendidasHist} uds)${
      Math.abs(d.ajusteUnidadesCosto || 0) > 1e-9
  ...
  function openAjusteUnidadesProvModal(ctx) {
    ...
    <div style="font-size:12px;margin-bottom:10px"><b>${tituloArt}</b><div style="font-size:10px;color:var(--text2);margin-top:4px">${provNombre} · acumulado ajuste: <b>${acum > 0 ? '+' : ''}${acum}</b> uds</div><div style="font-size:11px;color:var(--accent);margin-top:6px;line-height:1.4">Impacto acumulado a costo actual: <b>${acum === 0 ? '—' : fmt(acum * pc)}</b>${acum !== 0 ? ` <span style="color:var(--text2)">(${acum > 0 ? '+' : ''}${acum} uds × ${fmt(pc)})</span>` : ''}</div></div>
