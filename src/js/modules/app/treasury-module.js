...
(function initTreasuryModule(global) {
   ...
   * `tes_abonos_prov`: valores **positivos** = abonos pagados; **negativos** = deuda registrada (ej. entrada inventario a crédito con `CXPIV_ABONO_MARKER`).
  ...
  function calcDeudaProveedor(state, provId) {
    ...
    let dbgUdsMovesOnly = 0;
    let dbgInvoiceFallbackUds = 0;
      ...
      try {
        fetch('http://127.0.0.1:7397/ingest/fdd15a6c-f2bb-4e50-9cda-32cc3a03f3ff', {
          ...
          body: JSON.stringify({
            ...
            runId: 'post-fix',
            hypothesisId: 'B',
            location: 'treasury-module.js:calcDeudaProveedor',
            message: 'POS vendido por proveedor (diag)',
            data: {
              provId: String(provId),
              artCreditoCount: articulos.length,
              movesTotal: (state.stock_moves_ventas || []).length,
              udsFromMovesOnly: dbgUdsMovesOnly,
              udsFromInvoiceFallback: dbgInvoiceFallbackUds,
              costoVendidoHist,
              unidadesVendidasHist,
              sampleArtIds: articulos.slice(0, 4).map((a) => String(a.id)),
            },
            timestamp: Date.now(),
          }),
        }).catch(() => {});
