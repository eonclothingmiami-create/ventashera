...
(function initTreasuryModule(global) {
  ...
  function canalVentaPos(state, m) {
    ...
    let v = ventas.find((x) => String((x.desc || '').trim()) === ref);
    if (!v) v = ventas.find((x) => String(x.desc || '').includes(ref));
    if (!v && state.facturas) {
      const f = (state.facturas || []).find((x) => String((x.numero || '').trim()) === ref);
      if (f) v = ventas.find((x) => x.id === f.id);
    }
  ...
  async function guardarAbonoProv(ctx) {
    ...
    async function rollbackAbonoEnNube() {
      try {
        await supabaseClient.from('tes_abonos_prov').delete().eq('id', abono.id);
      } catch (e) {
        console.warn('[abono rollback tes_abonos_prov]', e.message || e);
      }
      try {
        await supabaseClient.from('tes_cxp_movimientos').delete().eq('id', cxpAbonoId);
      } catch (e) {
        console.warn('[abono rollback cxp]', e.message || e);
      }
    }

    function quitarAbonoYCxpDelEstado() {
      state.tes_abonos_prov = (state.tes_abonos_prov || []).filter((x) => x.id !== abono.id);
    ...
    try {
      ...
      if (cxpErr) {
        await rollbackAbonoEnNube();
        throw new Error('Libro CXP: ' + (cxpErr.message || 'no se pudo registrar el abono en cuenta por pagar.'));
      }
      ...
      if (cajaAbierta) {
        ...
        const mov = {
          ...
          tipo: 'egreso',
          valor,
          concepto: `Abono proveedor: ${provNombre} · ref:${abono.id}`,
          fecha,
          metodo,
        ...
        const okCaja = await saveRecord('cajas', cajaAbierta.id, cajaAbierta);
        if (!okCaja) {
          global.AppCajaLogic?.applyDeltaBucket?.(cajaAbierta, bucket, valor);
          quitarAbonoYCxpDelEstado();
          await rollbackAbonoEnNube();
          throw new Error('No se pudo guardar la caja en la nube (revisa RLS o conexión).');
        }
        ...
        if (!okMov) {
          ...
          quitarAbonoYCxpDelEstado();
          await rollbackAbonoEnNube();
          throw new Error('No se pudo guardar el egreso en tes_movimientos; el abono no se aplicó.');
        }
