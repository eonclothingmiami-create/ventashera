...
(function initTreasuryModule(global) {
   ...
   * El monto negativo entra en `abonos` y reduce el saldo estimado; la base inventario usa el stock real (no se resta de nuevo aquí).
   */
  const CXPIV_ABONO_MARKER = '[cxp:inv_entrada]';
  /** Misma tolerancia que `tes_abono_proveedor_aplicar` en Supabase (evita rechazo por centavos). */
  const ABONO_PROV_SALDO_TOLERANCIA = 0.05;
  ...
  function tsMsFromStockMoveFecha(m) {
    if (!m || m.fecha == null || m.fecha === '') return NaN;
    const s = String(m.fecha);
    if (s.length <= 10) return Date.parse(s + 'T12:00:00');
    return Date.parse(s);
  }

  function tsMsFromAbonoPositivo(ab) {
    if (!ab) return NaN;
    if (ab.fechaHora) {
      const t = Date.parse(ab.fechaHora);
      if (!Number.isNaN(t)) return t;
    }
    const d = normFechaMov(ab.fecha);
    if (d) return Date.parse(d + 'T23:59:59.999');
    return NaN;
  }

  /** Último abono con valor &gt; 0: instante de corte para ventas posteriores. Sin abonos → corteMs null (todo el histórico). */
  function corteVentasDesdeUltimoAbono(state, provId) {
    const pos = (state.tes_abonos_prov || []).filter(
      (ab) => String(ab.proveedorId) === String(provId) && parseFloat(ab.valor) > 0,
    );
    if (!pos.length) return { corteMs: null, fechaUltimoAbonoLabel: null };
    let bestAb = null;
    let bestMs = -Infinity;
    pos.forEach((ab) => {
      const t = tsMsFromAbonoPositivo(ab);
      if (!Number.isNaN(t) && t > bestMs) {
        bestMs = t;
        bestAb = ab;
      }
    });
    if (!bestAb) return { corteMs: null, fechaUltimoAbonoLabel: null };
    const fh = bestAb.fechaHora ? String(bestAb.fechaHora) : '';
    const fechaUltimoAbonoLabel = fh ? fh.replace('T', ' ').slice(0, 19) : normFechaMov(bestAb.fecha) || '—';
    return { corteMs: bestMs, fechaUltimoAbonoLabel };
  }
  ...
  function articuloUnidadesVendidasPosMovesAfter(state, artId, corteMs) {
    let u = 0;
    (state.stock_moves_ventas || []).forEach((m) => {
      if (String(m.productId) !== String(artId)) return;
      if (corteMs != null) {
        const tm = tsMsFromStockMoveFecha(m);
        if (Number.isNaN(tm) || tm <= corteMs) return;
      }
      u +=
        typeof global.unidadesVentaPosAbs === 'function'
          ? global.unidadesVentaPosAbs(m)
  ...
  function unidadesFacturasPosSinStockMoveEnEstadoAfter(state, artId, corteMs) {
    const netFn =
      global.AppPosRepository && typeof global.AppPosRepository.netCantidadMovesDocProduct === 'function'
        ? global.AppPosRepository.netCantidadMovesDocProduct
        : null;
    ...
    for (let fi = 0; fi < facturas.length; fi++) {
      ...
      if (!docId) continue;
      if (corteMs != null) {
        const fds = normFechaMov(f.fecha);
        const fts = fds ? Date.parse(fds + 'T12:00:00') : NaN;
        if (Number.isNaN(fts) || fts <= corteMs) continue;
      }
      let items = f.items;
      if (typeof items === 'string' && items.trim()) {
        try {
  ...
  /**
   * Deuda informativa (a costo) = stock×costo + vendido POS×costo + `tes_ajustes_unidades_prov` (módulo pagos).
   * Solo artículos con `tituloMercancia` = mercancía a crédito y `proveedorId` = este proveedor.
   * `inv_ajustes` entrada/salida **no se suman otra vez** a `refOperativaTotal`: el `stock` del artículo ya refleja esos movimientos cuando el inventario está alineado.
   * Las líneas `stock_moves` tipo venta_pos suman a «vendido» si el producto es crédito de este proveedor; **no se excluye vitrina**.
   * Si no hay movimientos en estado para una factura POS, se usan las líneas de `state.facturas` (misma lógica que «POS moves»).
   * `tes_abonos_prov`: **positivos** = pagos en caja; **negativos** = reconocimiento de deuda (ej. `CXPIV_ABONO_MARKER`). `saldoOperativoEstimado` usa solo **pagos > 0** y devoluciones: los negativos no se restan en esa fórmula porque la base `ref.` ya incluye el stock que esos abonos documentan (evita doble conteo).
   * Devoluciones operativas = `tes_devoluciones_prov` + N/C CXP sin espejo de esas devoluciones.
  ...
  function calcDeudaProveedor(state, provId) {
    ...
    const articuloCuentaComoCredito = (a) => {
      if (!a) return false;
      if (a.esCreditoProveedor != null) return !!a.esCreditoProveedor;
      return esMercCreditoTitulo(a.tituloMercancia);
    };
    const articulos = (state.articulos || []).filter((a) => {
      if (!articuloCuentaComoCredito(a)) return false;
    ...
    const valorInventarioCostoNeto = Math.max(0, valorInventarioCosto);

    const { corteMs, fechaUltimoAbonoLabel } = corteVentasDesdeUltimoAbono(state, provId);

    let costoVendidoHist = 0;
    let unidadesVendidasHist = 0;
    let costoVendidoPosMoves = 0;
    let udsVendidasPosMoves = 0;
    let costoVendidoPosFacturaSinMove = 0;
    let udsVendidasPosFacturaSinMove = 0;
    let vendidoDesdeUltimoAbono = 0;
    let unidadesVendidasDesdeUltimoAbono = 0;
    let costoVendidoUltAbPosMoves = 0;
    let udsVendidasUltAbPosMoves = 0;
    let costoVendidoUltAbFacturaSinMove = 0;
    let udsVendidasUltAbFacturaSinMove = 0;
    for (let ai = 0; ai < articulos.length; ai++) {
      ...
      costoVendidoPosMoves += uM * cost;
      udsVendidasPosMoves += uM;
      costoVendidoPosFacturaSinMove += uI * cost;
      udsVendidasPosFacturaSinMove += uI;
      const uM2 = articuloUnidadesVendidasPosMovesAfter(state, a.id, corteMs);
      const uI2 = unidadesFacturasPosSinStockMoveEnEstadoAfter(state, a.id, corteMs);
      const udsAb = uM2 + uI2;
      vendidoDesdeUltimoAbono += udsAb * cost;
      unidadesVendidasDesdeUltimoAbono += udsAb;
      costoVendidoUltAbPosMoves += uM2 * cost;
      udsVendidasUltAbPosMoves += uM2;
      costoVendidoUltAbFacturaSinMove += uI2 * cost;
      udsVendidasUltAbFacturaSinMove += uI2;
    }
    ...
    (state.tes_ajustes_unidades_prov || []).forEach((r) => {
      ...
      unidadesAjusteModulo += du;
    });
    const ajustesInvNeto = ajustesEntradaCosto - ajustesSalidaCosto;
    ...
    const saldoOperativoEstimado = Math.max(0, refOperativaTotal - abonosPagados - devolucionesOperativa);
    /** A costo: mínimo entre lo pendiente (est.) y lo ya vendido; referencia conservadora para “pagar con lo que ya salió”. No es caja real ni margen. */
    const topePagoRespaldadoVentas = Math.min(saldoOperativoEstimado, costoVendidoNeto);
    const pendienteSinRespaldoVentaCosto = Math.max(0, saldoOperativoEstimado - costoVendidoNeto);
    const cxp = sumCxpProveedor(state, provId);
    const saldo = cxp.hayCxp ? cxp.saldoOficial : saldoOperativoEstimado;
    const vendidoDesdeUltimoAbonoNeto = Math.max(0, vendidoDesdeUltimoAbono);
    const vendidoDesdeUltimoAbonoAlcanzaSaldo = vendidoDesdeUltimoAbonoNeto + 1e-6 >= saldo;
    ...
    return {
      ...
      /** Vendido a costo desde `state.stock_moves_ventas` (venta_pos), sin fallback factura. */
      costoVendidoPosMoves,
      udsVendidasPosMoves,
      /** Hueco cubierto por líneas de facturas POS cuando el neto de moves en estado ≈ 0 (misma regla que `unidadesFacturasPosSinStockMoveEnEstado`). */
      costoVendidoPosFacturaSinMove,
      udsVendidasPosFacturaSinMove,
      vendidoDesdeUltimoAbono: vendidoDesdeUltimoAbonoNeto,
      unidadesVendidasDesdeUltimoAbono: Math.max(0, unidadesVendidasDesdeUltimoAbono),
      costoVendidoUltAbPosMoves,
      udsVendidasUltAbPosMoves,
      costoVendidoUltAbFacturaSinMove,
      udsVendidasUltAbFacturaSinMove,
      fechaUltimoAbonoLabel,
      vendidoDesdeUltimoAbonoAlcanzaSaldo,
      refOperativaTotal,
      compromisoReconocido,
      compromisoTotal: compromisoReconocido,
      deudaBruta: refOperativaTotal,
      ajusteUnidadesCosto,
      unidadesAjusteModulo,
      abonos,
      abonosPagados,
      ...
      topePagoRespaldadoVentas,
      pendienteSinRespaldoVentaCosto,
      cxpCargo: cxp.cargo,
  ...
  /**
   * Unidades base de la referencia de deuda a costo (misma base que `lineaRefDeudaArticulo` / `calcDeudaProveedor`).
   * No es stock físico ni solo el ajuste virtual: es stock + vendido POS (+ hueco facturas) + ΣΔ módulo.
   */
  function udsRefDeudaArticulo(state, art, provId) {
  ...
  function renderTesPagosProv(ctx) {
    ...
    const totalVendidoDesdeUltimoAbono = provConDeuda.reduce(
      (s, p) => s + (parseFloat(p.vendidoDesdeUltimoAbono) || 0),
      0,
    );
    ...
    const showPagosProvQaChecklist =
      typeof global !== 'undefined' && global.__ERP_DEV_PAGOS_PROV_QA__ === true;
    ...
    ${
      showPagosProvQaChecklist
        ? `<details class="card" style="margin:0 0 10px;border-color:rgba(255,255,255,.06);padding:0">
      <summary style="padding:8px 12px;cursor:pointer;font-size:11px;font-weight:600;color:var(--text2);list-style-position:outside;margin-left:12px">QA dev — Checklist Pagos prov. (Tonina · Moda Latina)</summary>
      <div style="padding:0 12px 12px;font-size:11px;line-height:1.45;color:var(--text2)">
        <p style="margin:0 0 6px;font-size:10px;opacity:.9">Activa con <code>window.__ERP_DEV_PAGOS_PROV_QA__ = true</code> y recarga la vista.</p>
        <p style="margin:0 0 6px"><b>CREACIONES TONINA</b> (CXP + moves; reg. créd. legacy)</p>
        <ul style="margin:0 0 10px;padding-left:18px">
          <li>Primer nivel: saldo CXP, vend. últ. abono, sugerido, pagos, acciones, tabla — sin <b>pend. est.</b> en la franja.</li>
          <li>Detalle: <b>Libro CXP</b> = cargos / créditos / saldo; <b>Legacy</b> = reg. créd.; traza moves vs fallback.</li>
        </ul>
        <p style="margin:0 0 6px"><b>MODA LATINA</b> (fallback facturas; ajustes ±)</p>
        <ul style="margin:0 0 0;padding-left:18px">
          <li>Detalle: moves = 0 y uds/$ en fallback; ajustes en tabla.</li>
        </ul>
      </div>
    </details>`
        : ''
    }
    <details class="card" style="margin:0 0 12px;border-color:rgba(255,255,255,.08);padding:0">
      <summary style="padding:10px 12px;cursor:pointer;font-weight:600;font-size:12px;list-style-position:outside;margin-left:12px">Totales · <b style="color:var(--yellow)">${fmt(totalSaldo)}</b> saldo · <b style="color:var(--green)">${fmt(totalAbonosPagados)}</b> pagos · ${provConDeuda.length} prov.</summary>
      <div style="padding:0 12px 10px;font-size:10px;line-height:1.45;color:var(--text2)">
        <div style="display:flex;flex-wrap:wrap;gap:6px 10px;align-items:baseline">
          <span title="CXP cargos"><b style="color:var(--red)">${fmt(totalCxpCargo)}</b> c CXP</span><span style="color:var(--border)">·</span>
          <span title="CXP créditos"><b style="color:var(--green)">${fmt(totalCxpCredito)}</b> a CXP</span><span style="color:var(--border)">·</span>
          <span title="Ref. operativa"><b>${fmt(totalDeudaOperativa)}</b> ref.</span><span style="color:var(--border)">·</span>
          <span title="Abonos &gt; 0"><b>${fmt(totalAbonosPagados)}</b> pagos</span>${
            totalAbonosRegCredAbs > 0.5
              ? `<span style="color:var(--border)">·</span><span title="Registro crédito (no caja)"><b style="color:var(--orange)">${fmt(totalAbonosRegCredAbs)}</b> reg. créd.</span>`
              : ''
          }<span style="color:var(--border)">·</span>
          <span title="Pend. est."><b>${fmt(totalEstimacion)}</b> pend. est.</span><span style="color:var(--border)">·</span>
          <span title="Σ vend. últ. abono"><b style="color:var(--accent)">${fmt(totalVendidoDesdeUltimoAbono)}</b> vend. últ. ab.</span>
        </div>
        <div style="font-size:9px;opacity:.75;margin-top:6px">Por proveedor: <b>Operativo, libro y reconciliación</b>. Consola: <code>__erpAuditVendidoProv</code></div>
      </div>
    </details>
    ${
      provConDeuda.length === 0
        ...
        : provConDeuda
            .map((p) => {
              ...
              const vendDesdeAbNum = parseFloat(p.vendidoDesdeUltimoAbono) || 0;
              const colorVendDesdeAb =
                vendDesdeAbNum + 1e-6 >= saldoNum ? 'var(--green)' : 'var(--yellow)';
              const fechaUltAbEsc = p.fechaUltimoAbonoLabel
                ? String(p.fechaUltimoAbonoLabel).replace(/</g, '&lt;')
                : '— sin abonos positivos · se usa todo el histórico';
              ...
              const sugeridoPago = Math.max(0, Math.min(vendDesdeAbNum, saldoNum));
              const pendMenosCxp = (p.saldoOperativoEstimado || 0) - saldoNum;
              const saldoHeaderTitle = p.usaCxp
                ? `Saldo oficial CXP (cargos − créditos). Cargos ${fmt(p.cxpCargo)} · Abonos libro ${fmt(p.cxpCredito)}`
                : `Pendiente estimado operativo (sin libro CXP para este proveedor)`;
              const pendEstEnPrincipal = !p.usaCxp
                ? `<span style="color:var(--border)">|</span><span title="max(0, ref. − pagos caja &gt;0 − devoluciones). Con CXP el saldo principal es el libro; esto solo en vista sin CXP."><b style="color:var(--yellow)">${fmt(p.saldoOperativoEstimado)}</b> <span style="color:var(--text2)">pend. est.</span></span>`
                : '';
              const refCxpReco =
                p.usaCxp && p.difEstimVsCxp != null && Math.abs(p.difEstimVsCxp) > 0.5
                  ? `<span style="color:var(--text2)" title="ref. operativa − saldo oficial CXP">ref.−CXP ${p.difEstimVsCxp > 0 ? '+' : ''}${fmt(p.difEstimVsCxp)}</span>`
                  : '';
              const penCxpReco =
                p.usaCxp && Math.abs(pendMenosCxp) > 0.5
                  ? `<span style="color:var(--text2)" title="pend. est. − saldo CXP">pen.−CXP ${pendMenosCxp > 0 ? '+' : ''}${fmt(pendMenosCxp)}</span>`
                  : '';
              const recoLine =
                p.usaCxp && (refCxpReco || penCxpReco)
                  ? `<div style="margin-top:8px;padding-top:8px;border-top:1px solid rgba(255,255,255,.08);font-size:10px;line-height:1.5;color:var(--text2)"><span style="font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.03em;color:var(--text2)">Reconciliación</span> · ${[refCxpReco, penCxpReco].filter(Boolean).join(' · ')}</div>`
                  : '';
              const cxpLibroBloque = p.usaCxp
                ? `<div style="margin-bottom:10px;padding-bottom:10px;border-bottom:1px solid rgba(255,255,255,.08)">
                    <div style="font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.04em;color:var(--accent);margin-bottom:6px">Libro CXP (oficial)</div>
                    <div style="font-size:11px;line-height:1.45">Cargos <b>${fmt(p.cxpCargo)}</b> · Créditos <b>${fmt(p.cxpCredito)}</b> · Saldo <b style="color:var(--yellow)">${fmt(p.saldo)}</b> <span style="opacity:.85;font-size:10px">(c − a, neto ≥ 0; coincide con el saldo grande de la tarjeta)</span></div>
                  </div>`
                : `<div style="margin-bottom:10px;padding-bottom:10px;border-bottom:1px solid rgba(255,255,255,.08)">
                    <div style="font-size:9px;font-weight:700;text-transform:uppercase;color:var(--orange);margin-bottom:4px">Sin libro CXP</div>
                    <div style="font-size:10px;line-height:1.4">El importe principal es la capa operativa / estimada (no hay filas en libro CXP para este proveedor).</div>
                  </div>`;
              const notaClasAdv =
                p.compromisoReconocido > 0
                  ? `<span style="color:var(--border)">|</span><span><b>${fmt(p.compromisoReconocido)}</b> nota clás.</span>`
                  : '';
              const regCredAdv =
                Math.abs(p.abonosRegistroNegativo || 0) > 0.5
                  ? `<span style="color:var(--border)">|</span><span title="Abonos negativos en tes_abonos_prov (no son pago en caja)"><b style="color:var(--orange)">${fmt(Math.abs(p.abonosRegistroNegativo || 0))}</b> reg. créd.</span>`
                  : '';
              const pendEstDetailWhenCxp = p.usaCxp
                ? `<span style="color:var(--border)">|</span><span title="max(0, ref. − pagos caja &gt;0 − devoluciones)"><b style="color:var(--yellow)">${fmt(p.saldoOperativoEstimado)}</b> pend. est.</span>`
                : '';
              const sugeridoTitle = `min(vendido últ. abono, saldo). Tope útil a costo: ${fmt(p.topePagoRespaldadoVentas || 0)}.${
                (p.pendienteSinRespaldoVentaCosto || 0) > 0.5
                  ? ` Pend. est. por encima del vendido a costo: ${fmt(p.pendienteSinRespaldoVentaCosto)}.`
                  : ''
              }`;
              const saldoTitleEsc = String(saldoHeaderTitle).replace(/"/g, '&quot;');
              const sugeridoTitleEsc = String(sugeridoTitle).replace(/"/g, '&quot;');
              const legacyPrefix = [regCredAdv, notaClasAdv].filter(Boolean).join('');
              const ventasLines = ventasInformativasLista(state, p.id);
              const vendidoTraza = `<div style="font-size:10px;color:var(--text2);margin-top:4px;line-height:1.45;max-width:720px">Vendido a costo — <b>moves</b> ${p.udsVendidasPosMoves || 0} u · ${fmt(p.costoVendidoPosMoves || 0)} · <b>facturas POS (fallback)</b> ${p.udsVendidasPosFacturaSinMove || 0} u · ${fmt(p.costoVendidoPosFacturaSinMove || 0)} <span style="opacity:.85">(cuando el neto de <code>stock_moves_ventas</code> por factura+producto ≈ 0, se usa cantidad de la factura POS; si aquí hay uds/$ y moves = 0, el dato viene del fallback de facturas, validado en base)</span></div><div style="font-size:9px;opacity:.82;margin-top:6px;line-height:1.35;max-width:720px"><b>Fase 3 (pendiente):</b> en muchos casos <code>invoices.items</code> en jsonb trae arrays serializados como texto; el cliente reconstruye fallback aunque un SQL simple dé 0. Fase 1 no migra ni cambia escritura — solo contexto para validar UI vs base.</div>`;
              const ventasHtml =
                ventasLines.length === 0
                  ? `<div style="font-size:11px;color:var(--text2)">Sin líneas <b>venta_pos</b> en <code>state.stock_moves_ventas</code> para estos artículos. El <b>vendido total</b> puede incluir igualmente <b>facturas POS</b> vía fallback (ver desglose arriba). Usa «POS moves» / recarga si falta trazabilidad.</div>`
              ...
              const artsRows = (p.articulos || [])
                .map((a) => {
                  ...
                  const stNum = parseFloat(a.stock) || 0;
                  const udsRef = udsRefDeudaArticulo(state, a, p.id);
                  const linea = lineaRefDeudaArticulo(state, a, p.id);
                  const pcRow = parseFloat(a.precioCompra) || 0;
                  const adjMon = deltaU * pcRow;
                  const deltaHint =
                    deltaU !== 0
                      ? `<div style="font-size:9px;font-weight:400;color:var(--text2);line-height:1.35">Solo ajuste virtual (ΣΔ módulo): ${deltaU > 0 ? '+' : ''}${deltaU} uds → <span style="color:var(--red);font-weight:700">${adjMon >= 0 ? '+' : ''}${fmt(adjMon)}</span> <span style="opacity:.85">(el total $ arriba = uds ref. × costo)</span></div>`
                      : '';
                  const udsRefTitle = `Unidades referencia deuda = stock (${stNum}) + vendido POS (${uv}) + ajuste mód. (${deltaU > 0 ? '+' : ''}${deltaU}). Ref. $ = ${udsRef} × costo.`;
                  ...
                  return `<tr>
                    ...
                    <td class="tes-pp-num" style="font-size:15px;font-weight:800" title="Stock físico en inventario. No incluye ajuste virtual del módulo pagos.">${a.stock ?? 0}</td>
                    <td class="tes-pp-num" style="font-size:12px" title="Unidades vendidas POS y facturas sin move en estado; sin ajuste virtual del módulo.">${uv}</td>
                    <td class="tes-pp-num" style="font-size:14px;font-weight:800" title="${udsRefTitle.replace(/"/g, '&quot;')}">${udsRef}</td>
                    <td class="tes-pp-num" style="font-size:15px;font-weight:800;color:var(--accent)" title="Costo unitario actual del artículo (multiplicador de uds ref.).">${fmt(a.precioCompra || 0)}</td>
                    <td class="tes-pp-num" style="color:var(--red);font-weight:700" title="Deuda referencia a costo = uds ref. × costo unitario. Coincide con la suma por proveedor en ref. operativa. Debajo: solo el aporte del ajuste virtual (Δ×costo).">${fmt(linea)}${deltaHint}</td>
              ...
              <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:10px;margin-bottom:8px">
                <div style="min-width:0">
                  <div style="font-family:Syne;font-size:15px;font-weight:800;line-height:1.2">${p.nombre}</div>
                  <div style="font-size:10px;color:var(--text2);margin-top:2px">${[p.cedula, p.ciudad].filter(Boolean).join(' · ') || '—'}</div>
                </div>
                <div style="text-align:right;flex-shrink:0" title="${saldoTitleEsc}">
                  <div style="font-family:Syne;font-size:24px;font-weight:800;line-height:1;color:${p.saldo > 0 ? 'var(--yellow)' : 'var(--green)'}">${fmt(p.saldo)}</div>
                  <div style="font-size:9px;color:var(--text2)">${p.usaCxp ? 'saldo CXP' : 'saldo operativo'}</div>
                </div>
              </div>
              <div style="display:flex;flex-wrap:wrap;gap:8px 12px;align-items:baseline;margin-bottom:10px;font-size:13px;line-height:1.45">
                <span title="Ventas a costo posteriores al último abono &gt;0. Sin abonos: histórico completo. Último abono: ${fechaUltAbEsc}."><b style="color:${colorVendDesdeAb}">${fmt(vendDesdeAbNum)}</b> <span style="color:var(--text2)">vend. últ. abono</span> <span style="font-size:10px;color:var(--text2)">(${p.unidadesVendidasDesdeUltimoAbono || 0} u)</span></span>
                <span style="color:var(--border)">|</span>
                <span title="${sugeridoTitleEsc}"><b style="color:var(--green)">${fmt(sugeridoPago)}</b> <span style="color:var(--text2)">sugerido</span></span>
                <span style="color:var(--border)">|</span>
                <span title="Solo abonos con valor &gt; 0 en caja"><b style="color:var(--green)">${fmt(p.abonosPagados || 0)}</b> <span style="color:var(--text2)">pagos</span></span>
                ${pendEstEnPrincipal}
              </div>
              <div style="display:flex;gap:4px;flex-wrap:wrap;margin-bottom:10px">
                <button class="btn btn-sm btn-primary" onclick="openCargoCxpModal('${p.id}','${String(p.nombre).replace(/'/g, "\\'")}')">Cargo</button>
                <button class="btn btn-sm btn-secondary" onclick="openNotaCreditoCxpModal('${p.id}','${String(p.nombre).replace(/'/g, "\\'")}')">N/C</button>
                <button class="btn btn-sm btn-secondary" onclick="alinearCxpEstimacionProv('${p.id}')">Alinear</button>
                <button class="btn btn-sm btn-secondary" onclick="verLibroCxpModal('${p.id}','${String(p.nombre).replace(/'/g, "\\'")}')">Libro</button>
                <button class="btn btn-sm btn-secondary" onclick="openCompromisoProvModal('${p.id}','${String(p.nombre).replace(/'/g, "\\'")}')">Nota</button>
                <button class="btn btn-sm btn-secondary" onclick="openAbonoProvModal('${p.id}','${String(p.nombre).replace(/'/g, "\\'")}')">Abono</button>
                <button class="btn btn-sm btn-secondary" onclick="verLibroProveedorModal('${p.id}','${String(p.nombre).replace(/'/g, "\\'")}')">Hist.</button>
                <button class="btn btn-sm btn-secondary" onclick="verCompromisosProv('${p.id}','${String(p.nombre).replace(/'/g, "\\'")}')">Comp.</button>
                <button class="btn btn-sm btn-secondary" onclick="verAbonosProv('${p.id}','${String(p.nombre).replace(/'/g, "\\'")}')">Lista</button>
              </div>
              <details style="margin-bottom:8px"><summary style="font-size:12px;cursor:pointer;font-weight:700">Artículos</summary>
              <div class="table-wrap tes-pp-table-wrap" style="margin-top:6px"><table class="tes-pp-table"><thead><tr><th class="tes-pp-ref">Ref</th><th>Artículo</th><th class="tes-pp-num" title="Stock físico; no incluye ajuste virtual del módulo">Stock</th><th class="tes-pp-num" title="Vendido POS + facturas sin move en estado; sin ajuste módulo">Vend.</th><th class="tes-pp-num" title="Stock + vendido POS + ajuste virtual módulo. Base de la columna Ref. $">Uds ref.</th><th class="tes-pp-num" title="Precio compra actual">Costo u.</th><th class="tes-pp-num" title="Uds ref. × costo u. (deuda referencia a costo por línea)">Ref. $</th><th class="tes-pp-actions"></th></tr></thead><tbody>${artsRows || '<tr><td colspan="8" style="text-align:center;color:var(--text2)">—</td></tr>'}</tbody></table></div>
              </details>
              <details style="margin-bottom:8px"><summary style="font-size:11px;color:var(--text2);cursor:pointer;font-weight:600">Operativo, libro y reconciliación</summary>
              <div style="margin-top:8px;padding:10px 12px;border-radius:8px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);font-size:11px;line-height:1.5;color:var(--text2)">
                ${cxpLibroBloque}
                <div style="font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.04em;color:var(--text2);margin-bottom:6px">Operativo reconstruido</div>
                <div style="display:flex;flex-wrap:wrap;gap:6px 12px;margin-bottom:8px;align-items:baseline">
                  <span title="Inventario neto a costo + vendido POS + ajustes unidades (base ref. operativa)"><b>${fmt(p.refOperativaTotal)}</b> ref.</span>
                  ${pendEstDetailWhenCxp}
                  <span style="color:var(--border)">|</span>
                  <span title="Stock mercancía crédito × costo"><b style="color:var(--accent)">${fmt(p.valorInventarioCosto)}</b> inv. a costo</span>
                  <span style="color:var(--border)">|</span>
                  <span><b>${fmt(p.costoVendidoHist)}</b> vendido total <span style="opacity:.9">(${p.unidadesVendidasHist || 0} u)</span></span>
                  <span style="color:var(--border)">|</span>
                  <span title="Devoluciones operativas"><b style="color:var(--accent)">${fmt(p.devolucionesOperativa || 0)}</b> devol</span>
                </div>
                <div style="font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.04em;color:var(--text2);margin:10px 0 6px">Legacy / nota en libro (capa distinta a CXP operativo)</div>
                <div style="display:flex;flex-wrap:wrap;gap:6px 8px;align-items:center;font-size:10px;line-height:1.45;margin-bottom:8px">
                  ${legacyPrefix}${legacyPrefix ? '<span style="color:var(--border)">|</span>' : ''}
                  <span title="Compromiso reconocido − abonos (nota clásica; capa distinta al saldo CXP)"><b>${fmt(p.saldoLibro)}</b> saldoLibro</span>
                </div>
                ${recoLine}
                <div style="font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.04em;color:var(--text2);margin:10px 0 6px">Vendido POS: moves vs facturas (detalle)</div>
                ${vendidoTraza}
                <div style="margin-top:10px;padding-top:10px;border-top:1px solid rgba(255,255,255,.08)">
                  <div style="font-size:9px;font-weight:700;text-transform:uppercase;color:var(--green);margin-bottom:4px">Tope útil (a costo) — solo en detalle</div>
                  <div style="font-family:Syne;font-size:16px;font-weight:800;color:var(--green)" title="min(pend. est., vendido a costo). En la franja principal el sugerido es min(vend. últ. abono, saldo).">${fmt(p.topePagoRespaldadoVentas || 0)}</div>
                  ${
                    (p.pendienteSinRespaldoVentaCosto || 0) > 0.5
                      ? `<div style="font-size:10px;color:var(--orange);margin-top:4px;font-weight:600">Pend. est. por encima del vendido a costo: ${fmt(p.pendienteSinRespaldoVentaCosto)}</div>`
                      : ''
                  }
                </div>
                <div style="display:flex;align-items:center;gap:8px;margin-top:10px"><div style="flex:1;background:rgba(255,255,255,.05);border-radius:6px;height:6px;overflow:hidden"><div style="height:100%;border-radius:6px;background:linear-gradient(90deg,var(--green),var(--accent));width:${pct}%;transition:width 1s ease"></div></div><span style="font-size:10px;color:var(--text2);white-space:nowrap">${pct.toFixed(0)}% cubierto (ref.)</span></div>
                <div style="font-size:10px;margin-top:8px">Último abono (ref.): <b style="color:var(--text1)">${fechaUltAbEsc}</b></div>
              </div>
              </details>
              <details style="margin-bottom:4px"><summary style="font-size:11px;color:var(--accent);cursor:pointer;font-weight:600">Ventas POS (vitrina + despachos)</summary><div style="margin-top:4px;font-size:10px;color:var(--text2);line-height:1.35;margin-bottom:6px">Todas las salidas registradas en <b>stock_moves</b> tipo venta_pos; el canal solo afecta la etiqueta de cada línea.</div><div>${ventasHtml}</div></details>
              </div>`;
            })
            .join('')
    }
    ...
    (state.usu_proveedores || []).forEach((p) => {
      ...
      if (d.saldo > 0)
        opts.push({
          ...
          nombre: p.nombre,
          saldo: d.saldo,
          vendUltAb: Math.max(0, parseFloat(d.vendidoDesdeUltimoAbono) || 0),
        });
    });
    return opts;
  }
  ...
  function openAbonoProvModal(ctx) {
    ...
    const optHtml = opts
      .map((o) => {
        const sel = o.id === provId ? 'selected' : '';
        const nm = String(o.nombre).replace(/"/g, '&quot;');
        return `<option value="${o.id}" data-saldo="${o.saldo}" data-vend-ult-ab="${o.vendUltAb ?? 0}" data-nombre="${nm}" ${sel}>${o.nombre} · Saldo: ${fmt(o.saldo)}</option>`;
      })
      .join('');
    openModal(`
    ...
    <div id="ab-vendido-sug-wrap" style="display:none;background:rgba(0,229,180,.07);border:1px solid rgba(0,229,180,.22);border-radius:8px;padding:10px;margin-bottom:12px;font-size:12px;line-height:1.45">
      <div><span style="color:var(--text2)">Vendido a costo desde el último abono positivo:</span> <span id="ab-vendido-ult-val" style="font-weight:800;color:var(--accent)"></span></div>
      <div style="font-size:10px;color:var(--text2);margin-top:4px">Monto sugerido (no supera el saldo): <b id="ab-sugerido-pago-val" style="color:var(--text1)"></b> = mín(vendido, saldo)</div>
      <button type="button" class="btn btn-sm btn-secondary" style="margin-top:8px" onclick="abonoUsarMontoVendidoSugerido()">Usar monto vendido como abono</button>
    </div>
    ...
    <div id="ab-warning" style="display:none;color:var(--red);font-size:11px;margin-bottom:8px;font-weight:600"></div>
    <div id="ab-warning-vendido" style="display:none;color:var(--yellow);font-size:11px;margin-bottom:8px;border-left:3px solid rgba(251,191,36,.75);padding-left:10px;line-height:1.45"></div>
    <button type="button" class="btn btn-primary" id="ab-btn-guardar" style="width:100%" onclick="guardarAbonoProv()" ${global.AppSyncGuard && global.AppSyncGuard.isBusy() ? 'disabled' : ''} title="${global.AppSyncGuard && global.AppSyncGuard.isBusy() ? global.AppSyncGuard.waitMessage : ''}">💳 Guardar Abono</button>`);
    if (provId) {
      setTimeout(() => {
  ...
  function updateSaldoPendiente(ctx) {
    ...
    const vendUlt = parseFloat(opt?.getAttribute('data-vend-ult-ab') || 0);
    const info = document.getElementById('ab-saldo-info');
    const val = document.getElementById('ab-saldo-val');
    const vendWrap = document.getElementById('ab-vendido-sug-wrap');
    const vendValEl = document.getElementById('ab-vendido-ult-val');
    const sugValEl = document.getElementById('ab-sugerido-pago-val');
    ...
    const provId = sel?.value;
    if (vendWrap && vendValEl && sugValEl) {
      if (provId && (saldo > 0 || vendUlt > 0)) {
        vendWrap.style.display = 'block';
        vendValEl.textContent = fmt(vendUlt);
        sugValEl.textContent = fmt(Math.min(vendUlt, saldo));
      } else {
        vendWrap.style.display = 'none';
      }
    }
  ...
  function validateAbono(ctx) {
    ...
    const opt = sel?.options[sel.selectedIndex];
    const saldo = parseFloat(opt?.getAttribute('data-saldo') || 0);
    const vendUltOpt = parseFloat(opt?.getAttribute('data-vend-ult-ab') || 0);
    const provId = sel?.value;
    const valor = parseFloat(document.getElementById('ab-valor')?.value || 0);
    const warn = document.getElementById('ab-warning');
    const warnVend = document.getElementById('ab-warning-vendido');
    const fifoValid = document.getElementById('ab-fifo-validate');
    const excedeSaldo = valor > saldo + ABONO_PROV_SALDO_TOLERANCIA;
    if (warn) {
      if (excedeSaldo) {
        warn.style.display = 'block';
        warn.textContent = `El abono (${fmt(valor)}) supera el saldo pendiente (${fmt(saldo)}). Ajusta el monto o revisa CXP / deuda.`;
      } else {
        warn.style.display = 'none';
        warn.textContent = '';
      }
    }
    let vendRef = vendUltOpt;
    if (state && provId && provId !== '') {
      const d = calcDeudaProveedor(state, provId);
      vendRef = Math.max(0, parseFloat(d.vendidoDesdeUltimoAbono) || 0);
    }
    if (warnVend) {
      if (!excedeSaldo && valor > vendRef + 1e-6 && valor > 1e-9) {
        const diff = valor - vendRef;
        warnVend.style.display = 'block';
        warnVend.innerHTML = `Parte del abono (<b>${fmt(diff)}</b>) supera lo vendido a costo desde el último abono (${fmt(vendRef)}). Estarías usando <b>recursos propios</b> para cubrir esa diferencia (p. ej. mercancía aún en inventario o saldo por encima de lo ya cobrado a clientes a ese costo).`;
      } else {
        warnVend.style.display = 'none';
        warnVend.textContent = '';
      }
    }
    if (fifoValid) {
      fifoValid.style.display = 'none';
  ...
  function abonoUsarMontoVendidoSugerido(ctx) {
    ...
    const saldo = parseFloat(opt?.getAttribute('data-saldo') || 0);
    const provId = sel?.value;
    let vend = parseFloat(opt?.getAttribute('data-vend-ult-ab') || 0);
    if (state && provId) {
      const d = calcDeudaProveedor(state, provId);
      vend = Math.max(0, parseFloat(d.vendidoDesdeUltimoAbono) || 0);
    }
    const sugerido = Math.max(0, Math.min(vend, saldo));
    const inp = document.getElementById('ab-valor');
    if (inp) inp.value = sugerido > 0 ? String(sugerido) : '';
    validateAbono({ fmt, state });
  }
  ...
  async function guardarAbonoProv(ctx) {
    ...
    _guardarAbonoProvInFlight = true;
    const abSaveBtn = document.getElementById('ab-btn-guardar');
    const abSaveLabel = '💳 Guardar Abono';
    const releaseAbonoProvSubmit = () => {
      _guardarAbonoProvInFlight = false;
      if (abSaveBtn) {
        abSaveBtn.disabled = false;
        abSaveBtn.textContent = abSaveLabel;
      }
    };
    if (abSaveBtn) {
      abSaveBtn.disabled = true;
      abSaveBtn.textContent = '⏳ Guardando...';
    }
    const nextId = typeof dbId === 'function' ? dbId : uid;
    const sel = document.getElementById('ab-prov-sel');
    const opt = sel?.options[sel.selectedIndex];
    ...
    const deuda = calcDeudaProveedor(state, provId);
    const saldoMax = parseFloat(deuda.saldo) || 0;
    const fuenteSaldo = deuda.usaCxp ? 'CXP' : 'estimación';
    if (valor > saldoMax + ABONO_PROV_SALDO_TOLERANCIA) {
      notify(
        'danger',
        '⚠️',
        'Abono no permitido',
        `El monto ${fmt(valor)} supera el saldo pendiente ${fmt(saldoMax)} (${fuenteSaldo}). Máximo con tolerancia ${fmt(ABONO_PROV_SALDO_TOLERANCIA)}.`,
        { duration: 7000 },
      );
      releaseAbonoProvSubmit();
      return;
    ...
    function aplicarCajaRespuestaRpc(cajaJson) {
      ...
      mergeCajaFromRpcJson(state, cajaJson);
    }
    ...
    try {
      ...
      try {
        if (typeof global.refreshCriticalSlice === 'function') {
          const rr = await global.refreshCriticalSlice('tes_prov');
          if (!rr.ok) {
            notify('warning', '📡', 'No se pudo sincronizar', 'El abono se guardó; reintenta para actualizar saldos en pantalla.', { duration: 5500 });
          }
        } else {
          renderTesPagosProv();
        }
  ...
  function verAbonosProv(ctx) {
    ...
    <div><span style="color:var(--text2)">Saldo por pagar:</span> <b style="color:var(--yellow)">${fmt(d.saldo)}</b></div></div>
    <div style="font-size:11px;color:var(--text2);margin-bottom:12px;line-height:1.45">Desglose: en stock ${fmt(d.valorInventarioCosto)}${
      (d.invEntradaCredAbonoDedup || 0) > 0.5
        ? ` · reg. créd. inv. entrada ${fmt(d.invEntradaCredAbonoDedup)} (en abonos; no se resta del inventario en ref.)`
        : ''
    } · ya vendido (moves + fact. sin move en estado) ${fmt(d.costoVendidoHist)} (${d.unidadesVendidasHist} u): <b>moves</b> ${d.udsVendidasPosMoves || 0} u / ${fmt(d.costoVendidoPosMoves || 0)} · <b>fact.</b> ${d.udsVendidasPosFacturaSinMove || 0} u / ${fmt(d.costoVendidoPosFacturaSinMove || 0)}${
      Math.abs(d.ajusteUnidadesCosto || 0) > 1e-9
        ? ` · ajuste unidades (mód. pagos) ${fmt(d.ajusteUnidadesCosto)} (${(d.unidadesAjusteModulo || 0) > 0 ? '+' : ''}${d.unidadesAjusteModulo || 0} uds)`
        : ''
    } · inv. ajustes entrada (hist.) ${fmt(d.ajustesEntradaCosto || 0)} (${d.ajustesEntradaUds || 0} uds) · salida (hist.) ${fmt(d.ajustesSalidaCosto || 0)} (${d.ajustesSalidaUds || 0} uds)${
      Math.abs(d.ajustesInvNeto || 0) > 0.5
        ? ` · <span title="entrada − salida; informativo — suele estar ya en el stock de arriba">neto inv. ajustes ${fmt(d.ajustesInvNeto)}</span>`
        : ''
    }</div>
  ...
  function openAjusteUnidadesProvModal(ctx) {
    ...
    const stList = parseFloat(art.stock) || 0;
    const vendList = articuloUnidadesVendidas(state, artId);
    const udsRefList = udsRefDeudaArticulo(state, art, provId);
    const refMontoList = lineaRefDeudaArticulo(state, art, provId);
    ...
    const histRows =
      historial.length === 0
        ? '<tr><td colspan="5" style="text-align:center;color:var(--text2);padding:12px;font-size:11px">Sin movimientos aún</td></tr>'
        : historial
            .map((r) => {
              const auditRaw = [r.proveedorNombre, r.articuloNombre].filter((x) => x && String(x).trim()).join(' · ');
              const auditCell = String(auditRaw || '—').replace(/</g, '&lt;');
              return `<tr><td style="font-size:10px;white-space:nowrap">${fmtLineaHora(r.fechaHora)}</td><td style="font-weight:700">${(parseFloat(r.deltaUnidades) || 0) > 0 ? '+' : ''}${parseFloat(r.deltaUnidades) || 0}</td><td style="font-size:10px;color:var(--text2);max-width:140px">${auditCell}</td><td style="font-size:10px;color:var(--text2)">${String(r.nota || '—').replace(/</g, '&lt;')}</td><td><button type="button" class="btn btn-xs btn-danger" onclick="eliminarAjusteUnidadesProv('${r.id}','${provId}','${artId}')">✕</button></td></tr>`;
            })
            .join('');
    ...
    <div style="font-size:12px;margin-bottom:10px"><b>${tituloArt}</b><div style="font-size:10px;color:var(--text2);margin-top:4px">${provNombre} · acumulado ajuste virtual: <b>${acum > 0 ? '+' : ''}${acum}</b> uds</div><div style="font-size:11px;color:var(--accent);margin-top:6px;line-height:1.4">Impacto solo del ajuste (× costo actual): <b>${acum === 0 ? '—' : fmt(acum * pc)}</b>${acum !== 0 ? ` <span style="color:var(--text2)">(${acum > 0 ? '+' : ''}${acum} uds × ${fmt(pc)})</span>` : ''}</div><div style="font-size:10px;color:var(--text2);margin-top:8px;line-height:1.45;padding:8px;background:rgba(255,255,255,.05);border-radius:6px;border:1px solid rgba(255,255,255,.08)">Misma lógica que la tabla <b>Pagos proveedores</b>: <b>uds ref.</b> = stock (${stList}) + vendido POS (${vendList}) + ajuste acum. (${acum > 0 ? '+' : ''}${acum}) = <b>${udsRefList}</b> uds → <b>Ref. $</b> = ${fmt(refMontoList)} (= ${udsRefList} × ${fmt(pc)})</div></div>
    ...
    <div class="table-wrap"><table><thead><tr><th>Fecha</th><th>Δ uds</th><th>Prov. / art. (snapshot)</th><th>Nota</th><th></th></tr></thead><tbody>${histRows}</tbody></table></div>`);
    setTimeout(() => {
  ...
  async function guardarAjusteUnidadesProv(ctx) {
    ...
    const row = {
      ...
      proveedorNombre: String(prov.nombre || '').trim() || null,
      articuloNombre: String(art.nombre || art.codigo || artId || '').trim() || null
    };
    try {
      showLoadingOverlay('connecting');
        ...
        const { error } = await supabaseClient.from('tes_ajustes_unidades_prov').upsert(
          {
            ...
            articulo_nombre: row.articuloNombre
          },
          { onConflict: 'id' }
        );
        if (error) throw error;
  ...
  function mergeCajaFromRpcJson(state, cajaJson) {
    if (!cajaJson || typeof cajaJson !== 'object') return;
    const cid = cajaJson.id;
    if (!cid) return;
    const c = (state.cajas || []).find((x) => String(x.id) === String(cid));
    if (!c) return;
  ...
  function rpcRevertMissingError(err) {
    const c = err?.code;
    const m = String(err?.message || err?.error_description || '');
    return c === '42883' || c === 'PGRST202' || /does not exist|function.*not found|no existe la funci/i.test(m);
  }
  ...
  async function eliminarAbonoProv(ctx) {
    ...
    if (
      !confirm(
        '¿Eliminar este abono? Se recalculará el saldo al proveedor y, si hubo egreso en caja, se devolverá el monto al bucket correspondiente.',
      )
    )
      return;
    ...
    const syncStateAfterRevert = (rpcData) => {
      state.tes_abonos_prov = (state.tes_abonos_prov || []).filter((x) => String(x.id) !== String(id));
      state.tes_cxp_movimientos = (state.tes_cxp_movimientos || []).filter((r) => !cxpIds.includes(r.id));
      state.tes_movimientos = (state.tes_movimientos || []).filter((m) => {
        if (m.categoria !== 'abono_proveedor' || m.tipo !== 'egreso') return true;
        if (String(m.refAbonoProvId || '') === String(id)) return false;
        if (String(m.concepto || '').includes(`ref:${id}`)) return false;
        return true;
      });
      if (rpcData && rpcData.caja) mergeCajaFromRpcJson(state, rpcData.caja);
    };

    try {
      const { data: rpcData, error: rpcErr } = await supabaseClient.rpc('tes_abono_proveedor_revertir', {
        p_abono_id: String(id),
      });
      if (!rpcErr) {
        syncStateAfterRevert(rpcData);
        renderTesPagosProv();
        if (typeof renderTesCajas === 'function') renderTesCajas();
        notify('success', '🗑️', 'Abono eliminado', 'Saldo y caja actualizados.', { duration: 2500 });
        return;
      }
      if (!rpcRevertMissingError(rpcErr)) throw rpcErr;
      ...
      renderTesPagosProv();
      if (typeof renderTesCajas === 'function') renderTesCajas();
      notify('success', '🗑️', 'Abono eliminado', 'Saldo recalculado (sin RPC de reversión en servidor).', { duration: 3000 });
    } catch (e) {
  ...
  function verLibroProveedorModal(ctx) {
    ...
    (state.tes_ajustes_unidades_prov || [])
      ...
      .forEach((r) => {
        const artA = (state.articulos || []).find((a) => String(a.id) === String(r.articuloId));
        const nm = (r.articuloNombre && String(r.articuloNombre).trim()) || (artA ? artA.nombre || artA.codigo || r.articuloId : r.articuloId);
  ...
  async function logRegistroDeudaArticulo(ctx) {
    ...
    if (!esMercCreditoTitulo(artLocal.tituloMercancia)) {
      log('debt_log_skip:not_credit', { articuloId: artLocal.id, tituloMercancia: artLocal.tituloMercancia, reason: reason || '' }, 'B_debt_log_decision');
      return;
    }
    if (!artLocal.proveedorId) {
      log('debt_log_skip:no_proveedor', { articuloId: artLocal.id, reason: reason || '' }, 'B_debt_log_decision');
      return;
    }
    ...
    if (!(pc > 0) || !(st > 0)) {
      log('debt_log_skip:missing_cost_or_stock', { articuloId: artLocal.id, pc, st, reason: reason || '' }, 'B_debt_log_decision');
      return;
    }
    const valor = pc * st;
    ...
    const row = {
      // Idempotencia: 1 registro_deuda por artículo (evita duplicados por reintentos/doble-click).
      id: String(artLocal.id || uid),
    ...
    try {
      const { data: existing, error: exErr } = await supabaseClient
        .from('tes_libro_proveedor')
        .select('id')
        .eq('tipo', 'registro_deuda')
        .eq('articulo_id', artLocal.id)
        .limit(1);
      if (exErr) {
        log('debt_log_warn:precheck_failed', { articuloId: artLocal.id, err: exErr.message, reason: reason || '' }, 'C_idempotency_precheck');
      } else if (Array.isArray(existing) && existing.length > 0) {
        log('debt_log_skip:already_exists', { articuloId: artLocal.id, existingId: existing[0]?.id, reason: reason || '' }, 'C_idempotency_precheck');
        return;
      }
    } catch (e) {
      log('debt_log_warn:precheck_exception', { articuloId: artLocal.id, err: String(e?.message || e), reason: reason || '' }, 'C_idempotency_precheck');
    }
    ...
    if (error) {
      log('debt_log_fail:insert', { articuloId: artLocal.id, err: error.message, rowId: row.id, reason: reason || '' }, 'D_insert_result');
      console.warn('[libro proveedor]', error.message);
      return;
    }
    log('debt_log_ok:inserted', { articuloId: artLocal.id, rowId: row.id, valor, reason: reason || '' }, 'D_insert_result');
    if (!state.tes_libro_proveedor) state.tes_libro_proveedor = [];
  ...
  function renderTesDinero(ctx) {
    ...
    const vt =
      typeof global.ventasTablaCOPForYmdRange === 'function'
        ? global.ventasTablaCOPForYmdRange(desde, hasta)
        : null;
    const copVentasTabla = vt ? vt.cop : null;
    const nFacturasTabla = vt ? vt.facturas : null;
    const deltaCajaVsVentas =
      copVentasTabla != null ? sumVentaPosIng - copVentasTabla : null;
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:10px;margin-bottom:12px">
      <div class="card" style="padding:10px 12px;margin:0"><div style="font-size:10px;color:var(--text2)">Efectivo</div><div style="font-weight:800;color:${sumEfectivo >= 0 ? 'var(--green)' : 'var(--red)'}">${fmt(sumEfectivo)}</div><div style="font-size:9px;color:var(--text2);margin-top:2px;opacity:.85">Neto período (todas las clases)</div></div>
      <div class="card" style="padding:10px 12px;margin:0"><div style="font-size:10px;color:var(--text2)">Transferencia</div><div style="font-weight:800;color:${sumTransferencia >= 0 ? 'var(--green)' : 'var(--red)'}">${fmt(sumTransferencia)}</div><div style="font-size:9px;color:var(--text2);margin-top:2px;opacity:.85">Neto período (todas las clases)</div></div>
      ...
      <div class="card" style="padding:10px 12px;margin:0;border:1px solid rgba(167,139,250,.35)"><div style="font-size:10px;color:var(--text2)">Solo venta_pos (ingresos)</div><div style="font-weight:800;color:var(--accent)">${fmt(sumVentaPosIng)}</div><div style="font-size:9px;color:var(--text2);margin-top:2px;opacity:.85">${lineasVentaPos} líneas → va al lado de facturas</div></div>
      <div class="card" style="padding:10px 12px;margin:0"><div style="font-size:10px;color:var(--text2)">Tabla ventas (mismo rango)</div><div style="font-weight:800;color:#a78bfa">${copVentasTabla != null ? fmt(copVentasTabla) : '—'}</div><div style="font-size:9px;color:var(--text2);margin-top:2px;opacity:.85">${copVentasTabla != null ? `${nFacturasTabla} facturas activas` : 'Recarga tras core.js'}</div></div>
    </div>
    <div class="card" style="padding:12px 14px;margin-bottom:14px;font-size:11px;line-height:1.5;color:var(--text2)">
      <div style="font-weight:700;color:var(--text);margin-bottom:6px">📎 Conciliación con el dashboard (meta)</div>
      <div>La <b>meta</b> y el total por canal usan la tabla <code style="font-size:10px">ventas</code> (${copVentasTabla != null ? fmt(copVentasTabla) + ' en este filtro' : '…'}).</div>
      <div style="margin-top:4px">Los cobros POS en caja son <code style="font-size:10px">tes_movimientos</code> con clase <b>venta_pos</b>: ${fmt(sumVentaPosIng)} (${lineasVentaPos} líneas). Una venta mixta suele ser <i>2 líneas</i> por un solo total en facturas.</div>
      ${deltaCajaVsVentas != null ? `<div style="margin-top:8px;padding-top:8px;border-top:1px solid var(--border);color:${Math.abs(deltaCajaVsVentas) < 1 ? 'var(--green)' : 'var(--yellow)'}"><b>Δ (caja venta_pos − suma ventas):</b> ${fmt(deltaCajaVsVentas)}</div>` : ''}
      <div style="margin-top:6px;font-size:10px;opacity:.9">Si comparás con el número «769k» del dashboard: revisá que sea el <b>mismo mes</b> y que la sesión tenga todos los datos cargados (no compares un día de 2024 con el acumulado de otro mes).</div>
    </div>
  ...
  /**
   * Movimientos del libro CXP que no deben borrarse aislados (tes_abonos_prov, compromisos, devoluciones, caja).
   */
  function cxpMovimientoDerivadoInfo(state, r) {
    const id = String(r?.id || '');
    const meta = r?.meta && typeof r.meta === 'object' ? r.meta : {};
    const orig = String(meta.origen || '');
    const tipo = String(r?.tipo || '');

    if (id.startsWith('cxp-abono-')) {
      const aid = id.slice('cxp-abono-'.length);
      return {
        bloqueado: true,
        motivo:
          'Esta fila es el reflejo en CXP de un abono a proveedor. No se puede borrar solo aquí: use «Anular abono» para revertir tes_abonos_prov y el movimiento de caja.',
        accion: 'abono',
        idAccion: aid
      };
    }
    if (id.startsWith('cxp-leg-ab-')) {
      const aid = id.slice('cxp-leg-ab-'.length);
      return {
        bloqueado: true,
        motivo:
          'Abono CXP (formato legado). Anúlelo con «Anular abono» para mantener alineados abonos, CXP y caja.',
        accion: 'abono',
        idAccion: aid
      };
    }
    if (tipo === 'abono' && orig === 'abono_modal' && meta.abonoId != null && String(meta.abonoId).trim() !== '') {
      return {
        bloqueado: true,
        motivo:
          'Abono vinculado a tes_abonos_prov. Use «Anular abono» para la reversión integral (incluye caja).',
        accion: 'abono',
        idAccion: String(meta.abonoId).trim()
      };
    }

    if (id.startsWith('cxp-comp-')) {
      const cid = id.slice('cxp-comp-'.length);
      return {
        bloqueado: true,
        motivo:
          'Cargo generado desde una nota de compromiso o importación. Use «Quitar nota» para eliminar compromiso y su reflejo en CXP.',
        accion: 'compromiso',
        idAccion: cid
      };
    }
    if (id.startsWith('cxp-leg-cp-')) {
      const cid = id.slice('cxp-leg-cp-'.length);
      return {
        bloqueado: true,
        motivo: 'Cargo CXP legado vinculado a compromiso. Use «Quitar nota» con el id de la nota correspondiente.',
        accion: 'compromiso',
        idAccion: cid
      };
    }
    if ((orig === 'compromiso_modal' || orig === 'import_estimacion') && meta.compromisoId != null && String(meta.compromisoId).trim() !== '') {
      return {
        bloqueado: true,
        motivo:
          'Cargo vinculado a tes_compromisos_prov. Use «Quitar nota» para eliminar el compromiso y las líneas CXP asociadas.',
        accion: 'compromiso',
        idAccion: String(meta.compromisoId).trim()
      };
    }

    const devFromId = id.startsWith('cxp-devolucion-') ? id.slice('cxp-devolucion-'.length) : '';
    const devFromMeta = String(meta.devolucion_id || '').trim();
    if (devFromId || orig === 'devolucion_inventario') {
      const did = devFromId || devFromMeta;
      if (!did) {
        return {
          bloqueado: true,
          motivo:
            'Espejo de devolución sin id resoluble. Revise tes_devoluciones_prov o sincronice datos; no borre esta fila de forma aislada.',
          accion: 'devolucion',
          idAccion: '',
          invAjusteId: ''
        };
      }
      const dv = (state.tes_devoluciones_prov || []).find((x) => String(x.id) === String(did));
      const invAjusteId = dv && dv.invAjusteId != null ? String(dv.invAjusteId) : '';
      return {
        bloqueado: true,
        motivo: invAjusteId
          ? 'Espejo en CXP de tes_devoluciones_prov. Para alinear inventario, devolución y CXP use «Revertir ajuste» o elimine el ajuste de inventario (devolución) vinculado.'
          : 'Espejo de devolución a proveedor. No lo borre solo en CXP: elimine o revierta la devolución en el flujo de inventario para mantener tes_devoluciones_prov coherente.',
        accion: 'devolucion',
        idAccion: did,
        invAjusteId
      };
  ...
  function cxpLibroAccionesCell(state, r) {
    const info = cxpMovimientoDerivadoInfo(state, r);
    const escId = escCxp(r.id);
    if (!info.bloqueado) {
      return `<button type="button" class="btn btn-xs btn-danger" onclick="eliminarCxpMovimiento('${escId}')">✕</button>`;
    }
    let extra = '';
    if (info.accion === 'abono' && info.idAccion) {
      extra = `<button type="button" class="btn btn-xs btn-secondary" style="margin-left:4px" onclick="eliminarAbonoProv('${escCxp(info.idAccion)}')">Anular abono</button>`;
    } else if (info.accion === 'compromiso' && info.idAccion) {
      extra = `<button type="button" class="btn btn-xs btn-secondary" style="margin-left:4px" onclick="eliminarCompromisoProv('${escCxp(info.idAccion)}')">Quitar nota</button>`;
    } else if (info.accion === 'devolucion' && info.invAjusteId) {
      extra = `<button type="button" class="btn btn-xs btn-secondary" style="margin-left:4px" onclick="eliminarAjuste('${escCxp(info.invAjusteId)}')">Revertir ajuste</button>`;
    }
    return `<button type="button" class="btn btn-xs btn-danger" disabled style="opacity:0.45;cursor:not-allowed" title="${escCxp(info.motivo)}">✕</button>${extra}`;
  }
              ...
              const lineasCell =
                lns.length > 0
                  ? `<div style="font-size:10px;line-height:1.35;max-width:220px;color:var(--text2)">${lns
                      ...
                      .join('<br>')}</div>`
                  : '—';
              return `<tr><td style="font-size:11px;white-space:nowrap">${fmtLineaHora(r.fechaHora) || formatDate(r.fecha)}</td><td><span class="badge badge-warn" style="font-size:9px">${tipoLbl}</span></td><td style="font-size:11px;color:var(--text2)">${escCxp(r.referencia || '—')}</td><td style="font-size:11px;color:var(--text2)">${escCxp(r.nota || '—')}</td><td style="font-size:11px;vertical-align:top">${lineasCell}</td><td style="font-weight:700;color:${col}">${sign}${fmt(r.monto)}</td><td style="white-space:nowrap">${cxpLibroAccionesCell(state, r)}</td></tr>`;
            })
            .join('');
  ...
  async function eliminarCxpMovimiento(ctx) {
    ...
    if (deriv.bloqueado) {
      notify('warning', '⚠️', 'Movimiento vinculado', deriv.motivo, { duration: 6500 });
      return;
    }
  ...
  async function alinearCxpEstimacionProv(ctx) {
    ...
    try {
      ...
      try {
        if (typeof global.refreshCriticalSlice === 'function') {
          const rr = await global.refreshCriticalSlice('tes_prov');
          if (!rr.ok) notify('warning', '📡', 'No se pudo sincronizar', 'Reintenta para ver el libro actualizado.', { duration: 5000 });
        } else {
          renderTesPagosProv();
        }
      } catch (_) {
  ...
  /**
   * Consola: traza vendido a costo por proveedor (moves vs fallback facturas).
   * Uso: pasar el mismo `state` que usa el ERP (p. ej. desde breakpoint o si expones referencia global).
   */
  function logAuditVendidoProveedor(state, provId) {
    const d = calcDeudaProveedor(state, provId);
    console.table([
      {
        uds_moves: d.udsVendidasPosMoves,
        pesos_moves: d.costoVendidoPosMoves,
        uds_factura_sin_move: d.udsVendidasPosFacturaSinMove,
        pesos_factura_sin_move: d.costoVendidoPosFacturaSinMove,
        uds_total: d.unidadesVendidasHist,
        pesos_total: d.costoVendidoHist,
        ref_: d.refOperativaTotal,
        pend_est: d.saldoOperativoEstimado,
        saldo_cxp_oficial: d.usaCxp ? d.saldoOficialCxp : null,
        ref_menos_cxp: d.difEstimVsCxp,
        pen_menos_cxp: d.usaCxp ? d.saldoOperativoEstimado - d.saldo : null
      }
    ]);
    console.info(
      '[Pagos proveedor] Si uds_moves=0 y uds_factura_sin_move>0, el total sale del fallback de facturas POS (neto move en estado ≈ 0). Tras crear moves o reloadStockMovesVentasFromDb(), revalidar.',
    );
    return d;
  }

  /** Contrato explícito para core/alertas: KPI deuda vs pantallas tesorería. */
  const treasuryModuleSelf = {
    ...
    isTreasuryModuleValidForUi() {
      return (
        typeof this.calcDeudaProveedor === 'function' &&
        typeof this.renderTesPagosProv === 'function' &&
        typeof this.renderSimpleCollection === 'function'
      );
    }
  };
