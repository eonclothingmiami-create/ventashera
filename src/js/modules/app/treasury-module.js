...
(function initTreasuryModule(global) {
  /**
   * Deuda informativa (a costo) = stock a costo + vendido POS a costo, solo artículos a crédito con proveedor asignado.
   * Este módulo no modifica inventario; el saldo solo baja con abonos. Las ventas POS se muestran aparte como texto informativo.
   */
  ...
  function lineaRefDeudaArticulo(state, art) {
            ...
            .map((p) => {
              ...
              const artsRows = (p.articulos || [])
                .map((a) => {
                  ...
                  const linea = lineaRefDeudaArticulo(state, a);
                  return `<tr>
                    ...
                    <td style="color:var(--red);font-weight:700">${fmt(linea)}</td>
                    <td style="white-space:nowrap">
                      <button type="button" class="btn btn-xs btn-secondary" onclick="openArticuloModal('${a.id}')" title="Editar artículo">✏️</button>
              ...
              <div style="font-size:10px;color:var(--text2);margin:6px 0 8px">Desglose: en stock ${fmt(p.valorInventarioCosto)} · ya vendido (POS a costo) ${fmt(p.costoVendidoHist)} (${p.unidadesVendidasHist} uds) · ajustes salida hist. ${fmt(p.ajustesSalidaCosto || 0)} (${p.ajustesSalidaUds || 0} uds). Artículos a crédito: ${p.articulos.length}</div>
  ...
  function verLibroProveedorModal(ctx) {
    ...
    <p style="font-size:11px;color:var(--text2);line-height:1.45;margin:0 0 12px">Deuda, abonos y <b>devoluciones</b> (reducción de deuda) con fecha y hora.</p>
  ...
  async function quitarCreditoArticuloProveedor(ctx) {
    ...
    const refAntes = lineaRefDeudaArticulo(state, art);
