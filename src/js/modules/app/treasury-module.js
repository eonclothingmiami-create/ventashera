              ...
              <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:8px;margin-bottom:8px">
                ...
                <div style="border-left:3px solid rgba(251,191,36,.85);padding:8px 10px;background:rgba(251,191,36,.07);border-radius:8px">
                  ...
                  <div style="margin-top:8px;padding-top:8px;border-top:1px solid rgba(255,255,255,.08)">
                    ...
                    <div style="font-family:Syne;font-size:18px;font-weight:800;line-height:1.1;color:${
                      p.vendidoDesdeUltimoAbonoAlcanzaSaldo ? 'var(--green)' : 'var(--yellow)'
                    }" title="Ventas a costo posteriores al último pago (valor &gt; 0) registrado para este proveedor. Sin abonos positivos = mismo total que vendido histórico.">${fmt(p.vendidoDesdeUltimoAbono || 0)}</div>
                    <div style="font-size:10px;color:var(--text2);margin-top:4px;line-height:1.35">Último abono (ref.): <b style="color:var(--text1)">${p.fechaUltimoAbonoLabel ? String(p.fechaUltimoAbonoLabel).replace(/</g, '&lt;') : '— sin abonos positivos · se usa todo el histórico'}</b></div>
                    <div style="font-size:9px;color:var(--text2);margin-top:2px">Verde si ≥ saldo (${p.usaCxp ? 'CXP' : 'estim.'}); amarillo si no alcanza.</div>
                  </div>
                </div>
              </div>
