...
(function initProveedoresPaymentsModule(global) {
  ...
  let _devLineasDraft = [];
  let _lastCxpState = null;

  function resolveState() {
    return _lastCxpState || global.state || global.__HERA_STATE__ || {};
  }
  ...
  function render(ctx, mountEl) {
    ...
    if (Svc()?.fetchStockPendientesAbiertos && global.supabaseClient) {
      void Svc()
        .fetchStockPendientesAbiertos(global.supabaseClient)
        .then((pend) => {
          const html = Svc().htmlBannerStockPendiente(pend);
          if (global.__cxpStockBannerHtml !== html) {
            global.__cxpStockBannerHtml = html;
            render(ctx, mountEl);
          }
        });
    }
    ...
    const state = (ctx && ctx.state) || global.state || global.__HERA_STATE__ || {};
    _lastCxpState = state;
    ...
    const invCostoTotal = provs.reduce((s, p) => s + (Svc().metricasProveedor(state, p.id).inventarioCosto || 0), 0);
    const invSinCxpHint =
      deudaTotal <= 0.009 && invCostoTotal > 0.009
        ? `<div class="card" style="margin:0 0 12px;padding:12px;border-color:var(--border);background:rgba(100,140,255,.08)">
      <p style="font-size:12px;color:var(--text2);margin:0;line-height:1.45">
        Hay inventario asociado a proveedores, pero aún no existen compras/cargos CXP. Para generar deuda debes registrar compras a
        <b>crédito</b> o <b>consignación</b>, o crear saldos iniciales desde inventario existente.
      </p>
      <button type="button" class="btn btn-secondary btn-sm" style="margin-top:8px" onclick="AppProveedoresPayments.openComprasInicialesInventarioModal()">Compras iniciales desde inventario…</button>
    </div>`
        : '';
    ...
    const stockBanner = global.__cxpStockBannerHtml || '';
    el.innerHTML = `
    ${stockBanner}
    ${invSinCxpHint}
    ${cxpEmptyHint}
    ...
    <div class="card" style="margin:0 0 16px">
      <div class="card-title" style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px">
        ...
        <div class="btn-group">
          ...
          <button type="button" class="btn btn-secondary btn-sm" onclick="typeof __cxpGoLiveCheck==='function'?__cxpGoLiveCheck():alert('Recarga con compras11')">Diagnóstico</button>
        </div>
      </div>
      <p style="font-size:12px;color:var(--text2);margin:0 0 10px">La deuda se calcula desde compras a crédito o consignación menos pagos y notas crédito. Si una devolución o nota crédito supera la deuda, el excedente queda como saldo a favor del proveedor. Las ventas POS no cambian este saldo.</p>
  ...
  function openModalSinDeudaAbono(state) {
    const esc = Svc().esc;
    const fmt = global.fmt;
    const invCosto = (state.usu_proveedores || []).reduce(
      (s, p) => s + (Svc().metricasProveedor(state, p.id).inventarioCosto || 0),
      0,
    );
    if (typeof global.openModal !== 'function') {
      global.notify?.(
        ...
        'Registra una compra a crédito o consignación en Compras, o usa compras iniciales desde inventario.',
      );
      return;
    }
    global.openModal(
      `<div class="modal-title">Abono a proveedor<button type="button" class="modal-close" onclick="closeModal()">×</button></div>
      <p style="font-size:13px;line-height:1.5;margin:0 0 12px">
        <b>No hay deuda pendiente</b> en cuentas por pagar. Los abonos solo aplican a compras a <b>crédito</b> o <b>consignación</b>.
      </p>
      ${
        invCosto > 0.009
          ? `<p style="font-size:12px;color:var(--text2);margin:0 0 12px">Inventario a costo en pantalla: <b>${fmt(invCosto)}</b> — es referencia de stock×costo, no deuda CXP hasta registrar compras/cargos.</p>`
          : ''
      }
      <div style="display:flex;flex-wrap:wrap;gap:8px">
        <button type="button" class="btn btn-primary btn-sm" onclick="closeModal();AppProveedoresPayments.irACompras()">Ir a Compras</button>
        <button type="button" class="btn btn-secondary btn-sm" onclick="closeModal();AppProveedoresPayments.openComprasInicialesInventarioModal()">Compras iniciales inventario</button>
        <button type="button" class="btn btn-secondary btn-sm" onclick="closeModal()">Cerrar</button>
      </div>`,
      true,
    );
  }

  function openComprasInicialesInventarioModal() {
    if (!Svc()) {
      global.notify?.('danger', '❌', 'Módulo no cargado', 'Falta compras-cxp-service.js.');
      return;
    }
    const state = resolveState();
    const esc = Svc().esc;
    const fmt = global.fmt;
    const grupos = Svc().previewComprasInicialesInventario(state);
    const pendientes = grupos.filter((g) => !g.yaRegistrado && g.lineas.length > 0);
    if (!pendientes.length) {
      global.notify?.(
        'info',
        'ℹ️',
        'Sin inventario pendiente',
        grupos.some((g) => g.yaRegistrado)
          ? 'Ya existen compras iniciales (opening_inventory) para los proveedores con stock.'
          : 'No hay artículos con proveedor, stock y costo para convertir.',
      );
      return;
    }
    const rows = pendientes
      .map(
        (g) => `<tr>
      <td><input type="checkbox" class="cxp-open-chk" data-prov="${esc(g.proveedorId)}" checked /></td>
      <td>${esc(g.proveedorNombre)}</td>
      <td style="text-align:right">${g.lineas.length}</td>
      <td style="text-align:right">${fmt(g.subtotal)}</td>
    </tr>`,
      )
      .join('');
    global.openModal(
      `<div class="modal-title">Compras iniciales desde inventario<button type="button" class="modal-close" onclick="closeModal()">×</button></div>
      <p style="font-size:12px;color:var(--text2);margin:0 0 10px">Preview: crea documentos de compra <b>sin duplicar stock</b>. Marca <code>${esc(Svc().OPENING_INVENTORY_NOTA)}</code> para evitar duplicados.</p>
      <div class="form-group"><label>Tipo de compra inicial</label>
        <select id="cxp-open-tipo" class="form-control">
          <option value="credito">Crédito (genera deuda CXP)</option>
          <option value="consignacion">Consignación (genera deuda CXP)</option>
          <option value="contado">Contado (historial sin deuda)</option>
        </select>
      </div>
      <div class="table-wrap" style="max-height:220px;overflow:auto;margin-bottom:12px">
        <table><thead><tr><th></th><th>Proveedor</th><th>Líneas</th><th style="text-align:right">Total</th></tr></thead>
        <tbody>${rows}</tbody></table>
      </div>
      <button type="button" class="btn btn-primary" id="cxp-open-run" onclick="AppProveedoresPayments.ejecutarComprasInicialesInventario()">Confirmar y crear</button>
      <button type="button" class="btn btn-secondary" style="margin-left:8px" onclick="closeModal()">Cancelar</button>`,
      true,
    );
  }

  async function ejecutarComprasInicialesInventario() {
    const btn = document.getElementById('cxp-open-run');
    if (btn) btn.disabled = true;
    const tipo = document.getElementById('cxp-open-tipo')?.value || 'credito';
    const ids = [...document.querySelectorAll('.cxp-open-chk:checked')].map((el) => el.getAttribute('data-prov')).filter(Boolean);
    if (!ids.length) {
      global.notify?.('warning', '⚠️', 'Sin selección', 'Marca al menos un proveedor.');
      if (btn) btn.disabled = false;
      return;
    }
    try {
      global.showLoadingOverlay?.('connecting');
      const out = await Svc().ejecutarComprasInicialesInventario(ctxBase(), { tipoCompra: tipo, proveedorIds: ids });
      global.showLoadingOverlay?.('hide');
      global.closeModal?.();
      const okN = out.results.filter((r) => r.ok).length;
      const failN = out.results.filter((r) => !r.ok && !r.skipped).length;
      global.notify?.(
        okN ? 'success' : 'warning',
        okN ? '✅' : '⚠️',
        'Compras iniciales',
        `${okN} creada(s), ${failN} error(es). Ejecuta await __cxpGoLiveCheck() en consola.`,
      );
      render(ctxBase());
    } catch (e) {
      global.showLoadingOverlay?.('hide');
      global.notify?.('danger', '❌', 'Error', e.message || String(e));
      console.error('[CXP] ejecutarComprasInicialesInventario', e);
      if (btn) btn.disabled = false;
    }
  }
    ...
    try {
      ...
      if (!provs.length) {
        global.notify?.(
          ...
          'Crea proveedores en Usuarios → Proveedores antes de registrar abonos.',
        );
        return;
      }

      const provsConDeuda = provs.filter((p) => Svc().calcSaldoProveedor(state, p.id).saldo > 0.009);

      if (provIdPre) {
        const saldoPre = Svc().calcSaldoProveedor(state, provIdPre).saldo;
        if (saldoPre <= 0.009) {
          openModalSinDeudaAbono(state);
          return;
        }
      }

      if (!provsConDeuda.length) {
        openModalSinDeudaAbono(state);
        return;
      }
      ...
      console.error('[CXP] openAbonoModal', e);
      global.notify?.('danger', '❌', 'Error al abrir abono', e?.message || String(e));
    }
  }
  ...
  function onAbonoProvChange() {
    ...
    if (!id || !el) return;
    const { saldo } = Svc().calcSaldoProveedor(resolveState(), id);
    el.textContent =
      saldo > 0.009
  ...
  async function guardarAbono() {
    ...
    const stAb = resolveState();
    const valPrev = Svc().validarMontoAbono(stAb, proveedorId, monto, global.fmt);
    if (!valPrev.ok) {
      global.notify?.('warning', '⚠️', 'Abono no válido', valPrev.message);
      if (btn) btn.disabled = false;
      return;
    }
    if (crearEgresoCaja && !(stAb.cajas || []).some((c) => c.estado === 'abierta')) {
  ...
  global.AppProveedoresPayments = {
    ...
    cerrarDetalle,
    irACompras,
    openModalSinDeudaAbono,
    openComprasInicialesInventarioModal,
