...
(function initIngresosEgresosModule(global) {
  let _ctx = null;
  let _ieFechaDesde = null;
  let _ieFechaHasta = null;
  ...
  function renderList(ctx) {
    const { state, fmt, formatDate, today } = ctx;
    ...
    const desdeEl = document.getElementById('ie-fecha-desde');
    const hastaEl = document.getElementById('ie-fecha-hasta');
    if (desdeEl && desdeEl.value) _ieFechaDesde = desdeEl.value;
    if (hastaEl && hastaEl.value) _ieFechaHasta = hastaEl.value;
    if (_ieFechaDesde == null) _ieFechaDesde = t;
    if (_ieFechaHasta == null) _ieFechaHasta = t;
    let desde = _ieFechaDesde;
    let hasta = _ieFechaHasta;
    if (desde > hasta) {
      const x = desde;
      desde = hasta;
      hasta = x;
    ...
    el.innerHTML = `
      ...
      <div style="display:flex;gap:10px;align-items:flex-end;flex-wrap:wrap;margin-bottom:14px">
        <div class="form-group" style="margin:0"><label class="form-label">Desde</label><input type="date" class="form-control" id="ie-fecha-desde" value="${desde}"></div>
        <div class="form-group" style="margin:0"><label class="form-label">Hasta</label><input type="date" class="form-control" id="ie-fecha-hasta" value="${hasta}"></div>
        <button type="button" class="btn btn-secondary" onclick="AppIngresosEgresosModule.refresh()">Filtrar</button>
      </div>

      <div class="card" style="margin-top:12px">
        <div class="card-title">Ingresos / Egresos (${items.length})</div>
