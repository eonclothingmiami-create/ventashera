...
(function initPosRepository(global) {
  ...
  async function applyStockDecrementForLine(state, supabaseClient, articuloId, qty) {
    ...
    if (q <= 0) return;
    const ns = Math.max(0, (art.stock || 0) - q);
    const { error } = await supabaseClient.from('products').update({ stock: ns }).eq('id', articuloId);
        ...
        if (error) {
          ...
          if (typeof notify === 'function') {
            notify(
              ...
              '📦',
              'stock_moves',
              `No se registró la venta en trazabilidad: ${error.message}. El stock no se descontó en BD para esta línea — reintenta o usa «Rellenar movimientos POS» en Pagos a proveedores.`,
              { duration: 9000 }
            );
          }
        } else {
          ...
          try {
            await applyStockDecrementForLine(state, supabaseClient, pid, qty);
          } catch (e) {
            console.warn('[POS] stock products update tras move:', e?.message || e);
            if (typeof notify === 'function') {
              notify('warning', '📦', 'Stock', `Movimiento guardado pero no se actualizó stock: ${e?.message || e}`, { duration: 7000 });
            }
          }
        }
      }
   ...
   * No duplica: compara cantidad neta esperada vs movimientos existentes por factura+producto.
   */
    ...
    try {
      ...
      for (let fi = 0; fi < facturas.length; fi++) {
        ...
        for (const [pid, qtyNeed] of agg.entries()) {
          const net = netCantidadMovesDocProduct(state, docId, pid);
          ...
          const { error } = await supabaseClient.from('stock_moves').insert(row);
          if (error) {
            console.warn('[backfill] stock_moves', error.message, row);
            continue;
          }
          if (!Array.isArray(state.stock_moves_ventas)) state.stock_moves_ventas = [];
          state.stock_moves_ventas.push({
