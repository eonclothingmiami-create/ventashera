...
(function initPosRepository(global) {
  /** Inserts directos: si la tabla tiene `qty` NOT NULL y `cantidad` default 0, activar `STOCK_MOVES_DUAL_WRITE_QTY_CANTIDAD`. */
  function setStockMoveRowQty(row, value) {
    ...
    const col = typeof global.stockMovesQtyColumn === 'function' ? global.stockMovesQtyColumn() : 'cantidad';
    row[col] = value;
  }
  ...
  function insertedRowCantidadForState(row) {
    return typeof global.normalizeStockMoveQtyFromDbRow === 'function'
      ...
      : Number(row.cantidad != null ? row.cantidad : row.qty ?? 0);
  }
