...
(function initFalabellaMaquetador(global) {
  ...
  /**
   * Aplana respuestas tipo Options: { Option: [...] } o Option suelto (Falabella / variaciones JSON).
   */
  function unwrapAttributeOptionList(raw) {
    if (raw == null) return [];
    if (Array.isArray(raw)) return raw;
    if (typeof raw === 'string') return raw.trim() ? [raw.trim()] : [];
    if (typeof raw === 'object') {
      const inner = raw.Option ?? raw.Options ?? raw.option ?? raw.options;
      if (inner != null && inner !== raw) return unwrapAttributeOptionList(inner);
      if (
        raw.Id != null ||
        raw.Name != null ||
        raw.name != null ||
  ...
  /**
   * Opciones autorizadas desde fila atributo GetCategoryAttributes (estructura variable).
   * @param {object} attr
   * @returns {string[]}
   */
  function optionsFromAttribute(attr) {
    ...
    if (!raw) return [];
    const list = unwrapAttributeOptionList(raw);
    const out = [];
    for (const o of list) {
      if (typeof o === 'string') {
  ...
  /**
   * Mapea el atributo al FeedName XML de moda (ProductData) aunque la API use Label distinto o FeedName vacío.
   * @param {object} attr
   * @returns {string|null} TipoDeTrajeDeBano | MaterialDeVestuario | GeneroDeVestuario
   */
  function resolveModaCanonicalFeedKey(attr) {
    if (!attr || typeof attr !== 'object') return null;
    const feed = String(attr.FeedName ?? attr.feedName ?? '').trim();
    const moda = [FEED_NAMES.tipoTraje, FEED_NAMES.material, FEED_NAMES.genero];
    if (moda.includes(feed)) return feed;
    const nf = normalizeModaBlob(feed);
    for (const m of moda) {
      if (nf && nf === normalizeModaBlob(m)) return m;
    }
    const blob = normalizeModaBlob(
      [
        feed,
        attr.Label,
        attr.label,
    ...
    if (!blob) return null;
    if (blob.includes(normalizeModaBlob('TipoDeTrajeDeBano')) || (blob.includes('traje') && blob.includes('bano'))) {
      return FEED_NAMES.tipoTraje;
    }
    if (
      blob.includes(normalizeModaBlob('MaterialDeVestuario')) ||
      (blob.includes('material') && blob.includes('vestuario'))
    ) {
      return FEED_NAMES.material;
    }
    if (
      blob.includes(normalizeModaBlob('GeneroDeVestuario')) ||
      ((blob.includes('genero') || blob.includes('gender')) && blob.includes('vestuario'))
    ) {
      return FEED_NAMES.genero;
    }
    return null;
  }
  ...
  function indexCategoryAttributes(apiRes) {
    ...
    const byFeedName = {};
    const merge = (key, opts) => {
      if (!key || !opts.length) return;
      const prev = byFeedName[key] || [];
      byFeedName[key] = [...new Set([...prev, ...opts])];
    };
    for (const a of list) {
      const opts = optionsFromAttribute(a);
      if (!opts.length) continue;
      const modaKey = resolveModaCanonicalFeedKey(a);
      if (modaKey) {
        merge(modaKey, opts);
        continue;
      }
