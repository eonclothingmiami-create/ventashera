...
(function initGameSystemModule(global) {
  ...
  function buildAlerts(ctx) {
    ...
    const alerts = [];
    const esVitrina = (v) =>
      typeof global.canonCanalVenta === 'function'
        ? global.canonCanalVenta(v) === 'vitrina'
        : String(v.canal || '') === 'vitrina';
    const pend = (state.ventas || []).filter((v) => cuentaOk(v) && !esVitrina(v) && !v.liquidado);
