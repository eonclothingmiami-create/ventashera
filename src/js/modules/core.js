...
async function saveArticulo(existingId) {
    ...
    try {
        ...
        renderArticulos();
        showLoadingOverlay('hide');
        notify('success', '✅', 'Guardado', `${refID} guardado.${catalogVisibleBool ? '' : ' Oculto en catálogo web (el sitio debe filtrar visible=true).'}${mlNote}${metaNote}${googleNote}${pinterestNote}${dropiNote}`);
