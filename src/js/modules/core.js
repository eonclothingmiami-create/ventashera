...
async function loadStockMovesVentasIntoState() {
  ...
  const loadDiag = { path: 'rest', restErr: '', sdkErr: '' };
  try {
    const rows = await fetchAllRowsFiltered('stock_moves', '*', 'tipo=eq.venta_pos&order=id.asc');
    pushRows(rows);
  } catch (e) {
    loadDiag.path = 'sdk';
    loadDiag.restErr = String(e && e.message ? e.message : e);
    ...
    try {
      ...
      while (true) {
        ...
        if (smErr) {
          loadDiag.sdkErr = String(smErr.message || smErr);
      ...
      loadDiag.sdkErr = loadDiag.sdkErr || String(e2 && e2.message ? e2.message : e2);
      console.warn('stock_moves:', e2.message);
    }
  }
  // #region agent log
  try {
    const smv = state.stock_moves_ventas || [];
    fetch('http://127.0.0.1:7397/ingest/fdd15a6c-f2bb-4e50-9cda-32cc3a03f3ff', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-Debug-Session-Id': '5f4c3c' },
      body: JSON.stringify({
        sessionId: '5f4c3c',
        runId: 'post-fix',
        hypothesisId: 'A',
        location: 'core.js:loadStockMovesVentasIntoState',
        message: 'stock_moves venta_pos cargados',
        data: {
          count: smv.length,
          loadPath: loadDiag.path,
          restErr: loadDiag.restErr || null,
          sdkErr: loadDiag.sdkErr || null,
          sampleProductIds: smv.slice(0, 8).map((m) => String(m.productId)),
        },
        timestamp: Date.now(),
      }),
    }).catch(() => {});
