...
/** Total COP para barra de meta del mes: prioriza suma de ingresos POS en caja; si no hay movimientos cargados, tabla `ventas`. */
function totalMetaMensualCOP(s, ym) {
  const tes = s.tes_movimientos;
  if (Array.isArray(tes) && tes.length > 0) {
    return { cop: tesVentaPosMetricsForMonth(tes, ym).ingresosCOP, fuente: 'caja' };
  }
  return { cop: ventasMesCalendario(s, ym).totalCOP, fuente: 'ventas' };
}
...
function renderDashboard(){
    ...
    <div class="rpg-header">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:12px">
        <div>
          ...
          <div style="font-size:9px;color:var(--text2);margin-top:6px;max-width:520px;line-height:1.35;opacity:.88" title="Suma de ingresos categoría venta_pos en caja del mes (${ymMes}); coincide con Trazabilidad de dinero en ese periodo. La barra % usa este total.">ℹ️ Total mes = ingresos POS en caja (${metaTot.fuente==='caja'?'tes_movimientos':'tabla ventas'}). Despachos/racha siguen usando registros por canal.</div>
        </div>
      ...
      <div class="meta-sum-grid">
        <div class="meta-sum-row">
          ...
          <div class="meta-sum-body">
            <div class="meta-sum-title">Ventas del mes (real)</div>
            <div class="meta-sum-sub">${tesMes.ticketsUnicos} ventas POS · mes ${ymMes} · mismo criterio que Trazabilidad</div>
          </div>
        </div>
        <div class="meta-sum-row">
  ...
  <div style="font-size:10px;color:var(--text2);margin:-4px 0 16px;opacity:.9">Referencia por canal (${ymMes}, tabla ventas): vitrina ${fmt(vm.vitrineTotal||0)} · local ${fmt(vm.localTotal||0)} · inter ${fmt(vm.interTotal||0)} · otros ${fmt(vm.otrosTotal||0)} · suma ${fmt(vm.totalCOP)} (puede diferir del total caja si hay descuentos entre tablas).</div>
