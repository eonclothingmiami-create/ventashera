...
${(window.AppRepository?.SUPABASE_URL || (window.RAPPI_SYNC_ENDPOINT || '').trim()) ? `
  ...
  <div style="font-weight: bold; color: var(--text1); margin-bottom: 4px;">🛵 Rappi (automático)</div>
  <div style="font-size:11px;color:var(--text2);line-height:1.45;">Con prenda <strong>visible en catálogo web</strong> al guardar se llama a <code style="font-size:9px;">hera-rappi-sync</code>. Configura <code style="font-size:9px;">RAPPI_STORE_ID</code> en <code style="font-size:9px;">index.html</code> y secrets en Supabase.</div>
</div>` : ''}
${(window.AppRepository?.SUPABASE_URL || (window.FALABELLA_SYNC_ENDPOINT || '').trim()) ? `
  ...
  <div style="font-size:10px;color:var(--text2);margin-top:6px;margin-left:28px;line-height:1.35;">Solo con prenda visible. Credenciales en secrets; columna <code style="font-size:9px;">falabella_*</code> en <code style="font-size:9px;">products</code>. Deploy: <code style="font-size:9px;">falabella-sync-product</code>.</div>
</div>` : ''}
...
async function saveArticulo(existingId) {
    ...
    try {
        ...
        const artLocal = {
          ...
          falabellaProductDataJson: prevArt.falabellaProductDataJson && typeof prevArt.falabellaProductDataJson === 'object' ? prevArt.falabellaProductDataJson : {},
...
function calcDeudaProveedor(provId) {
  ...
  (state.tes_cxp_movimientos || [])
    ...
    .forEach((r) => {
      ...
      if (r.naturaleza === 'cargo') cxpCargo += m;
      else cxpCredito += m;
    });
  const saldoOficialCxp = Math.max(0, cxpCargo - cxpCredito);
  const usaCxp = (state.tes_cxp_movimientos || []).some((r) => String(r.proveedorId) === String(provId));
  const saldo = usaCxp ? saldoOficialCxp : saldoOperativoEstimado;
  return {
    valorInventarioCosto,
    costoVendidoHist: costoVN,
    unidadesVendidasHist: uVN,
    ajusteUnidadesCosto,
    refOperativaTotal,
    ...
    saldoOficialCxp,
    usaCxp,
    difEstimVsCxp: usaCxp ? saldoOperativoEstimado - saldoOficialCxp : null,
    articulos,
    ajustesSalidaCosto,
    ajustesSalidaUds,
    ajustesEntradaCosto,
...
async function backfillStockMovesVentaPos() {
  if (
    !confirm(
...
function openCargoCxpModal(provId = '', provNombre = '') {
  if (window.AppTreasuryModule?.openCargoCxpModal) {
    return window.AppTreasuryModule.openCargoCxpModal({ state, provId, provNombre, openModal, notify, today });
  }
}

function openNotaCreditoCxpModal(provId = '', provNombre = '') {
  if (window.AppTreasuryModule?.openNotaCreditoCxpModal) {
    return window.AppTreasuryModule.openNotaCreditoCxpModal({ state, provId, provNombre, openModal, notify, today });
  }
}

async function guardarCargoCxpMov() {
  if (window.AppTreasuryModule?.guardarCargoCxpMov) {
    return window.AppTreasuryModule.guardarCargoCxpMov({
      state, uid, dbId, today, showLoadingOverlay, supabaseClient, closeModal, renderTesPagosProv, notify, fmt
    });
  }
}
...
async function guardarNotaCreditoCxpMov() {
  if (window.AppTreasuryModule?.guardarNotaCreditoCxpMov) {
    return window.AppTreasuryModule.guardarNotaCreditoCxpMov({
      state, uid, dbId, today, showLoadingOverlay, supabaseClient, closeModal, renderTesPagosProv, notify, fmt
    });
  }
}
...
function verLibroCxpModal(provId, provNombre) {
  if (window.AppTreasuryModule?.verLibroCxpModal) {
    return window.AppTreasuryModule.verLibroCxpModal({ state, provId, provNombre, fmt, formatDate, openModal });
  }
}

async function eliminarCxpMovimiento(id) {
  if (window.AppTreasuryModule?.eliminarCxpMovimiento) {
    return window.AppTreasuryModule.eliminarCxpMovimiento({
...
async function alinearCxpEstimacionProv(provId) {
  if (window.AppTreasuryModule?.alinearCxpEstimacionProv) {
    return window.AppTreasuryModule.alinearCxpEstimacionProv({
