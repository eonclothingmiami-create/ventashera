...
async function saveArticulo(existingId) {
    ...
    try {
        ...
        renderArticulos();
        showLoadingOverlay('hide');
        notify('success', '✅', 'Guardado', `${refID} guardado.${catalogVisibleBool ? '' : ' Oculto en catálogo web (el sitio debe filtrar visible=true).'}${mlNote}${metaNote}`);
...
window.onload = async () => {
  ...
  try {
    if (window.location.hash && /type=recovery/i.test(window.location.hash)) {
      _erpRecoveryPending = true;
    }
  } catch (_) {
    /* noop */
  ...
  supabaseClient.auth.onAuthStateChange(async (event, sess) => {
    ...
    if (event === 'SIGNED_IN' && sess && !_erpSessionBootstrapped && !_erpRecoveryPending) {
      _erpSessionBootstrapped = true;
      if (overlay) overlay.style.display = 'none';
      await loadState();
    }
