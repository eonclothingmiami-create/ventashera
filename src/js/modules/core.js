...
function updateArtComprasHint() {
  ...
  if (!hint || !window.__PAGOS_PROV_REBUILD__) return;
    ...
    openModal(`
        ...
        <div style="max-height: 75vh; overflow-y: auto; padding-right: 10px; text-align:left;">
            ...
            ${
              ...
              Las entradas de mercancía con proveedor (contado o crédito) se registran en <b>Compras</b> (Tesorería). Aquí solo editas catálogo, precios y stock de referencia.
...
async function eliminarAjuste(id) {
  ...
  try {
    ...
    if (a.tipo === 'devolucion' && !pagosProvModuleWritesBlocked()) {
...
async function saveAjusteInv() {
  ...
  try {
    ...
    // Bloqueado durante rebuild de Pagos proveedores — inventario sigue; sin espejo tes_*.
    if (
      tipo === 'devolucion' &&
