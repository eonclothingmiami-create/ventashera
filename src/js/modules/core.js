...
const REWARDS=[{id:'meta_mes',icon:'🍽️',name:'Almuerzo con el Jefe',desc:'Alcanza los $34M de meta mensual',condition:s=>{const ym=yearMonthFromFecha(today());const vm=ventasMesCalendario(s,ym);return vm.active.length>0&&totalMetaMensualCOP(s,ym).cop>=s.meta}},{id:'dia_millon',icon:'💵',name:'Bonificación $100k',desc:'Un día con $1M+ sumando todos los canales (vitrina+local+inter)',condition:s=>hasDiaUnMillon(s)},{id:'super_meta',icon:'👑',name:'Segundo Almuerzo',desc:'Supera los $40M en el mes',condition:s=>{const ym=yearMonthFromFecha(today());return totalMetaMensualCOP(s,ym).cop>=40000000}}];
...
/** Canal POS coherente para sumas del dashboard (mayúsculas/acentos/espacios en BD). `otro` = no reconocido (sigue sumando al total del mes). */
function canonCanalVenta(v){
  const c=String(v&&v.canal!=null?v.canal:'').trim().toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'');
  if(c==='vitrina')return'vitrina';
  if(c==='local')return'local';
  if(c==='inter'||c==='intermunicipal'||c==='inter-municipal')return'inter';
  return'otro';
}
try { window.canonCanalVenta = canonCanalVenta; } catch (e) {}
function ventasMes(s){
  const active=(s.ventas||[]).filter(v=>ventaCuentaParaTotales(v));
  const despachos=active.filter(v=>canonCanalVenta(v)!=='vitrina');
  const vitrina=active.filter(v=>canonCanalVenta(v)==='vitrina');
  const local=active.filter(v=>canonCanalVenta(v)==='local');
  const inter=active.filter(v=>canonCanalVenta(v)==='inter');
  const otros=active.filter(v=>canonCanalVenta(v)==='otro');
  ...
  const otrosTotal=otros.reduce((a,v)=>a+(parseFloat(v.valor)||0),0);
  return{active,despachos,vitrina,local,inter,otros,otrosCount:otros.length,otrosTotal,totalDespachos:despachos.length,totalCOP,totalCOPDespachos,vitrineTotal:vitrina.reduce((a,v)=>a+(parseFloat(v.valor)||0),0),localTotal:local.reduce((a,v)=>a+(parseFloat(v.valor)||0),0),interTotal:inter.reduce((a,v)=>a+(parseFloat(v.valor)||0),0),totalAll};
}
/** Misma forma que ventasMes pero solo ventas cuyo día cae en el mes calendario `ym` (YYYY-MM). */
function ventasMesCalendario(s, ym){
  const active = ventasEnMesCalendario(s.ventas, ym);
  const despachos = active.filter(v => canonCanalVenta(v) !== 'vitrina');
  const vitrina = active.filter(v => canonCanalVenta(v) === 'vitrina');
  const local = active.filter(v => canonCanalVenta(v) === 'local');
  const inter = active.filter(v => canonCanalVenta(v) === 'inter');
...
/** Fecha de venta en YYYY-MM-DD (zona local). Corrige timestamptz de BD que al cortar con slice() podía caer en otro mes. */
function normFechaVentaToYmd(f) {
  if (f == null || f === '') return '';
  const s = String(f).trim();
  if (/^\d{4}-\d{2}-\d{2}$/.test(s)) return s;
  if ((/^\d{4}-\d{2}-\d{2}T/.test(s) && s.includes('T')) || (s.length > 10 && s.includes('T'))) {
    const d = new Date(s);
...
try { window.normFechaVentaToYmd = normFechaVentaToYmd; } catch (e) {}
/** YYYY-MM desde fecha de factura/venta (ISO o timestamptz). */
function yearMonthFromFecha(fechaStr) {
  const ymd = normFechaVentaToYmd(fechaStr);
  return ymd.length >= 7 ? ymd.slice(0, 7) : '';
}
/** Ingresos venta_pos en caja del mes `ym`: suma COP (misma base que Trazabilidad), líneas y tickets únicos (mixto = 2 líneas / 1 venta). */
function tesVentaPosMetricsForMonth(tesMovs, ym) {
  const rows = (tesMovs || []).filter((m) => {
    if (!m || m.tipo !== 'ingreso' || m.categoria !== 'venta_pos') return false;
    const d = normFechaVentaToYmd(m.fecha);
    return yearMonthFromFecha(d) === ym;
  });
  let ingresosCOP = 0;
  rows.forEach((m) => {
    ingresosCOP += parseFloat(m.valor) || 0;
  });
  const tickets = new Set();
  rows.forEach((m) => {
    const matches = String(m.concepto || '').match(/POS-[0-9]+/gi);
    if (matches) matches.forEach((x) => tickets.add(x.toUpperCase()));
  });
  return { lineas: rows.length, ticketsUnicos: tickets.size, ingresosCOP };
}
/** Suma COP de tabla `ventas` en rango de fechas inclusive (YYYY-MM-DD); misma exclusión que la meta (anuladas/archivo). */
function ventasTablaCOPForYmdRange(desdeYmd, hastaYmd) {
  if (!desdeYmd || !hastaYmd) return { cop: 0, facturas: 0 };
  const rows = (state.ventas || []).filter((v) => {
    if (!ventaCuentaParaTotales(v)) return false;
    const d = normFechaVentaToYmd(v.fecha);
    return d && d >= desdeYmd && d <= hastaYmd;
  });
  const cop = rows.reduce((a, v) => a + (parseFloat(v.valor) || 0), 0);
  return { cop, facturas: rows.length };
}
/** Ingresos `tes_movimientos` con categoría venta_pos en rango inclusive (fechas con `normFechaVentaToYmd`). */
function tesVentaPosIngresosForYmdRange(tesMovs, desdeYmd, hastaYmd) {
  if (!desdeYmd || !hastaYmd) return { ingresosCOP: 0, lineas: 0 };
  let ingresosCOP = 0;
  let lineas = 0;
  (tesMovs || []).forEach((m) => {
    if (!m || m.tipo !== 'ingreso' || m.categoria !== 'venta_pos') return;
    const d = normFechaVentaToYmd(m.fecha);
    if (!d || d < desdeYmd || d > hastaYmd) return;
    ingresosCOP += parseFloat(m.valor) || 0;
    lineas++;
  });
  return { ingresosCOP, lineas };
}
try {
  window.ventasTablaCOPForYmdRange = ventasTablaCOPForYmdRange;
  window.tesVentaPosIngresosForYmdRange = tesVentaPosIngresosForYmdRange;
} catch (e) {}
/** Total COP para meta del mes: misma base que las tarjetas por canal (vitrina+local+inter+otros = facturas del mes en tabla `ventas`). */
function totalMetaMensualCOP(s, ym) {
  const vm = ventasMesCalendario(s, ym);
  return { cop: vm.totalCOP, fuente: 'ventas' };
}
/** Mes calendario local actual (timezone del navegador), alineado con `today()` y fechas de venta. */
function calendarYearMonthLocal(){return yearMonthFromFecha(today());}
...
function calcStreak(){const active=(state.ventas||[]).filter(v=>ventaCuentaParaTotales(v)&&canonCanalVenta(v)!=='vitrina');const byDay={};active.forEach(v=>{byDay[v.fecha]=(byDay[v.fecha]||0)+1});let streak=0;const d=new Date(today()+'T12:00:00');while(true){const ds=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;if((byDay[ds]||0)>=5){streak++;d.setDate(d.getDate()-1)}else break}return streak}
...
/** Actualización periódica ligera (sin `loadState` completo). Intervalo por defecto 30s; p. ej. `window.__ERP_BG_REFRESH_MS = 10000` antes de cargar. */
let _bgVentasRefreshTimer = null;
let _bgVentasRefreshInFlight = false;
let _bgVentasSliceStarted = false;
let _bgVisibilityHooked = false;

const _VENTAS_SLICE_PAGE_IDS = new Set([
...
async function hydrateVentasFacturasTesSliceFromSupabase(opts) {
  if (!_sbConnected || !supabaseClient) return { ok: false, skipped: true };
  const includeVc = !!(opts && opts.includeVentasCatalogo);

  let ventas = [];
  try {
    ventas = await fetchAllRows('ventas');
  } catch (e) {
    console.warn('[ventasSlice] ventas:', e.message);
    const { data: v } = await supabaseClient.from('ventas').select('*');
    ventas = v || [];
  }
  ...
  try {
    ...
    console.warn('[ventasSlice] invoices:', e.message);
  ...
  try {
    ...
    console.warn('[ventasSlice] tes_movimientos:', e.message);
  ...
  if (includeVc) {
    try {
      ...
      console.warn('[ventasSlice] ventas_catalogo:', e.message);
    }
  }

  state.ventas = (ventas || []).map((v) => ({
    id: v.id,
    fecha: normFechaVentaToYmd(v.fecha) || v.fecha,
    canal: v.canal,
    ...
    liquidado: v.liquidado || false,
    fechaLiquidacion: v.fecha_liquidacion,
    esSeparado: v.es_separado || false,
    esContraEntrega: v.es_contraentrega || false,
  ...
  state.facturas = (facturas || []).map((f) => {
    ...
    return {
      ...
      cedulaCliente: f.cedula_cliente || '',
      comprobante: f.comprobante || '',
      esSeparado: !!f.es_separado,
      tipoPago: f.tipo_pago || 'contado'
    };
  });

  state.tes_movimientos = (tesMov || []).map((m) => ({
    id: m.id,
    cajaId: m.caja_id,
    tipo: m.tipo,
...
function pulseVentasSliceUI() {
  ...
  const active = document.querySelector('.page.active')?.id?.replace('page-', '');
  if (!active || !_VENTAS_SLICE_PAGE_IDS.has(active)) return;
  if (typeof renderPage === 'function') renderPage(active);
}

async function tickBackgroundVentasSliceRefresh() {
  if (!_sbConnected || !supabaseClient) return;
  if (typeof document !== 'undefined' && document.hidden) return;
  const G = window.AppSyncGuard;
  if (G?.isBusy?.()) return;
  if (_bgVentasRefreshInFlight) return;
  if (document.getElementById('modal-overlay')?.classList.contains('active')) return;
  const active = document.querySelector('.page.active')?.id?.replace('page-', '');
  ...
  try {
    ...
    console.warn('[bgVentasSlice]', e);
  } finally {
    _bgVentasRefreshInFlight = false;
  }
}

function startBackgroundVentasSliceRefresh() {
  if (_bgVentasSliceStarted) return;
  _bgVentasSliceStarted = true;
  const ms = Math.max(5000, Number(window.__ERP_BG_REFRESH_MS) || 30000);
  if (_bgVentasRefreshTimer) clearInterval(_bgVentasRefreshTimer);
  _bgVentasRefreshTimer = setInterval(tickBackgroundVentasSliceRefresh, ms);
  if (!_bgVisibilityHooked) {
    _bgVisibilityHooked = true;
    document.addEventListener('visibilitychange', () => {
      if (!document.hidden) tickBackgroundVentasSliceRefresh();
    });
  }
}

function stopBackgroundVentasSliceRefresh() {
  if (_bgVentasRefreshTimer) {
    clearInterval(_bgVentasRefreshTimer);
    _bgVentasRefreshTimer = null;
  }
  _bgVentasSliceStarted = false;
}

try {
  window.hydrateVentasFacturasTesSliceFromSupabase = hydrateVentasFacturasTesSliceFromSupabase;
  window.startBackgroundVentasSliceRefresh = startBackgroundVentasSliceRefresh;
  window.stopBackgroundVentasSliceRefresh = stopBackgroundVentasSliceRefresh;
} catch (e) {}
...
async function loadState() {
  ...
  try {
    const [
      ...
      Promise.resolve({data:[]}), // ventas: fetchAllRows debajo (evita techo ~1000 filas → total mes falsamente bajo)
    ...
    try {
      ...
      console.warn('ventas (paginado):', e.message);
      const { data: v } = await supabaseClient.from('ventas').select('*');
      ventas = v || [];
    }
    ...
    // Ventas
    state.ventas = (ventas||[]).map(v=>({id:v.id,fecha:normFechaVentaToYmd(v.fecha)||v.fecha,canal:v.canal,valor:parseFloat(v.valor)||0,
    ...
    (configs||[]).forEach(c=>{
      try {
        ...
        else if(c.key==='erp_calendar_month')state.erpCalendarMonth=(typeof val==='string'&&/^\d{4}-\d{2}$/.test(val))?val:null;
    ...
    renderAll();
    try {
      startBackgroundVentasSliceRefresh();
    } catch (e) {}
...
/** Insignias ligadas al avance % de la meta mensual (se pueden volver a ganar cada mes). */
function resetMonthlyMetaBadges(){
  if(!state.game) state.game={xp:0,streakMax:0,earnedBadges:[],claimedSnacks:{}};
  const strip={meta25:1,meta50:1,meta75:1,meta100:1,super:1};
  state.game.earnedBadges=(state.game.earnedBadges||[]).filter(id=>!strip[id]);
}
function checkMonthReset(){
  const ymNow=calendarYearMonthLocal();
  const prev=state.erpCalendarMonth;
  let rolled=false;
  if(prev && prev!==ymNow){
    resetMonthlyMetaBadges();
    rolled=true;
    notify('milestone','📅','Nuevo mes','Progreso de meta e insignias mensuales reiniciados. Tus ventas y demás registros no se archivan automáticamente.',{duration:6000});
  }
  state.currentMonth=ymNow;
  state.erpCalendarMonth=ymNow;
  const needPersistKey=!prev || prev!==ymNow;
  if(needPersistKey && _sbConnected) saveConfig('erp_calendar_month',ymNow);
  if(rolled && _sbConnected) saveConfig('game',state.game);
}
window._erpResetMonthlyMetaBadges=resetMonthlyMetaBadges;
...
function updateNavBadges(){
  const pend=(state.ventas||[]).filter(v=>ventaCuentaParaTotales(v)&&canonCanalVenta(v)!=='vitrina'&&!v.liquidado);
...
function checkBadges(){const active=(state.ventas||[]).filter(v=>ventaCuentaParaTotales(v));const despachos=active.filter(v=>canonCanalVenta(v)!=='vitrina');const ymB=calendarYearMonthLocal();const metaTot=totalMetaMensualCOP(state,ymB);const pct=state.meta>0?metaTot.cop/state.meta:0;if(active.length>=1)checkAndAwardBadge('primera_venta');if(despachos.length>=20)checkAndAwardBadge('v20');if(despachos.length>=50)checkAndAwardBadge('v50');if(despachos.length>=100)checkAndAwardBadge('v100');if(despachos.length>=150)checkAndAwardBadge('v150');if(pct>=0.25)checkAndAwardBadge('meta25');if(pct>=0.50)checkAndAwardBadge('meta50');if(pct>=0.75)checkAndAwardBadge('meta75');if(pct>=1.00)checkAndAwardBadge('meta100');if(metaTot.cop>=40000000)checkAndAwardBadge('super');if(active.some(v=>(parseFloat(v.valor)||0)>=500000))checkAndAwardBadge('gran_venta');const hoy=today();const todayCanals=new Set(active.filter(v=>v.fecha===hoy).map(v=>canonCanalVenta(v)));if(todayCanals.has('vitrina')&&todayCanals.has('local')&&todayCanals.has('inter'))checkAndAwardBadge('multicanal')}
...
function renderDashboard(){
  checkMonthReset();
  const ymMes=calendarYearMonthLocal();
  const vm=ventasMesCalendario(state,ymMes);
  const metaTot=totalMetaMensualCOP(state,ymMes);
  const tesMes=tesVentaPosMetricsForMonth(state.tes_movimientos||[],ymMes);
  const conciliaMesDelta=(tesMes.ingresosCOP||0)-metaTot.cop;
  const metaGoal=parseFloat(state.meta)||0;
  const pct=metaGoal>0?Math.min(100,(metaTot.cop/metaGoal)*100):0;
  const faltaMeta=Math.max(0,metaGoal-metaTot.cop);
  const g=state.game||{};
  const lv=calcLevel(g.xp||0);
  const{lv:lvp,next:nextLv,pct:xpPct,xpToNext}=calcLevelProgress(g.xp||0);
  const streak=calcStreak();
  const hoy=today();
  const ventasHoy=(state.ventas||[]).filter(v=>ventaCuentaParaTotales(v)&&v.fecha===hoy&&canonCanalVenta(v)!=='vitrina');
  const pendientes=(state.ventas||[]).filter(v=>ventaCuentaParaTotales(v)&&canonCanalVenta(v)!=='vitrina'&&!v.liquidado).length;
  ...
  const facturasMesOtrasFechas=sortVentasRecientes(ventasActivasNoArch.filter(v=>yearMonthFromFecha(v.fecha)===ymMes&&v.fecha!==hoy)).slice(0,8);
  const resumenMesCal=ventasEnMesCalendario(state.ventas,ymMes);
  ...
  const hoyVitrina=hoyVentas.filter(v=>canonCanalVenta(v)==='vitrina');
  const hoyVitrinaMostrador=hoyVitrina.filter(v=>!v.esSeparado);
  const hoyVitrinaSeparados=hoyVitrina.filter(v=>v.esSeparado);
  const hoyLocal=hoyVentas.filter(v=>canonCanalVenta(v)==='local');
  const hoyInter=hoyVentas.filter(v=>canonCanalVenta(v)==='inter');
  const hoyOtros=hoyVentas.filter(v=>canonCanalVenta(v)==='otro');
    ...
    <div class="rpg-header">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:12px">
        <div>
          ...
          <div style="font-family:Syne;font-size:22px;font-weight:800">${fmt(metaTot.cop)} <span style="font-size:13px;color:var(--text2);font-weight:400">de ${fmt(metaGoal)}</span></div>
          <div style="font-size:12px;font-weight:700;color:${faltaMeta<=0?'var(--green)':'var(--accent)'};margin-top:8px">${faltaMeta<=0?'✓ Meta cubierta con ingresos del mes':`Faltan ${fmt(faltaMeta)} para la meta`}</div>
          <div style="font-size:9px;color:var(--text2);margin-top:6px;max-width:520px;line-height:1.35;opacity:.88" title="Suma de facturas del mes por canal; coincide con vitrina + local + inter + otros abajo.">ℹ️ Meta = suma ventas del mes (facturas por canal). La barra % usa el mismo total que las tarjetas.</div>
        </div>
      ...
      <div class="meta-sum-grid">
        <div class="meta-sum-row">
          <div class="meta-sum-val">${fmt(metaTot.cop)}</div>
          <div class="meta-sum-body">
            <div class="meta-sum-title">Ventas del mes <span style="font-size:10px;font-weight:600;color:var(--text2);opacity:.92">· ${ymMes}</span></div>
            <div class="meta-sum-sub">${resumenMesCal.length} facturas · acumulado del mes (vitrina+local+inter+otros) · igual al % meta</div>
          </div>
        </div>
        <div class="meta-sum-row">
          <div class="meta-sum-val">${fmt(sumValor(hoyVitrina))}</div>
          <div class="meta-sum-body">
            <div class="meta-sum-title">Vitrina · solo hoy</div>
            <div class="meta-sum-sub">${formatDate(hoy)} · ${hoyVitrina.length} fact. · mostrador + separados (si hoy no vendiste acá, verás $0 aunque el mes tenga total arriba)</div>
          </div>
        </div>
        <div class="meta-sum-row">
          <div class="meta-sum-val">${fmt(sumDespachosHoy)}</div>
          <div class="meta-sum-body">
            <div class="meta-sum-title">Despachos · solo hoy</div>
            <div class="meta-sum-sub">${formatDate(hoy)} · ${ventasHoy.length} fact. · todo canal ≠ vitrina (local ${hoyLocal.length} · inter ${hoyInter.length}${hoyOtros.length ? ` · otros ${hoyOtros.length}` : ''})</div>
          </div>
        </div>
      </div>
      <div style="margin-top:12px;padding:10px 12px;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.12);border-radius:12px;font-size:10px;line-height:1.55;color:var(--text2)">
        <div style="font-weight:700;color:var(--text);margin-bottom:6px">📎 ¿Dónde queda lo de «trazabilidad» vs los ${fmt(metaTot.cop)} de la meta?</div>
        <div><b>Meta y tarjetas por canal</b> suman solo la tabla <code style="font-size:9px">ventas</code>: <b>${fmt(metaTot.cop)}</b> (${resumenMesCal.length} facturas en ${ymMes}).</div>
        <div style="margin-top:4px"><b>Caja POS</b> es otra tabla: <code style="font-size:9px">tes_movimientos</code> con clase <b>venta_pos</b>: <b>${fmt(tesMes.ingresosCOP||0)}</b> (${tesMes.lineas||0} líneas en caja · ~${tesMes.ticketsUnicos||0} tickets).</div>
        <div style="margin-top:8px;padding-top:8px;border-top:1px solid var(--border)">${Math.abs(conciliaMesDelta)<=0.5?'<span style="color:var(--green)">Δ (caja − facturas) ~0</span> en este mes.':`<span style="color:var(--yellow)">Δ caja − facturas: ${fmt(conciliaMesDelta)}</span>. Suele pasar si hay cobros en caja sin fila en ventas, facturas anuladas, fechas distintas entre caja y factura, o varias líneas de pago (efectivo + transferencia) por una misma venta.`}</div>
        <div style="margin-top:8px;font-size:9px;opacity:.88">Para cuadrar un día concreto: <button type="button" class="btn btn-xs btn-secondary" onclick="showPage('tes_dinero')">Trazabilidad de dinero</button> → mismo rango en ambas columnas del cuadro de conciliación.</div>
      </div>
    </div>
  ...
  <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(100px,1fr));gap:10px;margin-bottom:10px">
    <div class="rpg-stat" title="Facturas del mes con canal distinto a vitrina (local + inter + otros). Suma con Vitrina = facturas del mes."><div class="rpg-stat-val" style="color:var(--accent2)">${vm.totalDespachos}</div><div class="rpg-stat-label">📦 Sin vitrina</div><div style="font-size:9px;color:var(--text2);margin-top:3px;line-height:1.2;opacity:.9">Mes · despachos</div></div>
    ...
    <div class="rpg-stat"><div class="rpg-stat-val" style="color:var(--text2)">${vm.otrosCount||0}</div><div class="rpg-stat-label">📌 Otros</div><div style="font-size:10px;color:var(--text2);margin-top:2px">${fmt(vm.otrosTotal||0)}</div><div style="font-size:8px;color:var(--text2);margin-top:3px;opacity:.85">canal vacío / distinto</div></div>
  ...
  <div style="font-size:10px;color:var(--text2);margin:-4px 0 16px;opacity:.9">Suma por canal (${ymMes}): vitrina ${fmt(vm.vitrineTotal||0)} + local ${fmt(vm.localTotal||0)} + inter ${fmt(vm.interTotal||0)} + otros ${fmt(vm.otrosTotal||0)} = <b>${fmt(vm.totalCOP)}</b> (igual al total meta arriba).</div>
      ...
      <div class="card" style="margin:0;flex:1">
        ...
        <div style="margin-top:12px;padding-top:10px;border-top:1px solid var(--border);display:grid;grid-template-columns:1fr 1fr;gap:10px;font-size:11px;line-height:1.45">
          ...
          <div>
            ...
            ${hoyOtros.length ? `<div style="color:var(--text2)">📌 Otros canal <span style="color:var(--text)">${hoyOtros.length}</span> · ${fmt(sumValor(hoyOtros))}</div>` : ''}
            <div style="margin-top:4px;font-weight:700;color:var(--text)">∑ ≠ vitrina ${ventasHoy.length} · ${fmt(sumDespachosHoy)}</div>
          </div>
        </div>
        <div style="font-size:10px;color:var(--text2);margin-top:10px;opacity:.9">Misión “5 despachos”: cuenta ventas hoy sin vitrina (local+inter+otros) → ${ventasHoy.length} · ${fmt(sumDespachosHoy)}</div>
      </div>
    </div>
  </div>
  ...
  <div class="card">
    <div class="card-title">🗓️ MES ${ymMes} — OTRAS FECHAS (hasta 8 más recientes)</div>
    <div style="font-size:11px;color:var(--text2);margin:-6px 0 10px">Mes en caja (meta): <b>${fmt(metaTot.cop)}</b> · ${tesMes.ticketsUnicos} POS · tabla ventas este mes: <b>${resumenMesCal.length}</b> fact. · <b>${fmt(totalMesCal)}</b></div>
...
function renderPOS(){
  ...
  try {
    ...
    if (cartEl) posCartScroll = cartEl.scrollTop;
  } catch (_) {}
...
function promptTallaYAgregar(artId){
  ...
  const tallasRaw=art.tallas?art.tallas.split(','):['XS','S','M','L','XL','Única'];
  const tallas=tallasRaw.map((t)=>t.trim()).filter(Boolean);
  if(tallas.length===0)tallas.push('Única');
  if(tallas.length===1){addToCart(artId,tallas[0]);return}
  const esc=(s)=>String(s||'').replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/</g,'&lt;');
  const esVideo=art.imagen&&String(art.imagen).split('?')[0].toLowerCase().match(/\.(mp4|mov|webm|avi)$/);
  const btnClass=(state.pos_cart||[]).length>0?'btn btn-secondary pos-talla-btn pos-talla-btn--compact':'btn btn-secondary pos-talla-btn';
  const btnGrid=tallas.map((t)=>{
    const tl=t.trim();
    // onclick en comillas simples: si usamos "… ${JSON.stringify(tl)} …" las comillas de JSON rompen el atributo HTML.
    return`<button type="button" class="${btnClass}" onclick='addToCart(${JSON.stringify(artId)}, ${JSON.stringify(tl)})'>${esc(tl)}</button>`;
  }).join('');
  const quickMode=(state.pos_cart||[]).length>0;
  if(quickMode){
    let thumb='';
    if(art.imagen&&!esVideo){
      thumb=`<img src="${esc(art.imagen)}" alt="" class="pos-talla-quick-thumb"/>`;
    }else if(esVideo&&art.imagen){
      thumb=`<div class="pos-talla-quick-thumb-wrap"><video src="${esc(art.imagen)}" muted loop autoplay playsinline class="pos-talla-quick-thumb-video"></video></div>`;
    }else{
      thumb=`<span class="pos-talla-quick-emoji">${art.emoji||'👙'}</span>`;
    }
    openModal(`
      <div class="modal-title pos-talla-quick-title">Otra prenda · elige talla<button class="modal-close" onclick="closeModal()">×</button></div>
      <div class="pos-talla-quick">
        <div class="pos-talla-quick-head">
          ${thumb}
          <div class="pos-talla-quick-meta">
            <div class="pos-talla-quick-name">${esc(art.nombre)}</div>
            <div class="pos-talla-quick-price">${fmt(art.precioVenta)}</div>
          </div>
        </div>
        <div class="pos-talla-grid pos-talla-grid--compact">${btnGrid}</div>
      </div>
    `,false);
    return;
  }
  let preview='';
  if(art.imagen&&!esVideo){
    preview=`<div style="border-radius:16px;overflow:hidden;border:1px solid var(--border);background:var(--bg3);max-height:240px;aspect-ratio:3/4"><img src="${esc(art.imagen)}" alt="" style="width:100%;height:100%;object-fit:cover;display:block"/></div>`;
  }else if(esVideo&&art.imagen){
    preview=`<div style="border-radius:16px;overflow:hidden;border:1px solid var(--border);max-height:240px;aspect-ratio:3/4;background:#000"><video src="${esc(art.imagen)}" muted loop autoplay playsinline style="width:100%;height:100%;object-fit:cover;display:block"></video></div>`;
  }else{
    preview=`<div style="display:flex;align-items:center;justify-content:center;min-height:180px;border-radius:16px;background:var(--bg3);border:1px solid var(--border);font-size:72px">${art.emoji||'👙'}</div>`;
  }
  openModal(`
    <div class="modal-title" style="margin-bottom:12px">Facturar prenda<button class="modal-close" onclick="closeModal()">×</button></div>
    <div class="pos-talla-layout">
      <div class="pos-talla-preview">
        ${preview}
        <div class="pos-talla-nombre">${esc(art.nombre)}</div>
        <div class="pos-talla-precio">${fmt(art.precioVenta)}</div>
        ${art.descripcion?`<div class="pos-talla-desc">${esc(art.descripcion)}</div>`:''}
      </div>
      <div class="pos-talla-rail">
        <div class="pos-talla-rail-label">Elige talla</div>
        <div class="pos-talla-grid">${btnGrid}</div>
      </div>
    </div>
  `,true);
}
...
function renderPendientes(){
  ...
  const pend=(state.ventas||[]).filter(v=>ventaCuentaParaTotales(v)&&canonCanalVenta(v)!=='vitrina'&&!v.liquidado).sort((a,b)=>(a.fechaLiquidacion||'')>(b.fechaLiquidacion||'')?1:-1);
