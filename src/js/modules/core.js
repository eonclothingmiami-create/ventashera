...
${(window.AppRepository?.SUPABASE_URL || (window.MERCADOLIBRE_SYNC_ENDPOINT || '').trim()) ? `
            <div class="form-group" style="margin-top: 8px; padding: 10px; background: rgba(255,230,120,0.12); border-radius: 8px; border: 1px solid rgba(255,200,80,0.35);">
  <label style="display: flex; align-items: center; gap: 10px; cursor: pointer; color: var(--text1); font-weight: bold;">
...
async function saveArticulo(existingId) {
    ...
    try {
        ...
        const mlEndpoint = getMercadoLibreSyncEndpoint();
        const mlChk = document.getElementById('art-sync-mercadolibre');
        const wantMercadoLibre =
          mlEndpoint && mlChk && mlChk.checked && catalogVisibleBool;
        let mlNote = '';
        if (wantMercadoLibre) {
          try {
            ...
            if (mlRes && mlRes.skipped) {
              mlNote = ' · ML: no se llamó al endpoint';
            } else if (mlRes && mlRes.dryRun) {
              mlNote = ' · ML: modo prueba (configura ML_ACCESS_TOKEN y ML_DEFAULT_CATEGORY_ID_MCO en Supabase)';
              notify('warning', '🛒', 'Mercado Libre', mlRes.message || 'Secrets pendientes — revisa la función en Supabase.', { duration: 8000 });
            } else if (mlRes && mlRes.ok) {
              const link = mlRes.permalink || mlRes.itemId || '';
              mlNote = link ? ` · ML: ${link}` : ' · ML: publicado';
            } else {
              mlNote = ' · ML: respuesta inesperada';
            }
          } catch (mlErr) {
            console.warn('[Mercado Libre]', mlErr);
            mlNote = ' · ML: error (consola / Edge Function)';
        ...
        renderArticulos();
        showLoadingOverlay('hide');
        notify('success', '✅', 'Guardado', `${refID} guardado.${catalogVisibleBool ? '' : ' Oculto en catálogo web (el sitio debe filtrar visible=true).'}${mlNote}`);
