...
async function loadFalabellaCategoryOptionsIntoMaquetador() {
  ...
  try {
    ...
    notify(
      ...
      '🏬',
      'Falabella',
      `Opciones cargadas (${(res.attributes || []).length} atributos).`,
      { duration: 5000 },
    );
    refreshFalabellaMaquetadorValidation();
  } catch (e) {
...
async function runOneCriticalRefresh(slice) {
  ...
  // #region agent log
  fetch('http://127.0.0.1:7397/ingest/fdd15a6c-f2bb-4e50-9cda-32cc3a03f3ff',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'5f4c3c'},body:JSON.stringify({sessionId:'5f4c3c',location:'core.js:refreshCriticalSlice',message:'refresh start',data:{slice,_sbConnected:!!_sbConnected},timestamp:Date.now(),hypothesisId:'H1'})}).catch(()=>{});
  // #endregion
  if (!_sbConnected || !supabaseClient) {
  ...
  try {
    ...
    updateNavBadges();
    // #region agent log
    fetch('http://127.0.0.1:7397/ingest/fdd15a6c-f2bb-4e50-9cda-32cc3a03f3ff',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'5f4c3c'},body:JSON.stringify({sessionId:'5f4c3c',location:'core.js:refreshCriticalSlice',message:'refresh ok',data:{slice},timestamp:Date.now(),hypothesisId:'H1'})}).catch(()=>{});
    // #endregion
    return { ok: true };
  } catch (e) {
    // #region agent log
    fetch('http://127.0.0.1:7397/ingest/fdd15a6c-f2bb-4e50-9cda-32cc3a03f3ff',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'5f4c3c'},body:JSON.stringify({sessionId:'5f4c3c',location:'core.js:refreshCriticalSlice',message:'refresh fail',data:{slice,err:String(e&&e.message||e)},timestamp:Date.now(),hypothesisId:'H2'})}).catch(()=>{});
    // #endregion
...
async function ensureAntiDesyncBefore(module) {
  const G = window.AppSyncGuard;
  // #region agent log
  fetch('http://127.0.0.1:7397/ingest/fdd15a6c-f2bb-4e50-9cda-32cc3a03f3ff',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'5f4c3c'},body:JSON.stringify({sessionId:'5f4c3c',location:'core.js:ensureAntiDesyncBefore',message:'ensure',data:{module,busy:!!G?.isBusy?.(),stale:!!G?.isStale?.(module)},timestamp:Date.now(),hypothesisId:'H3'})}).catch(()=>{});
  // #endregion
    ...
    openModal(`
        ...
        <div style="max-height: 75vh; overflow-y: auto; padding-right: 10px; text-align:left;">
            ...
            <div id="falabella-requisitos-wrap" style="display:none;padding:14px;border-radius:10px;border:1px solid rgba(100,80,200,0.35);background:rgba(100,80,200,0.06);margin-bottom:16px;">
                ...
                <p style="font-size:10px;color:var(--text2);margin:0 0 10px;line-height:1.35;">Completa PrimaryCategory y pulsa «Cargar opciones» para rellenar listas (GetCategoryAttributes). No uses texto libre en campos con desplegable.</p>
                <div class="form-row">
                ...
                <div class="form-row">
                    <div class="form-group"><label class="form-label">Tipo traje baño</label><select class="form-control" id="m-fal-tipo-traje"><option value="">— Cargar opciones —</option></select></div>
                    <div class="form-group"><label class="form-label">Material vestuario</label><select class="form-control" id="m-fal-material"><option value="">— Cargar opciones —</option></select></div>
                    <div class="form-group"><label class="form-label">Género vestuario</label><select class="form-control" id="m-fal-genero"><option value="">— Cargar opciones —</option></select></div>
                </div>
...
function renderDocumentList(pageId,title,collection,tipo,fields){
  ...
  }).join('')||`<tr><td colspan="6" style="text-align:center;color:var(--text2);padding:24px">Sin registros</td></tr>`;
