    ...
    try {
      ...
      while (supabaseClient) {
        const { data: smRows, error: smErr } = await supabaseClient
          .from('stock_moves')
          .select('id,product_id,cantidad,tipo,fecha,referencia,documento_id')
          .eq('tipo', 'venta_pos')
...
async function saveArticulo(existingId) {
    ...
    try {
        ...
        const mlEndpoint = ((window.MERCADOLIBRE_SYNC_ENDPOINT || '').trim()) ||
          (window.AppRepository?.SUPABASE_URL
            ? String(window.AppRepository.SUPABASE_URL).replace(/\/$/, '') + '/functions/v1/mercadolibre-sync-product'
            : '');
        const mlChk = document.getElementById('art-sync-mercadolibre');
...
function calcDeudaProveedor(provId) {
  ...
  (state.stock_moves_ventas||[]).forEach(m=>{
    const art = (state.articulos||[]).find(x=>x.id===m.productId);
