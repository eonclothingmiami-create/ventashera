...
async function reenviarFalabellaFeedModal() {
  ...
  try {
    ...
    if (falRes && falRes.ok) {
      const rid = falRes.requestId ? ` · Feed ${falRes.requestId}` : '';
      notify('success', '🏬', 'Falabella', (falRes.message || 'Feed enviado.') + (falRes.sellerSku ? ` · SKU ${falRes.sellerSku}` : '') + rid, { duration: 10000 });
      if (typeof console !== 'undefined' && console.info) {
      ...
      if (art) {
        art.falabellaSyncStatus = 'synced';
...
${(window.AppRepository?.SUPABASE_URL || (window.FALABELLA_SYNC_ENDPOINT || '').trim()) ? `
  ...
  <div style="font-size:10px;color:var(--text2);margin-top:4px;line-height:1.35;">Usa automáticamente el id de este artículo en Supabase (no el requestId del feed de la consola).</div>` : `<div style="font-size:10px;color:var(--text2);margin-top:8px;line-height:1.35;">Tras guardar el artículo podrás reenviar el feed sin copiar UUID a mano.</div>`}
</div>` : ''}
