...
function renderDashboard(){
  checkMonthReset();
  const ymMeta=calendarYearMonthLocal();
  const vm=ventasMesCalendario(state,ymMeta);
    ...
    <div class="rpg-header">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:12px">
        <div>
          ...
          <div style="font-size:9px;color:var(--text2);margin-top:4px;max-width:520px;line-height:1.35;opacity:.88" title="Barra % y cifra principal: ventas del mes calendario actual (vitrina+local+inter). Insignias v20–v150: acumulado histórico de despachos; racha y misión 5 despachos: solo local/inter sin vitrina.">ℹ️ Meta mensual = ventas del mes calendario (${ymMeta}). Logros de “despacho” no cuentan vitrina.</div>
        </div>
  ...
  <div style="font-size:10px;color:var(--text2);margin:-4px 0 16px;opacity:.9">Suma canales (${ymMeta}): ${fmt((vm.vitrineTotal||0)+(vm.localTotal||0)+(vm.interTotal||0)+(vm.otrosTotal||0))} = total mes ${fmt(vm.totalCOP)}</div>
