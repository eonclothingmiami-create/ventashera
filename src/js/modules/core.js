...
// ===================================================================
// ===== SINCRONIZADORES CATÃLOGO WEB (SUPABASE) =====
// ===================================================================

async function syncProductToSupabase(articulo) {
  try {
    // 1. Mapear la data de ERP a lo que necesita el CatÃ¡logo Supabase
    const productData = {
...
async function deleteProductFromSupabase(codigo) {
  try {
    ...
    console.error('âŒ Error eliminando de Supabase:', err);
    return { success: false, error: err.message };
  }
}
...
async function importProductsFromSupabase() {
  try {
    ...
    const SECCION_MAP = {
      'Trajes de BaÃ±o': 'Trajes de baÃ±o',
    ...
    for (const p of products) {
      ...
      const articulo = {
        ...
        stockMinimo: 0,
        activo: true,
        mostrarEnWeb: normalizeVisibleFlag(p.visible),
    ...
    console.error('âŒ Error importando de Supabase:', err);
    return [];
  }
}
...
let state = {
  ...
  nom_anticipos: [],
  nom_conceptos: [
    {id:'c1',nombre:'Salario BÃ¡sico',tipo:'devengo',formula:'fijo',valor:0},
    ...
    {id:'c4',nombre:'PensiÃ³n (4%)',tipo:'deduccion',formula:'porcentaje',valor:4}
  ],
  nom_nominas: [],
  ...
  // ===== CONFIGURACIONES =====
  cfg_categorias: [
    {id:'cat1',seccion:'Trajes de BaÃ±o',nombre:'Enterizos'},
    {id:'cat2',seccion:'Trajes de BaÃ±o',nombre:'Bikinis'},
    {id:'cat3',seccion:'Trajes de BaÃ±o',nombre:'Tankinis'},
    {id:'cat4',seccion:'Trajes de BaÃ±o',nombre:'Asoleadores'},
    {id:'cat5',seccion:'Trajes de BaÃ±o',nombre:'Salidas de BaÃ±o'},
    {id:'cat6',seccion:'Trajes de BaÃ±o',nombre:'3 Piezas'},
  ...
  cfg_secciones: [
    {id:'sec1',nombre:'Trajes de BaÃ±o'},
  ...
  cfg_transportadoras: [
    ...
    {id:'t3',nombre:'EnvÃ­a',activa:true},
    {id:'t4',nombre:'InterrapidÃ­simo',activa:true},
  ...
  cfg_tarifas: [
    ...
    {id:'tar2',nombre:'Precio PÃºblico',porcentaje:15,descripcion:'Mayorista + 15k'},
...
// ===== CONSTANTS =====
const LEVELS=[{level:1,name:'Novata',avatar:'ðŸŒ±',minXp:0},{level:2,name:'Activa',avatar:'âœ¨',minXp:200},{level:3,name:'Vendedora',avatar:'ðŸ’«',minXp:500},{level:4,name:'Destacada',avatar:'ðŸŒŸ',minXp:1000},{level:5,name:'Pro',avatar:'âš¡',minXp:1800},{level:6,name:'Experta',avatar:'ðŸ”¥',minXp:3000},{level:7,name:'Campeona',avatar:'ðŸ’Ž',minXp:4500},{level:8,name:'Leyenda',avatar:'ðŸ‘‘',minXp:6500},{level:9,name:'Ã‰lite',avatar:'ðŸ†',minXp:9000},{level:10,name:'Mega Vendedora',avatar:'ðŸš€',minXp:12000}];
const BADGES=[{id:'primera_venta',icon:'ðŸŽ¯',name:'Primera Venta',desc:'Registra tu primera venta'},{id:'racha3',icon:'ðŸ”¥',name:'Racha x3',desc:'3 dÃ­as consecutivos con â‰¥5 despachos'},{id:'racha7',icon:'ðŸŒŠ',name:'Racha x7',desc:'7 dÃ­as seguidos con â‰¥5 despachos'},{id:'racha14',icon:'âš¡',name:'Racha x14',desc:'14 dÃ­as seguidos con â‰¥5 despachos'},{id:'meta25',icon:'ðŸ“ˆ',name:'25% Meta',desc:'Alcanza 25% de la meta mensual'},{id:'meta50',icon:'ðŸŽ¯',name:'Mitad Meta',desc:'Alcanza el 50% de la meta mensual'},{id:'meta75',icon:'ðŸ”',name:'75% Meta',desc:'Alcanza el 75% de la meta mensual'},{id:'meta100',icon:'ðŸ†',name:'Â¡Meta!',desc:'Â¡Alcanzas la meta mensual!'},{id:'super',icon:'ðŸ’¥',name:'SÃºper Meta',desc:'Superas $40M en el mes'},{id:'v20',icon:'ðŸ“¦',name:'20 Ventas',desc:'20 ventas en el mes'},{id:'v50',icon:'ðŸ›µ',name:'50 Ventas',desc:'50 ventas en el mes'},{id:'v100',icon:'ðŸ’¯',name:'100 Ventas',desc:'100 ventas en el mes'},{id:'v150',icon:'ðŸš€',name:'150 Ventas',desc:'150 ventas en el mes'},{id:'gran_venta',icon:'ðŸ’µ',name:'Gran Venta',desc:'Una venta mayor a $500k'},{id:'multicanal',icon:'ðŸŒ',name:'Multicanal',desc:'Ventas en los 3 canales el mismo dÃ­a'},{id:'nivel5',icon:'â­',name:'Nivel 5',desc:'Alcanza el nivel Pro'},{id:'nivel8',icon:'ðŸŒŸ',name:'Nivel 8',desc:'Alcanza el nivel Leyenda'}];
const MISSIONS_LADDER=[5,10,20,30,40,50,65,80,100,120,150];
const SNACKS=[{id:'bonyourt',name:'Bonyourt',emoji:'ðŸ¥¤'},{id:'bimbo',name:'Bimbo',emoji:'ðŸž'},{id:'turron',name:'TurrÃ³n',emoji:'ðŸ¬'},{id:'refrigerio',name:'Refrigerio',emoji:'ðŸ§ƒ'},{id:'paleta',name:'Paleta',emoji:'ðŸ«'},{id:'chocono',name:'Chocono',emoji:'ðŸ¦'},{id:'galleta',name:'Galletas',emoji:'ðŸª'},{id:'churro',name:'Churro',emoji:'ðŸ¥'},{id:'panpizza',name:'Pan Pizza',emoji:'ðŸ•'},{id:'empanada',name:'Empanada',emoji:'ðŸ¥™'},{id:'bunuelo',name:'BuÃ±uelo',emoji:'ðŸ«“'},{id:'pollo',name:'Pollo',emoji:'ðŸ—'},{id:'chocoramo',name:'Chocoramo',emoji:'ðŸ«'},{id:'doritos',name:'Doritos',emoji:'ðŸŒ®'}];
const SNACK_XP_GOAL=100;
const REWARDS=[{id:'meta_mes',icon:'ðŸ½ï¸',name:'Almuerzo con el Jefe',desc:'Alcanza los $34M de meta mensual',condition:s=>ventasMes(s).totalDespachos>0&&ventasMes(s).totalCOP>=s.meta},{id:'dia_millon',icon:'ðŸ’µ',name:'BonificaciÃ³n $100k',desc:'Un dÃ­a con $1M+ en despachos',condition:s=>hasDiaUnMillon(s)},{id:'super_meta',icon:'ðŸ‘‘',name:'Segundo Almuerzo',desc:'Supera los $40M en el mes',condition:s=>ventasMes(s).totalCOP>=40000000}];
...
function formatDate(d){if(!d)return 'â€”';const[y,m,day]=d.split('-');return `${day}/${m}/${y}`}
function normalizeVisibleFlag(v){
  if(v === false || v === 0 || v === '0' || v === 'false' || v === 'FALSE' || v === 'False' || v === null || v === undefined || v === '') return false;
  return true;
}
...
function getArticuloStock(artId, bodegaId) {
  ...
  // Si se pide por bodega especÃ­fica, usa inv_movimientos como antes
  if(bodegaId) {
...
async function loadState() {
  ...
  try {
    ...
    // Productos + imÃ¡genes + tallas + colores
    ...
    state.articulos = (products||[]).map(p=>{
      ...
      return {id:p.id,codigo:p.ref||'',ref:p.ref||'',nombre:p.name||'',name:p.name||'',
        ...
        activo:p.active!==false,mostrarEnWeb:normalizeVisibleFlag(p.visible),supabaseId:p.id,
    ...
    // Cargar clientes con paginaciÃ³n (pueden ser miles)
    ...
    // usu_empleados es el mismo array â€” renderUsuEmpleados lo usa
    ...
    renderAll();
    if(isFirstLoad){isFirstLoad=false;notify('success','â˜ï¸','Â¡Conectado!',`${(products||[]).length} artÃ­culos Â· ${(ventas||[]).length} ventas.`,{duration:3000});}
    ...
    _sbConnected = false;
    ['fb-status-dot','fb-status-dot-mobile'].forEach(id=>{const el=document.getElementById(id);if(el){el.style.background='#f87171';el.title='Sin conexiÃ³n: '+errMsg;}});
    showLoadingOverlay('hide');
    renderAll();
    notify('warning','ðŸ“¡','Error BD',errMsg,{duration:8000});
  }
}
...
const COLLECTION_MAP = {
  ...
  'facturas':        { table:'invoices', mapFn:(d)=>({
    ...
    items:JSON.stringify((d.items||[]).map(i=>({id:i.articuloId||'',nombre:i.nombre||'',talla:i.talla||'',qty:i.qty||i.cantidad||1,precio:i.precio||i.price||0})))
  }) },
...
function checkMonthReset(){const now=new Date();const ym=now.getFullYear()+'-'+now.getMonth();if(state.currentMonth&&state.currentMonth!==ym){state.ventas=state.ventas.map(v=>({...v,archived:true}));notify('milestone','ðŸŒ™','Â¡Nuevo Mes!','Ventas archivadas.',{duration:6000})}state.currentMonth=ym}
...
function showLoadingOverlay(st){
  ...
  if(st==='connecting')ov.innerHTML='<div style="font-size:52px;animation:fbspin 1.2s linear infinite">â˜ï¸</div><div style="font-family:Syne;font-size:18px;font-weight:800;color:#00e5b4;margin-top:18px">Conectando...</div><div style="margin-top:20px;width:200px;height:4px;background:rgba(255,255,255,.1);border-radius:99px;overflow:hidden"><div style="height:100%;width:40%;background:#00e5b4;border-radius:99px;animation:fbslide 1.4s ease-in-out infinite"></div></div>';
  if(st==='error')ov.innerHTML='<div style="font-size:52px">ðŸ“¡</div><div style="font-family:Syne;font-size:20px;font-weight:800;color:#f87171;margin-top:16px">Sin conexiÃ³n</div><button onclick="location.reload()" style="margin-top:24px;background:linear-gradient(135deg,#00e5b4,#00c4ff);color:#0a0f1e;border:none;border-radius:12px;padding:14px 32px;font-family:Syne;font-size:15px;font-weight:800;cursor:pointer">ðŸ”„ Reintentar</button>';
}
...
function openModal(html,isLg){
  ...
  if(mc && !mc.querySelector('.modal-close')) {
    ...
    x.className = 'modal-close'; x.textContent = 'Ã—'; x.title = 'Cerrar (ESC)';
    x.onclick = closeModal;
    mc.insertBefore(x, mc.firstChild);
  }
}
function closeModal(){document.getElementById('modal-overlay').classList.remove('active')}
// Click fuera del modal NO cierra (solo ESC o botÃ³n Ã—)
...
function notify(type,icon,title,body,opts={}){
  ...
  toast.innerHTML=`<div class="notif-icon">${icon}</div><div style="flex:1"><div class="notif-title">${title}</div><div class="notif-body">${body}</div></div><div class="notif-close" onclick="this.parentElement.remove()">Ã—</div>`;
...
function awardXP(xp){
  ...
  if(newLv>oldLv){const lv=calcLevel(state.game.xp);showCelebration(lv.avatar,'Â¡SUBISTE DE NIVEL!','Â¡Ahora eres '+lv.name+'!');screenFlash('green');if(newLv>=5)checkAndAwardBadge('nivel5');if(newLv>=8)checkAndAwardBadge('nivel8')}
...
function checkAndAwardBadge(id){if(!state.game.earnedBadges)state.game.earnedBadges=[];if(state.game.earnedBadges.includes(id))return;const badge=BADGES.find(b=>b.id===id);if(!badge)return;state.game.earnedBadges.push(id);notify('milestone',badge.icon,'Â¡Insignia: '+badge.name+'!',badge.desc,{duration:5000})}
...
function syncPOSFormState() {
  ...
  // Sincroniza precios editados manualmente (aunque el input no haya perdido foco)
    ...
    <div class="pos-layout">
      <div class="pos-products">
        <div class="search-bar">
          ...
          <input type="text" id="pos-search" placeholder="Buscar artÃ­culo o escanear..." oninput="filterPOSProducts()" onkeydown="handlePOSScan(event)">
          <button class="btn btn-sm btn-secondary scanner-btn" onclick="openScannerOverlay()" title="Escanear cÃ³digo de barras">ðŸ“¡</button>
        </div>
        ...
        ${articulos.length === 0 ? `
          <div class="empty-state">
            ...
            <div class="es-title">Sin artÃ­culos</div>
            <div class="es-text">Agrega artÃ­culos en Inventario â†’ ArtÃ­culos para empezar a vender.</div>
            <button class="btn btn-primary" style="margin-top:16px" onclick="showPage('articulos')">Ir a ArtÃ­culos</button>
          </div>` : ''}
      </div>
      ...
      <div class="pos-cart">
        ...
        <div class="pos-cart-items" id="pos-cart-items">
          ${cart.length === 0 ? 
            '<div style="text-align:center;padding:40px 16px;color:var(--text2)"><div style="font-size:40px;margin-bottom:8px">ðŸ›’</div><div style="font-size:12px">Agrega productos al carrito</div></div>' : ''}
          ...
          ${cart.map((item, i) => `
            <div class="pos-item">
              <div style="flex:1">
                ...
                <div style="display:flex; align-items:center; gap:4px; margin-top:4px;">
                  ...
                  <input type="number" 
                         ...
                         data-pos-price-idx="${i}"
                         value="${item.precio}" 
                         oninput="updatePOSCartPrice(${i}, this.value)"
              ...
              <div class="pos-item-qty">
                <button onclick="posCartQty(${i},-1)">âˆ’</button>
                <span>${item.qty}</span>
              ...
              <button class="btn-icon btn-danger" style="font-size:10px; width:24px; height:24px" onclick="posCartRemove(${i})">âœ•</button>
            </div>`).join('')}
        </div>
        ...
        <div class="pos-cart-footer">
          ...
          <div class="form-group" style="margin-bottom:10px">
            <select class="form-control" id="pos-canal" onchange="handlePOSShippingUI()" style="padding:8px 12px; background:var(--bg3)">
              <option value="vitrina" ${posFormState.canal === 'vitrina' ? 'selected' : ''}>ðŸª Venta Vitrina</option>
              <option value="local" ${posFormState.canal === 'local' ? 'selected' : ''}>ðŸ›µ MensajerÃ­a Local</option>
              <option value="inter" ${posFormState.canal === 'inter' ? 'selected' : ''}>ðŸ“¦ Intermunicipal</option>
            </select>
          </div>
          ...
          <div id="pos-shipping-fields" style="display:${posFormState.canal !== 'vitrina' ? 'flex' : 'none'}; flex-direction:column; gap:8px; margin-bottom:12px; background:var(--bg3); padding:10px; border-radius:10px; border:1px solid var(--border)">
             ...
             <select class="form-control" id="pos-transportadora" style="display:none">
                <option value="">â€” Transportadora â€”</option>
                ...
                <option value="EnvÃ­a" ${posFormState.transportadora === 'EnvÃ­a' ? 'selected' : ''}>EnvÃ­a</option>
                <option value="Interrapidisimo" ${posFormState.transportadora === 'Interrapidisimo' ? 'selected' : ''}>Inter RapidÃ­simo</option>
                <option value="Servientrega" ${posFormState.transportadora === 'Servientrega' ? 'selected' : ''}>Servientrega</option>
             </select>
             <input type="text" class="form-control" id="pos-guia" placeholder="NÃºmero de GuÃ­a" value="${posFormState.guia}">
             <input type="text" class="form-control" id="pos-ciudad" placeholder="Ciudad / DirecciÃ³n" value="${posFormState.ciudad}">
             ...
             <div style="border-top:1px solid var(--border);padding-top:8px;margin-top:2px">
               ...
               <select class="form-control" id="pos-tipo-pago" onchange="syncPOSFormState();renderPOS()" style="padding:8px">
                 <option value="contado" ${posFormState.tipoPago==='contado'?'selected':''}>ðŸ’µ Pago de Contado</option>
                 <option value="contraentrega" ${posFormState.tipoPago==='contraentrega'?'selected':''}>ðŸ“¦ Contra Entrega</option>
               </select>
               ${posFormState.tipoPago==='contraentrega' ? `<div style="font-size:10px;color:var(--yellow);margin-top:4px">âš ï¸ Se registrarÃ¡ en Cobros Pendientes hasta recibir el pago.</div>` : `<div style="font-size:10px;color:var(--green);margin-top:4px">âœ… Pago inmediato â€” solo se registra en facturas.</div>`}
             </div>
          </div>
${posFormState.metodo === 'efectivo' ? (() => {
              ...
              <div style="background:var(--bg3); padding:12px; border-radius:10px; border:1px dashed var(--border); margin-bottom:12px;">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
                  <label class="form-label" style="margin:0; font-size:11px;">ðŸ’µ EFECTIVO RECIBIDO</label>
          ...
          <div class="form-group" style="margin-bottom:10px; display:flex; gap:8px;">
            <select class="form-control" id="pos-metodo-pago" style="padding:8px 12px" onchange="renderPOS()">
              ${(state.cfg_metodos_pago && state.cfg_metodos_pago.filter(m=>m.activo!==false).length > 0
                ...
                : [{id:'efectivo',nombre:'ðŸ’µ Efectivo'},{id:'tarjeta',nombre:'ðŸ’³ Tarjeta'},{id:'transferencia',nombre:'ðŸ“± Transferencia'},{id:'addi',nombre:'ðŸ’œ Addi'},{id:'mixto',nombre:'ðŸ”„ Mixto'}]
          ...
          <div class="form-row" style="gap:8px; margin-bottom:8px">
            <input type="text" class="form-control" id="pos-cedula" placeholder="ðŸ” CÃ©dula o Celular (Buscar)" style="padding:8px 12px; font-weight:bold; color:var(--accent)" onblur="autocompletarCliente(this)">
          </div>
          <div class="form-row" style="gap:8px; margin-bottom:10px">
            ...
            <input type="tel" class="form-control" id="pos-telefono" placeholder="TelÃ©fono (opcional)" style="padding:8px 12px" value="${posFormState.telefono}">
          </div>
...
<div class="form-group" style="margin-top: 15px; margin-bottom: 15px; background: #f8f9fa; padding: 10px; border-radius: 8px; border: 1px dashed var(--accent);">
  <label style="cursor: pointer; display: flex; align-items: center; gap: 10px; font-weight: bold; color: var(--accent);">
    ...
    ðŸ›ï¸ ES UN SEPARADO (Stand By en el local)
  </label>
</div>

          <button class="btn btn-primary" style="width:100%" onclick="procesarVentaPOS()" ${cart.length === 0 ? 'disabled' : ''}>ðŸ’° Cobrar ${fmt(total)}</button>
          <div style="display:flex;gap:8px;margin-top:8px">
            <button class="btn btn-secondary btn-sm" style="flex:1" onclick="previewReceipt()">ðŸ§¾ Vista Previa</button>
            <button class="btn btn-secondary btn-sm" style="flex:1" onclick="openCashDrawer()">ðŸ§ Abrir CajÃ³n</button>
          </div>
        </div>
      </div>
    </div>`;
...
// Actualiza el precio de un Ã­tem especÃ­fico en el carrito y repinta
...
function handlePOSShippingUI() {
  ...
  if(canal === 'vitrina') {
    ...
    if(canal === 'local') {
      empresaSel.innerHTML = `<option value="">â€” MensajerÃ­a Local â€”</option><option value="MensLocal" ${posFormState.empresa==='MensLocal'?'selected':''}>MensajerÃ­a Propia</option><option value="Rappi" ${posFormState.empresa==='Rappi'?'selected':''}>Rappi</option><option value="Picap" ${posFormState.empresa==='Picap'?'selected':''}>Picap</option>`;
      transSel.style.display = 'none';
      liqInfo.textContent = 'âš¡ LiquidaciÃ³n al dÃ­a siguiente hÃ¡bil.';
    } else if(canal === 'inter') {
      empresaSel.innerHTML = `<option value="">â€” Plataforma / Directo â€”</option><option value="HEKA" ${posFormState.empresa==='HEKA'?'selected':''}>HEKA</option><option value="DROPI" ${posFormState.empresa==='DROPI'?'selected':''}>Dropi</option><option value="Directo" ${posFormState.empresa==='Directo'?'selected':''}>Directo / Otra</option>`;
      transSel.style.display = 'block';
      liqInfo.textContent = `ðŸ“¦ LiquidaciÃ³n en ${state.diasInter} dÃ­as hÃ¡biles.`;
    }
  }
}
...
const videoIcon = (a.video || esVideo) ? `<div style="position:absolute;top:8px;right:8px;background:rgba(0,0,0,0.5);border-radius:50%;padding:4px;font-size:12px;z-index:2;">â–¶ï¸</div>` : '';
  ...
  ${videoEl}
  ${videoIcon}
  ${!a.imagen && !esVideo ? `<div class="p-emoji">${a.emoji||'ðŸ‘™'}</div>` : ''}
  ...
  <div class="p-stock" style="position:relative;z-index:2;${(a.imagen||esVideo)?'color:#ddd;':''}">${out?'âŒ Agotado':stock+' en stock'+(low?' âš ï¸':'')}</div>
  ${a.codigo&&!a.imagen&&!esVideo?'<div style="font-size:9px;color:var(--meta);margin-top:2px">'+a.codigo+'</div>':''}
  
    </div>`}).join('')||'<div style="grid-column:1/-1;text-align:center;color:var(--text2);padding:24px">No se encontraron artÃ­culos</div>';
}

function promptTallaYAgregar(artId){
  const art=(state.articulos||[]).find(a=>a.id===artId);if(!art)return;
  const tallas = art.tallas ? art.tallas.split(',') : ['XS','S','M','L','XL','Ãšnica'];

  openModal(`
    <div class="modal-title">Seleccionar Talla - ${art.nombre}<button class="modal-close" onclick="closeModal()">Ã—</button></div>
...
function addToCart(artId, talla = 'Ãšnica'){
  syncPOSFormState(); 
  const art=(state.articulos||[]).find(a=>a.id===artId);if(!art)return;
  ...
  if(currentQty>=stock){notify('warning','âš ï¸','Sin stock','No hay suficiente inventario.',{duration:3000});return}
...
async function procesarVentaPOS() {
  ...
  // Contado: ya estÃ¡ liquidado. ContraEntrega: pendiente hasta cobro.
  const liquidadoInicial = canal === 'vitrina' || tipoPago === 'contado';
  // Fecha de liquidaciÃ³n segÃºn canal
  ...
  const persistedOK = facturaSaved && ventaSaved;
  if(!persistedOK){
    notify('warning','ðŸ“¡','Sin sincronizaciÃ³n BD','La venta se guardÃ³ localmente, pero no se pudo sincronizar factura/venta a la base de datos.',{duration:6000});
  }

  // Actualizar stock en BD si hay conexiÃ³n
  if(_sbConnected){
    try {
      for(const item of cart){
        ...
        if(art){
          ...
          await supabaseClient.from('products').update({stock:ns}).eq('id',item.articuloId);
          art.stock=ns;
        }
      }
    } catch(e){ console.warn('Supabase POS stock update failed:', e.message); }
  }
  ...
  notify('sale','âœ…','Â¡Venta registrada!',`${numFactura} Â· ${fmt(total)} Â· +${xpGained}XP`,{duration:4000});
  spawnConfetti();
  screenFlash('green');
  renderPOS();
  updateNavBadges();
  ...
  inp.onkeydown=function(e){
    if(e.key==='Enter'){
      ...
      if(code){
        ...
        else notify('warning','âš ï¸','No encontrado','CÃ³digo: '+code,{duration:3000});
      }
    }
  };
}
...
function printReceipt(factura) {
  ...
  <div class="center small">NIT: ${emp.nit||''} | ${emp.regimenFiscal||'RÃ©gimen ordinario No responsable de IVA'}</div>
  <div class="center small">${emp.departamento||''} / ${emp.ciudad||''} / ${emp.direccion||''}</div>
  <div class="center small">TelÃ©fonos: ${emp.telefono||''}${emp.telefono2?' / '+emp.telefono2:''}</div>
  ${emp.email?`<div class="center small">Email: ${emp.email}</div>`:''}
  ${emp.web?`<div class="center small">PÃ¡gina web: ${emp.web}</div>`:''}
  ...
  ${vendedora?`<div class="small">ElaborÃ³: ${vendedora}</div>`:''}
  ${bodega?`<div class="small">Vendedor: ${vendedora||''} | Bodega: ${bodega}</div>`:''}

  <div class="line"></div>
  <table>
    <thead><tr><th>DESCRIPCIÃ“N</th><th>CANT</th><th>TOTAL</th></tr></thead>
  ...
  if(!pWin) { notify('warning','âš ï¸','Popup bloqueado','Permite popups para imprimir.',{duration:4000}); return; }
  pWin.document.write(receiptHTML);
  pWin.document.close();
  setTimeout(() => { pWin.print(); }, 600);
}
function previewReceipt(){
  const cart=state.pos_cart||[];if(cart.length===0){notify('warning','âš ï¸','Carrito vacÃ­o','Agrega productos primero.',{duration:3000});return}
...
function openCashDrawer(){
  try{
    ...
    notify('success','ðŸ§','CajÃ³n','Comando enviado al cajÃ³n POS.',{duration:2000});
  }catch(e){notify('warning','âš ï¸','CajÃ³n','No se pudo abrir el cajÃ³n.',{duration:3000})}
}
...
function renderArticulos(){
  ...
  document.getElementById('articulos-content').innerHTML=`
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;flex-wrap:wrap;gap:8px">
      <div class="search-bar" style="flex:1;max-width:400px;margin:0"><span class="search-icon">ðŸ”</span><input type="text" id="art-search" placeholder="Buscar artÃ­culo..." oninput="renderArticulosList()"></div>
      <div class="btn-group">
        <button class="btn btn-primary" onclick="openArticuloModal()">+ Nueva Prenda</button>
        <button class="btn btn-secondary btn-sm" onclick="importarDesdeCatalogo()" style="border-color:rgba(0,229,180,.3);color:var(--accent);">â¬‡ï¸ Importar</button>
        <button class="btn btn-secondary btn-sm" onclick="showPage('config');setCfgTab('inventario')" title="Configurar categorÃ­as">âš™ï¸ CategorÃ­as</button>
      </div>
    </div>
    <div class="card"><div class="card-title">CATÃLOGO DE PRENDAS (${items.length})</div>
      <div class="table-wrap"><table><thead><tr><th>Foto</th><th>Referencia</th><th>Nombre</th><th>CategorÃ­a</th><th>TÃ­tulo</th><th>Proveedor</th><th>P.Compra</th><th>P.Venta</th><th>Stock</th><th>Min</th><th></th></tr></thead><tbody id="art-tbody"></tbody></table></div>
    </div>`;
  renderArticulosList();
}
...
function renderArticulosList(){
  ...
  artTbody.innerHTML=items.map(a=>{
    ...
    const thumb = a.imagen ? `<div style="width:36px;height:36px;border-radius:8px;background:url('${a.imagen}') center/cover;border:1px solid var(--border)"></div>` : `<div style="font-size:24px">ðŸ‘™</div>`;
   const tituloLabel = {propia:'ðŸ·ï¸ Propia',contado:'ðŸ’µ Contado',credito:'ðŸ’³ CrÃ©dito'};
   return `<tr>
        <td style="width:50px">${thumb}</td>
        <td>${a.codigo || 'â€”'}</td>
        <td style="font-weight:700">${a.nombre}</td>
        <td><span class="badge badge-info">${a.categoria || 'â€”'}</span></td>
        <td>${a.tituloMercancia ? `<span class="badge badge-warn">${tituloLabel[a.tituloMercancia]||a.tituloMercancia}</span>` : 'â€”'}</td>
        <td style="font-size:11px;color:var(--text2)">${a.proveedorNombre||'â€”'}</td>
        ...
        <td>
          <div class="btn-group">
            <button class="btn btn-xs btn-secondary" onclick="generateProductQR('${a.id}')" title="Generar QR">ðŸ“± QR</button>
            <button class="btn btn-xs btn-secondary" onclick="openArticuloModal('${a.id}')" title="Editar">âœï¸</button>
            <button class="btn btn-xs btn-danger" onclick="deleteArticulo('${a.id}')" title="Eliminar">âœ•</button>
          </div>
        </td>
      </tr>`;
    }).join('');
...
async function deleteArticulo(id){
  ...
  if(!confirm(`Â¿Eliminar "${art.nombre}"? Esta acciÃ³n no se puede deshacer.`)) return;
  ...
  try {
    ...
    showLoadingOverlay('hide');
    notify('success', 'ðŸ—‘ï¸', 'ArtÃ­culo eliminado', art.nombre, { duration: 2500 });
  } catch (err) {
    showLoadingOverlay('hide');
    console.error('deleteArticulo error:', err);
    notify('danger', 'âš ï¸', 'Error eliminando artÃ­culo', err.message || 'Revisa consola', { duration: 5000 });
  }
}
  
// ===================================================================
// ===== MAQUETADOR PRO (MODO CATÃLOGO + ERP INTEGRADO) =====
// ===================================================================

function openArticuloModal(id){
    const art = id ? (state.articulos || []).find(a => a.id === id) : null;
    // Cargar imÃ¡genes existentes del artÃ­culo en la galerÃ­a temporal
    ...
    // Flag para saber si es ediciÃ³n (preservar imÃ¡genes si no se tocan)
    window._editingArticuloId = id || null; 
    
    openModal(`
        <div class="modal-title" style="font-family:'Syne'; letter-spacing:1px;">ðŸš€ MAQUETADOR DE PRENDA PROFESIONAL</div>
        <div style="max-height: 75vh; overflow-y: auto; padding-right: 10px; text-align:left;">
            ...
            <div style="background:rgba(255,255,255,0.03); padding:20px; border-radius:12px; border:1px solid var(--border); margin-bottom:20px;">
                <label class="form-label">ðŸ“¸ GALERÃA MULTIMEDIA</label>
                <div style="background:var(--bg); border:1px dashed var(--accent); padding:20px; text-align:center; border-radius:8px; position:relative; cursor:pointer;">
                    <span style="font-size:20px;">ðŸ“¤ Subir Fotos / Videos</span><br>
                    <span style="font-size:10px; opacity:0.6;">Toca la â­ para elegir la foto de portada.</span>
            ...
            <div class="form-row">
                ...
                <div class="form-group"><label class="form-label">COLECCIÃ“N / TEMPORADA</label><input class="form-control" id="m-art-coleccion" value="${art?.coleccion || ''}" placeholder="Ej: Verano 2026"></div>
            </div>

            <div class="form-row">
                <div class="form-group">
                    <label class="form-label">SECCIÃ“N WEB</label>
                    <select class="form-control" id="m-art-seccion" onchange="actualizarCatsERP()">
                        ${(state.cfg_secciones && state.cfg_secciones.length > 0
                          ? state.cfg_secciones
                          : [{nombre:'Trajes de BaÃ±o'},{nombre:'Resort & Pijamas'},{nombre:'Activewear'},{nombre:'Casual'}]
                        ).map(s => `<option value="${s.nombre}" ${art?.seccion === s.nombre ? 'selected' : ''}>${s.nombre}</option>`).join('')}
                    </select>
                </div>
                <div class="form-group"><label class="form-label">CATEGORÃA</label><select class="form-control" id="m-art-cat"></select></div>
            </div>
            ...
            <div class="form-group"><label class="form-label">DESCRIPCIÃ“N (Para el CatÃ¡logo)</label><textarea class="form-control" id="m-art-desc" rows="2">${art?.descripcion || ''}</textarea></div>
            ...
            <div class="form-group"><label class="form-label">TÃTULO DE MERCANCÃA</label>
                    <select class="form-control" id="m-art-titulo-mercancia">
                        <option value="" ${!art?.tituloMercancia ? 'selected' : ''}>â€” Seleccionar â€”</option>
                        <option value="propia" ${art?.tituloMercancia === 'propia' ? 'selected' : ''}>ðŸ·ï¸ MercancÃ­a Propia</option>
                        <option value="contado" ${art?.tituloMercancia === 'contado' ? 'selected' : ''}>ðŸ’µ MercancÃ­a de Contado</option>
                        <option value="credito" ${art?.tituloMercancia === 'credito' ? 'selected' : ''}>ðŸ’³ MercancÃ­a a CrÃ©dito</option>
                    </select>
                </div>

            <div class="card-title" style="margin-top:10px; border-top:1px solid var(--border); padding-top:15px; color:var(--accent);">ðŸ’° INVENTARIO Y PRECIOS</div>
            ...
            <div class="form-group"><label class="form-label">ðŸ­ PROVEEDOR</label>
                <select class="form-control" id="m-art-proveedor">
                    <option value="">â€” Sin proveedor â€”</option>
                    ${(state.usu_proveedores||[]).map(p => `<option value="${p.id}" ${art?.proveedorId === p.id ? 'selected' : ''}>${p.nombre}${p.cedula ? ' Â· ' + p.cedula : ''}</option>`).join('')}
                </select>
                ${(state.usu_proveedores||[]).length === 0 ? '<span style="font-size:10px;color:var(--text2)">Sin proveedores. <a onclick="closeModal();showPage(\'usu_proveedores\')" style="color:var(--accent);cursor:pointer">â†’ Crear proveedor</a></span>' : ''}
            </div>
  ...
  <label style="display: flex; align-items: center; gap: 10px; cursor: pointer; color: var(--text1); font-weight: bold;">
    ...
    ðŸŒ Mostrar esta prenda en el CatÃ¡logo Web (Supabase)
  </label>
</div>
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                  ...
                  ${art ? '<div style="font-size:10px;color:var(--text2);margin-top:3px">Para ajustar el stock usa: Inventario â†’ Ajustes</div>' : ''}
                </div>
            </div>
        </div>
        <button class="btn btn-primary" style="width:100%; margin-top:15px; font-weight:800;" onclick="saveArticulo('${id || ''}')">ðŸ’¾ GUARDAR Y ACTUALIZAR WEB</button>
    `, true);
  setTimeout(() => { document.getElementById('art-mostrar-web').checked = art ? normalizeVisibleFlag(art.mostrarEnWeb) : true; }, 10);
    ...
    container.innerHTML = _tempGaleria.map((url, idx) => {
        ...
        return `
            <div style="position:relative; width:72px; height:96px; border-radius:6px; overflow:hidden; border:2px solid ${idx === _portadaIndex ? 'var(--accent)' : 'transparent'}; cursor:pointer;" onclick="_portadaIndex=${idx}; window._galeriaModificada=true; renderGaleriaVisual();">
                ${media}
                <button class="btn-danger" style="position:absolute; top:2px; right:2px; width:18px; height:18px; border-radius:50%; border:none; font-size:9px;" onclick="event.stopPropagation(); _tempGaleria.splice(${idx},1); window._galeriaModificada=true; if(_portadaIndex>=${idx})_portadaIndex=0; renderGaleriaVisual();">âœ•</button>
                <div style="position:absolute; bottom:2px; left:2px; background:${idx === _portadaIndex ? 'var(--accent)' : 'rgba(0,0,0,0.5)'}; color:#000; border-radius:50%; width:18px; height:18px; display:flex; align-items:center; justify-content:center; font-size:10px;">â­</div>
            </div>`;
    }).join('');
}
...
function actualizarCatsERP(selectedCat){
    ...
    // â˜… Usar categorÃ­as del ERP (cfg_categorias) si estÃ¡n disponibles
    ...
    if(cfgCats.length > 0) {
      ...
      // Fallback hardcoded
      if(sec === 'Trajes de BaÃ±o') opciones = ['Enterizos','Bikinis','Tankinis','Asoleadores','Salidas de BaÃ±o','3 Piezas'];
...
async function uploadGalleryImages(input) {
  ...
  try {
    ...
    renderGaleriaVisual();
    showLoadingOverlay('hide');
    notify('success', 'ðŸ“¸', 'Completado', `Subidos ${added} archivos.`);

  } catch (e) {
    showLoadingOverlay('hide');
    notify('danger', 'âš ï¸', 'Error', e.message);
  }
}
async function saveArticulo(existingId) {
    ...
    const productData = {
        ...
        active: true,
        visible: normalizeVisibleFlag(document.getElementById('art-mostrar-web').checked),
    ...
    try {
        ...
        if(stockInicial > 0) {
          ...
          try {
            await supabaseClient.from('inv_ajustes').insert({
              ...
              motivo: existingId ? 'Ajuste de stock' : 'Stock inicial al crear artÃ­culo',
              fecha: today()
            });
        ...
        // 2. MANEJAR IMÃGENES en product_media - solo si hubo cambios reales
        if(window._galeriaModificada || !existingId) {
          // Obtener imÃ¡genes actuales en BD
          ...
          // Eliminar las que ya no estÃ¡n en _tempGaleria
          ...
          // Insertar las nuevas (que no existÃ­an antes)
          ...
          // Actualizar is_cover si cambiÃ³ la portada
        ...
        renderArticulos();
        showLoadingOverlay('hide');
        notify('success', 'âœ…', 'Guardado', `${refID} guardado con tallas y colores.`);
...
// ===================================================================
// ===== GENERIC DOCUMENT RENDERER (Cotizaciones, Ã“rdenes, etc) =====
// ===================================================================
function getDocItemQty(i){
  return parseFloat(i?.cantidad ?? i?.qty ?? 1) || 1;
}
function getDocItemPrice(i){
  return parseFloat(i?.precio ?? i?.price ?? 0) || 0;
}
function getFacturaAuditStats(doc){
  const items = doc?.items || [];
  let changed = 0;
  let comparable = 0;
  items.forEach(i=>{
    const artId = i?.articuloId || i?.id;
    const art = (state.articulos||[]).find(a=>a.id===artId);
    if(!art) return;
    comparable++;
    const base = parseFloat(art.precioVenta ?? art.price ?? 0) || 0;
    const val = getDocItemPrice(i);
    if(Math.abs(val-base) > 0.009) changed++;
  });
  return { changed, comparable, total: items.length };
}
function renderDocumentList(pageId,title,collection,tipo,fields){
  ...
  const rowsHtml = items.map(d=>`<tr>
    ...
    ${showAudit?(()=>{const st=getFacturaAuditStats(d);return `<td><span class="badge ${st.changed>0?'badge-warn':'badge-ok'}">${st.changed>0?`${st.changed} manual`:'sin cambio'}</span></td>`;})():''}
    <td><div class="btn-group">
      <button class="btn btn-xs btn-secondary" onclick="viewDoc('${collection}','${d.id}')">ðŸ‘</button>
      <button class="btn btn-xs btn-secondary" onclick="printDoc('${collection}','${d.id}')">ðŸ–¨</button>
      <button class="btn btn-xs btn-danger" onclick="deleteDoc('${collection}','${d.id}')">âœ•</button>
    </div></td>
  </tr>`).join('')||`<tr><td colspan="${showAudit?7:6}" style="text-align:center;color:var(--text2);padding:24px">Sin registros</td></tr>`;
    ...
    <div style="display:flex;gap:10px;align-items:flex-end;flex-wrap:wrap;margin-bottom:16px;">
      ...
      <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap;">
        ...
        <button class="btn btn-xs btn-secondary" id="${pageId}-doc-limpiar" style="display:${(q||desde||hasta)?'inline-flex':'none'}"
          onclick="document.getElementById('${pageId}-search').value='';document.getElementById('${pageId}-desde').value='';document.getElementById('${pageId}-hasta').value='';renderDocumentList('${pageId}','${title}','${collection}','${tipo}')">âœ• Limpiar</button>
      </div>
    </div>
    <div class="card">
      <div class="card-title">${title.toUpperCase()} â€” <span id="${contId}">${items.length} de ${total}</span></div>
      <div class="table-wrap"><table>
        <thead><tr><th>#</th><th>Fecha</th><th>Cliente</th><th>Total</th><th>Estado</th>${showAudit?'<th>Auditoria Precio</th>':''}<th></th></tr></thead>
...
function openDocModal(collection,tipo,existingId){
  const tipos={cotizacion:'CotizaciÃ³n',orden:'Orden de Venta',factura:'Factura',nc:'Nota CrÃ©dito',nd:'Nota DÃ©bito',remision:'RemisiÃ³n',devolucion:'DevoluciÃ³n',anticipo_cliente:'Anticipo Cliente'};
  const label=tipos[tipo]||tipo;
  openModal(`
    <div class="modal-title">Nueva ${label}<button class="modal-close" onclick="closeModal()">Ã—</button></div>
    <div class="form-row">
    ...
    ${(tipo==='nc'||tipo==='nd'||tipo==='devolucion')?`<div class="form-group"><label class="form-label">FACTURA REFERENCIA</label><select class="form-control" id="m-doc-ref"><option value="">â€” Seleccionar â€”</option>${(state.facturas||[]).map(f=>'<option value="'+f.id+'">'+f.numero+' Â· '+fmt(f.total)+'</option>').join('')}</select></div>`:''}
    ...
    <div class="card-title" style="margin-top:16px">ÃTEMS</div>
    <div id="m-doc-items"></div>
    <button class="btn btn-sm btn-secondary" style="margin-bottom:16px" onclick="addDocItem()">+ Agregar Ãtem</button>
...
function renderDocItems(){
  ...
  el.innerHTML=_docItems.map((item,i)=>`
    <div style="display:grid;grid-template-columns:2fr 80px 120px 40px;gap:8px;margin-bottom:8px;align-items:end">
      <div class="form-group" style="margin:0"><label class="form-label">${i===0?'ARTÃCULO':''}</label><select class="form-control" onchange="docItemChanged(${i},this.value)" style="padding:8px"><option value="">â€” Seleccionar â€”</option>${(state.articulos||[]).map(a=>'<option value="'+a.id+'" '+(item.articuloId===a.id?'selected':'')+'>'+a.nombre+'</option>').join('')}<option value="custom">âœï¸ Personalizado</option></select></div>
      ...
      <button class="btn btn-xs btn-danger" onclick="removeDocItem(${i})" style="margin-bottom:0;height:38px">âœ•</button>
    </div>`).join('');
  updateDocTotal();
}
...
async function saveDoc(collection,tipo){
  ...
  if(items.length===0){notify('warning','âš ï¸','Sin Ã­tems','Agrega al menos un Ã­tem.',{duration:3000});return}
  ...
  notify('success','âœ…','Documento creado',`${numero} Â· ${fmt(total)}`,{duration:3000});
}
...
function viewDoc(collection,id){
  ...
  const showAudit = collection==='facturas';
  const audit = showAudit ? getFacturaAuditStats(doc) : null;
  openModal(`
    <div class="modal-title">${doc.numero}<button class="modal-close" onclick="closeModal()">Ã—</button></div>
    <div class="grid-2" style="margin-bottom:16px">
      <div><span style="color:var(--text2);font-size:12px">Fecha:</span> ${formatDate(doc.fecha)}</div>
      <div><span style="color:var(--text2);font-size:12px">Cliente:</span> ${doc.cliente||'â€”'}</div>
    </div>
    ${showAudit?`<div style="margin-bottom:10px"><span class="badge ${audit.changed>0?'badge-warn':'badge-ok'}">${audit.changed>0?`${audit.changed} item(s) con precio manual`:'Sin cambios manuales de precio'}</span></div>`:''}
    <div class="table-wrap" style="margin-bottom:16px"><table><thead><tr><th>ArtÃ­culo</th><th>Cant</th><th>Precio</th><th>Total</th></tr></thead><tbody>
    ${(doc.items||[]).map(i=>`<tr><td>${i.nombre||'â€”'}</td><td>${getDocItemQty(i)}</td><td>${fmt(getDocItemPrice(i))}</td><td style="font-weight:700;color:var(--accent)">${fmt(getDocItemQty(i)*getDocItemPrice(i))}</td></tr>`).join('')}
    </tbody></table></div>
    ...
    <div class="btn-group" style="margin-top:16px">
      <button class="btn btn-primary btn-sm" onclick="printDoc('${collection}','${id}')">ðŸ–¨ Imprimir</button>
      ${doc.estado!=='pagada'?`<button class="btn btn-sm" style="background:rgba(74,222,128,.15);color:var(--green);border:1px solid rgba(74,222,128,.3)" onclick="changeDocStatus('${collection}','${id}','pagada')">âœ“ Marcar Pagada</button>`:''}
      ${doc.estado!=='anulada'?`<button class="btn btn-sm btn-danger" onclick="changeDocStatus('${collection}','${id}','anulada')">âœ• Anular</button>`:''}
    </div>
  `);
}
...
function changeDocStatus(collection,id,newStatus){
  ...
  closeModal();
  renderPage(document.querySelector('.page.active')?.id.replace('page-',''));
  notify('success','âœ…','Estado actualizado',doc.numero+' â†’ '+newStatus,{duration:3000});
}

function deleteDoc(collection, id) {
  // --- CANDADO DE SEGURIDAD PARA FACTURAS ---
  if (collection === 'facturas') {
    alert('âš ï¸ Â¡Alto ahÃ­! Para mantener tu inventario y caja perfectamente cuadrados, las facturas solo se pueden anular desde la pestaÃ±a: SISTEMA > Historial.');
    return; // Detiene la acciÃ³n inmediatamente
  }
  // ------------------------------------------
...
function renderCotizaciones(){_docItems=[];renderDocumentList('cotizaciones','CotizaciÃ³n','cotizaciones','cotizacion')}
...
function renderNotasCredito(){_docItems=[];renderDocumentList('notas_credito','Nota CrÃ©dito','notas_credito','nc')}
function renderNotasDebito(){_docItems=[];renderDocumentList('notas_debito','Nota DÃ©bito','notas_debito','nd')}
function renderRemisiones(){_docItems=[];renderDocumentList('remisiones','RemisiÃ³n','remisiones','remision')}
function renderDevoluciones(){_docItems=[];renderDocumentList('devoluciones','DevoluciÃ³n','devoluciones','devolucion')}
...
function renderUsuarios(pageId, titulo, collection, tipo){
  ...
  el.innerHTML = `
    <div style="display:flex;gap:10px;align-items:flex-end;flex-wrap:wrap;margin-bottom:16px;">
      ...
      <button class="btn btn-secondary" onclick="importarUsuariosCSV('${collection}','${tipo}','${pageId}')" title="Importar CSV/Excel">ðŸ“¥ Importar</button>
      <button class="btn btn-secondary" onclick="exportarUsuarios('${collection}','${tipo}')" title="Exportar a CSV">â¬† Exportar</button>
      <button class="btn btn-secondary" onclick="descargarPlantilla('${tipo}')" title="Descargar plantilla">â¬‡ Plantilla</button>
      <div class="search-bar" style="flex:1;min-width:180px;max-width:300px;margin:0">
        <span class="search-icon">ðŸ”</span>
        <input type="text" id="${pageId}-search" placeholder="Nombre, cÃ©dula, ciudad..."
          value="${q}"
      ...
      <button class="btn btn-xs btn-secondary" id="${pageId}-limpiar" style="display:${(q||desde||hasta)?'inline-flex':'none'}"
        onclick="document.getElementById('${pageId}-search').value='';document.getElementById('${pageId}-desde').value='';document.getElementById('${pageId}-hasta').value='';renderUsuariosTabla('${pageId}','${titulo}','${collection}','${tipo}')">âœ• Limpiar</button>
    </div>

    <div class="card">
      <div class="card-title">ðŸ‘¥ ${titulo.toUpperCase()}S â€” <span id="${contadorId}">${items.length} de ${total}</span></div>
      <div class="table-wrap">
        <table>
          <thead><tr>
            <th>Nombre</th><th>IdentificaciÃ³n</th><th>Celular</th><th>WhatsApp</th>
...
function renderUsuariosRows(items, collection, tipo, pageId) {
  ...
  const more = items.length > 200 ? `<tr><td colspan="8" style="text-align:center;color:var(--text2);padding:12px;font-size:11px">... y ${items.length-200} mÃ¡s. Usa el buscador para filtrar.</td></tr>` : '';
  return visible.map((u,idx) => `<tr>
    <td style="font-weight:700">${u.nombre||'â€”'}</td>
    <td>${u.tipoId||''} ${u.cedula||'â€”'}</td>
    <td>${u.celular||'â€”'}</td>
    <td>${u.whatsapp||'â€”'}</td>
    <td>${u.email||'â€”'}</td>
    <td>${u.ciudad||'â€”'}</td>
    ...
    <td><div class="btn-group">
      <button class="btn btn-xs btn-secondary" onclick="openUsuarioModal('${collection}','${tipo}','${pageId}','${idx}',true)">âœï¸</button>
      <button class="btn btn-xs btn-danger" onclick="eliminarUsuario('${collection}','${u.id}','${pageId}','${tipo}','${tipo}')">âœ•</button>
    </div></td>
  </tr>`).join('') + more;
}
...
function openUsuarioModal(collection, tipo, pageId, idx, editar){
  ...
  openModal(`
    <div class="modal-title">${u?'Editar':'Nuevo'} ${titulos[tipo]||tipo}<button class="modal-close" onclick="closeModal()">Ã—</button></div>
    <div class="form-row">
      <div class="form-group"><label class="form-label">TIPO ID</label>
        <select class="form-control" id="usu-tipoid">
          <option value="CC" ${u?.tipoId==='CC'?'selected':''}>CC - CÃ©dula</option>
          <option value="NIT" ${u?.tipoId==='NIT'?'selected':''}>NIT</option>
          <option value="CE" ${u?.tipoId==='CE'?'selected':''}>CE - ExtranjerÃ­a</option>
          <option value="PA" ${u?.tipoId==='PA'?'selected':''}>PA - Pasaporte</option>
        </select>
      </div>
      <div class="form-group"><label class="form-label">NÂ° IDENTIFICACIÃ“N</label><input class="form-control" id="usu-cedula" value="${u?.cedula||''}"></div>
    </div>
    ...
    <div class="form-row">
      ...
      <div class="form-group"><label class="form-label">DIRECCIÃ“N</label><input class="form-control" id="usu-dir" value="${u?.direccion||''}"></div>
    </div>
    <div class="form-row">
      <div class="form-group"><label class="form-label">TIPO PERSONA</label>
        <select class="form-control" id="usu-tipopersona">
          ...
          <option value="JurÃ­dica" ${u?.tipoPersona==='JurÃ­dica'?'selected':''}>JurÃ­dica</option>
        </select>
      </div>
...
async function guardarUsuario(collection, tipo, pageId, existingId) {
  ...
  if(!nombre){ notify('danger','âš ï¸','Error','El nombre es obligatorio'); return; }

  // Determinar la tabla y preparar los datos segÃºn el tipo
  let table = '';
  let data = {};
  ...
  try {
    ...
    // Mantener sincronÃ­a entre state.empleados y state.usu_empleados
    ...
    renderPage(pageId);
    showLoadingOverlay('hide');
    notify('success','âœ…','Guardado',`${nombre} guardado correctamente en BD`,{duration:3000});
...
async function eliminarUsuario(collection, id, pageId, titulo, tipo) {
  if(!confirm(`Â¿Eliminar este ${titulo}? Esta acciÃ³n no se puede deshacer.`)) return;
  ...
  try {
    ...
    showLoadingOverlay('hide');
    notify('success', 'ðŸ—‘ï¸', 'Eliminado', `${titulo} borrado del sistema.`);
  } catch (err) {
    showLoadingOverlay('hide');
    notify('danger', 'âš ï¸', 'Error al eliminar', err.message, {duration: 5000});
  }
...
function descargarPlantilla(tipo){
  ...
  let csv = '';
  if(tipo === 'cliente'){
    const headers = 'ID EFFI Tipo de identificaciÃ³n,Tipo de identificaciÃ³n,NÃºmero de identificaciÃ³n,Nombre,TelÃ©fono 1,TelÃ©fono 2,Celular,WhatsApp,Facetime,Skype,Email,Web,Direcciones,PaÃ­s,Departamento,Ciudad,ID EFFI Ciudad,DirecciÃ³n,Fecha de nacimiento,GÃ©nero,Tipo de persona,RÃ©gimen tributario,Tipo de cliente,Tipo de marketing,Tarifa de precios,Actividad econÃ³mica CIIU,Forma de pago,Descuento,Cupo de crÃ©dito CXC,Moneda principal,Sucursal,Ruta logÃ­stica,Vendedor,Responsable asignado,Fecha Ãºltima venta,ObservaciÃ³n,Vigencia,Fecha de creaciÃ³n,Responsable de creaciÃ³n,Fecha de modificaciÃ³n,Responsable de modificaciÃ³n,Fecha de anulaciÃ³n,Responsable de anulaciÃ³n';
    const ejemplo = '2,CÃ©dula de ciudadanÃ­a,12345678,MarÃ­a GarcÃ­a LÃ³pez,3001234567,,3001234567,3001234567,,,maria@email.com,,,,Antioquia,MedellÃ­n,,Calle 10 # 5-20,1990-05-15,,FÃ­sica (natural),,,,,,,0,0,Peso Colombiano $ COP,,,,,,,Cliente de prueba,Vigente,' + new Date().toISOString().split("T")[0] + ',,,,';
    csv = headers + '\n' + ejemplo;
  } else if(tipo === 'empleado'){
    csv = 'Nombre,CÃ©dula,Celular,Email,Ciudad,Salario Base,Tipo Contrato\nJuan PÃ©rez,12345678,3001234567,juan@email.com,MedellÃ­n,1750000,indefinido';
  } else {
    csv = 'Tipo ID,CÃ©dula/NIT,Nombre,Celular,WhatsApp,Email,Ciudad,Departamento,DirecciÃ³n,Tipo Persona,ObservaciÃ³n\nNIT,900123456,Empresa XYZ,3001234567,,info@empresa.com,MedellÃ­n,Antioquia,Cra 1 #2-3,JurÃ­dica,';
  }
  const blob = new Blob(['\uFEFF'+csv], {type:'text/csv;charset=utf-8;'});
  const a = document.createElement('a');
  a.href = URL.createObjectURL(blob); a.download = `plantilla_${titulos[tipo]||tipo}.csv`; a.click();
  notify('success','â¬‡','Plantilla descargada','Completa y luego importa el archivo.',{duration:3000});
}

function exportarUsuarios(collection, tipo){
  const items = state[collection] || [];
  if(items.length === 0){ notify('warning','âš ï¸','Sin datos','No hay registros para exportar.',{duration:3000}); return; }

  const BOM = '\uFEFF';
  const headers = [
    'ID EFFI Tipo de identificaciÃ³n','Tipo de identificaciÃ³n','NÃºmero de identificaciÃ³n',
    'Nombre','TelÃ©fono 1','TelÃ©fono 2','Celular','WhatsApp','Facetime','Skype',
    'Email','Web','Direcciones','PaÃ­s','Departamento','Ciudad','ID EFFI Ciudad',
    'DirecciÃ³n','Fecha de nacimiento','GÃ©nero','Tipo de persona','RÃ©gimen tributario',
    'Tipo de cliente','Tipo de marketing','Tarifa de precios','Actividad econÃ³mica CIIU',
    'Forma de pago','Descuento','Cupo de crÃ©dito CXC','Moneda principal','Sucursal',
    'Ruta logÃ­stica','Vendedor','Responsable asignado','Fecha Ãºltima venta','ObservaciÃ³n',
    'Vigencia','Fecha de creaciÃ³n','Responsable de creaciÃ³n','Fecha de modificaciÃ³n',
    'Responsable de modificaciÃ³n','Fecha de anulaciÃ³n','Responsable de anulaciÃ³n'
  ];

  const q = (v) => `"${String(v||'').replace(/"/g,'""')}"`;

  const rows = items.map(u => [
    q('2'), q(u.tipoId==='NIT'?'NIT':'CÃ©dula de ciudadanÃ­a'),
    ...
    q(u.tipoPersona==='JurÃ­dica'?'JurÃ­dica':'FÃ­sica (natural)'),
    q(''), q('ComÃºn'), q(''), q('Tarifa normal | Mayorista'),
  ...
  a.click();
  notify('success','â¬†','ExportaciÃ³n exitosa',`${items.length} registros exportados.`,{duration:3000});
}
...
function procesarArchivoUsuarios(input, collection, tipo, pageId) {
  ...
  reader.onload = async function(e) {
    try {
      ...
      if(rows.length < 2) throw new Error("Archivo vacÃ­o o sin datos.");
      ...
      for(let i = 1; i < rows.length; i++) {
        ...
        if(isEFFI) {
          ...
          // 0=ID EFFI tipo, 1=Tipo ID texto, 2=NÃºmero ID, 3=Nombre
          // 4=Tel1, 5=Tel2, 6=Celular, 7=WhatsApp, 10=Email
          // 13=PaÃ­s, 14=Departamento, 15=Ciudad, 17=DirecciÃ³n
          // 18=Fecha nac, 19=GÃ©nero, 20=Tipo persona, 35=ObservaciÃ³n
          ...
          u = {
            ...
            fechaNac: clean(18),
            genero: clean(19),
            tipoPersona: (clean(20)||'').toLowerCase().includes('natural') || (clean(20)||'').toLowerCase().includes('fÃ­sica') ? 'Natural' : 'JurÃ­dica',
      ...
      showLoadingOverlay('hide');
      renderPage(pageId);
      notify('success','ðŸ“¥','ImportaciÃ³n exitosa',`${importados} importados Â· ${duplicados} duplicados omitidos`,{duration:4000});

    } catch(err) {
      showLoadingOverlay('hide');
      console.error('Import error:', err);
      notify('danger','âš ï¸','Error en importaciÃ³n', err.message, {duration:5000});
    }
  };
  ...
  try {
    ...
    renderPage(document.querySelector('.page.active')?.id.replace('page-',''));
    showLoadingOverlay('hide');
    notify('success','âœ…','Documento creado',`${numero} guardado en BD.`,{duration:3000});

  } catch (err) {
    showLoadingOverlay('hide');
    notify('danger','âš ï¸','Error al crear documento', err.message);
  }
}
...
// ===================================================================
// ===================================================================
// ===== TESORERÃA: PAGOS A PROVEEDORES =====
// ===================================================================
...
function renderTesPagosProv() {
  ...
  // Recopilar proveedores que tienen mercancÃ­a a crÃ©dito
  ...
  el.innerHTML = `
    <div style="display:flex;gap:10px;margin-bottom:16px;flex-wrap:wrap;align-items:center">
      <button class="btn btn-primary" onclick="openAbonoProvModal()">ðŸ’³ Registrar Abono</button>
      <div style="margin-left:auto;font-size:11px;color:var(--text2)">
        Solo muestra proveedores con mercancÃ­a <span class="badge badge-warn">ðŸ’³ A CrÃ©dito</span>
      </div>
    </div>
    ...
    <div class="grid-3" style="margin-bottom:16px">
      <div class="card" style="margin:0;text-align:center;border-color:rgba(248,113,113,.3)">
        ...
        <div style="font-size:11px;color:var(--text2);margin-top:4px">ðŸ“¦ Deuda Total Bruta</div>
      </div>
      <div class="card" style="margin:0;text-align:center;border-color:rgba(74,222,128,.3)">
        ...
        <div style="font-size:11px;color:var(--text2);margin-top:4px">âœ… Total Abonado</div>
      </div>
      <div class="card" style="margin:0;text-align:center;border-color:rgba(251,191,36,.3)">
        ...
        <div style="font-size:11px;color:var(--text2);margin-top:4px">âš ï¸ Saldo Por Pagar</div>
      </div>
    </div>
    ...
    ${provConDeuda.length === 0 ? `
      <div class="empty-state">
        <div class="es-icon">ðŸ­</div>
        <div class="es-title">Sin deudas a crÃ©dito</div>
        <div class="es-text">Cuando registres artÃ­culos con tÃ­tulo "MercancÃ­a a CrÃ©dito" y un proveedor, aparecerÃ¡n aquÃ­.</div>
        ...
        <div class="card" style="margin-bottom:12px">
          <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:12px">
            <div>
              ...
              <div style="font-size:11px;color:var(--text2)">${p.cedula||''} Â· ${p.ciudad||''}</div>
              ${diasDesde !== null ? `<div style="font-size:10px;color:var(--orange);margin-top:2px">â± ${diasDesde} dÃ­as desde primera compra</div>` : ''}
            </div>
            <div style="text-align:right">
          ...
          <div style="display:flex;gap:16px;margin-bottom:10px;flex-wrap:wrap">
            ...
            <div style="font-size:12px"><span style="color:var(--text2)">ArtÃ­culos:</span> <b>${p.articulos.length}</b></div>
          </div>
          ...
          <details style="margin-bottom:10px">
            <summary style="font-size:11px;color:var(--text2);cursor:pointer">ðŸ“¦ Ver artÃ­culos a crÃ©dito (${p.articulos.length})</summary>
            <div class="table-wrap" style="margin-top:8px">
              <table><thead><tr><th>Ref</th><th>ArtÃ­culo</th><th>Stock</th><th>Costo unit.</th><th>Total</th><th>Desde</th></tr></thead><tbody>
              ${p.articulos.map(a => `<tr>
                ...
                <td style="font-size:10px;color:var(--text2)">${a.fechaCompra ? formatDate(a.fechaCompra) : (a.createdAt ? formatDate(a.createdAt.split('T')[0]) : 'â€”')}</td>
              </tr>`).join('')}
              </tbody></table>
            </div>
          </details>
          ...
          <div style="display:flex;gap:8px">
            <button class="btn btn-sm btn-primary" onclick="openAbonoProvModal('${p.id}','${p.nombre}')">ðŸ’³ Abonar</button>
            <button class="btn btn-sm btn-secondary" onclick="verAbonosProv('${p.id}','${p.nombre}')">ðŸ“‹ Ver abonos</button>
          </div>
        </div>`;
    }).join('')}
    ...
    <div class="card">
      <div class="card-title">ðŸ’³ ÃšLTIMOS ABONOS REGISTRADOS</div>
      <div class="table-wrap"><table><thead><tr>
        <th>Fecha</th><th>Proveedor</th><th>Valor</th><th>MÃ©todo</th><th>Nota</th><th></th>
      </tr></thead><tbody>
      ${abonosRecientes.map(ab => `<tr>
        <td>${formatDate(ab.fecha)}</td>
        <td style="font-weight:700">${ab.proveedorNombre||'â€”'}</td>
        <td style="color:var(--green);font-weight:700">${fmt(ab.valor||0)}</td>
        <td>${ab.metodo||'â€”'}</td>
        <td style="color:var(--text2);font-size:11px">${ab.nota||'â€”'}</td>
        <td><button class="btn btn-xs btn-danger" onclick="eliminarAbonoProv('${ab.id}')">âœ•</button></td>
      </tr>`).join('')}
      </tbody></table></div>
    </div>` : ''}`;
}
...
function openAbonoProvModal(provId='', provNombre='') {
  ...
  if(proveedores.length === 0 && !provId) {
    notify('warning','âš ï¸','Sin deudas','No hay proveedores con saldo pendiente.',{duration:3000});
    return;
  }

  openModal(`
    <div class="modal-title">ðŸ’³ Registrar Abono a Proveedor<button class="modal-close" onclick="closeModal()">Ã—</button></div>
    <div class="form-group"><label class="form-label">PROVEEDOR *</label>
      <select class="form-control" id="ab-prov-sel" onchange="updateSaldoPendiente()">
        ...
        ${proveedores.map(p => {
          const d = calcDeudaProveedor(p.id);
          return `<option value="${p.id}" data-saldo="${d.saldo}" data-nombre="${p.nombre}" ${p.id===provId?'selected':''}>${p.nombre} Â· Saldo: ${fmt(d.saldo)}</option>`;
        }).join('')}
      </select>
    </div>
    ...
    <div class="form-row">
      ...
      <div class="form-group"><label class="form-label">MÃ‰TODO DE PAGO</label>
        <select class="form-control" id="ab-metodo">
          ${(state.cfg_metodos_pago && state.cfg_metodos_pago.filter(m=>m.activo!==false).length > 0
          ? state.cfg_metodos_pago.filter(m=>m.activo!==false)
          : [{id:'efectivo',nombre:'ðŸ’µ Efectivo'},{id:'transferencia',nombre:'ðŸ“± Transferencia'},{id:'nequi',nombre:'Nequi'},{id:'daviplata',nombre:'Daviplata'},{id:'cheque',nombre:'Cheque'}]
    ...
    <div class="form-row">
      ...
      <div class="form-group"><label class="form-label">COMPROBANTE / NOTA</label>
        <input class="form-control" id="ab-nota" placeholder="NÂ° transferencia, observaciÃ³n...">
      </div>
    </div>
    <div id="ab-warning" style="display:none;color:var(--red);font-size:11px;margin-bottom:8px"></div>
    <button class="btn btn-primary" style="width:100%" onclick="guardarAbonoProv()">ðŸ’³ Guardar Abono</button>
  `);
...
function validateAbono() {
  ...
  if(valor > saldo && saldo > 0) {
    ...
    warn.textContent = `âš ï¸ El abono (${fmt(valor)}) supera el saldo pendiente (${fmt(saldo)})`;
  } else {
...
async function guardarAbonoProv() {
  ...
  if(!provId) { notify('warning','âš ï¸','Selecciona un proveedor','',{duration:3000}); return; }
  if(valor <= 0) { notify('warning','âš ï¸','Ingresa un valor','',{duration:3000}); return; }
  ...
  try {
    ...
    closeModal();
    renderTesPagosProv();
    notify('success','ðŸ’³','Abono registrado',`${fmt(valor)} a ${provNombre}`,{duration:3000});

  } catch(err) {
    showLoadingOverlay('hide');
    notify('danger','âš ï¸','Error al guardar', err.message, {duration:5000});
    console.error(err);
  }
}
...
function verAbonosProv(provId, provNombre) {
  ...
  openModal(`
    <div class="modal-title">ðŸ“‹ Abonos â€” ${provNombre}<button class="modal-close" onclick="closeModal()">Ã—</button></div>
    ...
    <div class="table-wrap"><table><thead><tr><th>Fecha</th><th>Valor</th><th>MÃ©todo</th><th>Nota</th></tr></thead><tbody>
...
async function eliminarAbonoProv(id) {
  if(!confirm('Â¿Eliminar este abono? El saldo pendiente se recalcularÃ¡.')) return;
  ...
  renderTesPagosProv();
  notify('success','ðŸ—‘ï¸','Abono eliminado','Saldo recalculado.',{duration:2000});
}
  ...
  document.getElementById('tes_cajas-content').innerHTML=`
    ...
    <div class="grid-2">${cajas.map(c=>`
      <div class="card" style="margin:0;border-color:${c.estado==='abierta'?'rgba(0,229,180,.3)':'var(--border)'}">
        ...
        <div class="btn-group">
          ${c.estado==='abierta'?`
            <button class="btn btn-sm btn-secondary" onclick="movCaja('${c.id}','ingreso')">ðŸ“¥ Ingreso</button>
            <button class="btn btn-sm btn-secondary" onclick="movCaja('${c.id}','egreso')">ðŸ“¤ Egreso</button>
            <button class="btn btn-sm btn-danger" onclick="cerrarCaja('${c.id}')">ðŸ”’ Cerrar</button>
          `:`<button class="btn btn-sm btn-primary" onclick="abrirCaja('${c.id}')">ðŸ”“ Abrir</button>`}
        </div>
      </div>`).join('')}</div>`;
}
...
function openCajaModal(){
  openModal(`
    <div class="modal-title">Nueva Caja<button class="modal-close" onclick="closeModal()">Ã—</button></div>
...
function cerrarCaja(id){
  ...
  if(c){
    ...
    renderTesCajas();notify('success','ðŸ”’','Caja cerrada',c.nombre+' Â· Saldo: '+fmt(c.saldo),{duration:3000});
  }
}
...
function saveMovCaja(cajaId, tipo) {
  ...
  closeModal();
  renderTesCajas();
  notify('success', 'âœ…', tipo === 'ingreso' ? 'Ingreso' : 'Egreso', fmt(valor) + ' Â· ' + concepto, { duration: 3000 });
}
...
function renderTesDinero(){
  ...
  document.getElementById('tes_dinero-content').innerHTML=`
    ...
    <div class="table-wrap"><table><thead><tr><th>Fecha</th><th>Caja</th><th>Tipo</th><th>Valor</th><th>Concepto</th><th>MÃ©todo</th></tr></thead><tbody>
    ${movs.map(m=>{const caja=(state.cajas||[]).find(c=>c.id===m.cajaId);return`<tr><td>${formatDate(m.fecha)}</td><td>${caja?.nombre||'â€”'}</td><td><span class="badge ${m.tipo==='ingreso'?'badge-ok':'badge-pend'}">${m.tipo}</span></td><td style="color:${m.tipo==='ingreso'?'var(--green)':'var(--red)'};font-weight:700">${fmt(m.valor)}</td><td>${m.concepto||'â€”'}</td><td>${m.metodo||'â€”'}</td></tr>`}).join('')||'<tr><td colspan="6" style="text-align:center;color:var(--text2);padding:24px">Sin movimientos</td></tr>'}
    </tbody></table></div></div>`;
}

// GENERIC SIMPLE RENDERERS FOR TESORERÃA SUB-MODULES
function renderSimpleCollection(pageId,title,collection,columns){
  ...
  el.innerHTML=`
    ...
    ${items.map(item=>`<tr>${columns.map(c=>{const key=c.split(':')[0];const type=c.split(':')[1];const val=item[key];return type==='number'?`<td style="font-weight:700;color:var(--accent)">${fmt(val||0)}</td>`:`<td>${val||'â€”'}</td>`}).join('')}<td><button class="btn btn-xs btn-danger" onclick="deleteFromCollection('${collection}','${item.id}','${pageId}')">âœ•</button></td></tr>`).join('')||`<tr><td colspan="${columns.length+1}" style="text-align:center;color:var(--text2);padding:24px">Sin registros</td></tr>`}
    </tbody></table></div></div>`;
}
...
function openSimpleFormModal(collection,title,columns){
  ...
  openModal(`
    <div class="modal-title">Nuevo - ${title}<button class="modal-close" onclick="closeModal()">Ã—</button></div>
...
function saveSimpleForm(collection, columns) {
  ...
  closeModal();
  renderPage(document.querySelector('.page.active')?.id.replace('page-', ''));
  notify('success', 'âœ…', 'Registro guardado', '', { duration: 2000 });
}
...
function deleteFromCollection(collection, id, pageId) {
  ...
  renderPage(pageId);
  notify('success', 'ðŸ—‘ï¸', 'Eliminado', 'Registro borrado correctamente.');
}
...
function renderTesCompRetencion(){renderSimpleCollection('tes_comp_retencion','Comprobantes RetenciÃ³n','tes_comp_retencion',['fecha:date:FECHA','numero:text:NÃšMERO','tercero:text:TERCERO','concepto:text:CONCEPTO','base:number:BASE','valor:number:VALOR'])}
function renderTesCompIngreso(){renderSimpleCollection('tes_comp_ingreso','Comprobantes Ingreso','tes_comp_ingreso',['fecha:date:FECHA','numero:text:NÃšMERO','tercero:text:TERCERO','concepto:text:CONCEPTO','valor:number:VALOR'])}

function renderTesCompEgreso(){renderSimpleCollection('tes_comp_egreso','Comprobantes Egreso','tes_comp_egreso',['fecha:date:FECHA','numero:text:NÃšMERO','tercero:text:TERCERO','concepto:text:CONCEPTO','valor:number:VALOR'])}
