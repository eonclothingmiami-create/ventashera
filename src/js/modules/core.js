...
function updateFalabellaStatusLineInModal(art) {
  ...
  el.style.color =
    st === 'error' ? 'var(--danger, #f87171)' : st === 'pending' ? 'var(--accent)' : 'var(--text2)';
}
...
async function postSaveFalabellaIntegration(productId) {
  ...
  try {
    ...
    if (falRes && falRes.ok) {
      ...
      const label = st === 'synced' ? 'OK' : st === 'error' ? 'error' : 'pendiente';
      return {
...
/** Deuda proveedor: solo mercancía a crédito + proveedor; ventas POS cuentan solo si el artículo cumple ambos (fallback si no hay AppTreasuryModule). */
function calcDeudaProveedor(provId) {
  ...
  const esCred = (t) => (typeof window.esMercanciaCredito === 'function' ? window.esMercanciaCredito(t) : String(t||'').trim().toLowerCase() === 'credito');
  const articulos = (state.articulos||[]).filter(a => {
    if(!esCred(a.tituloMercancia)) return false;
  ...
  const cxpIvMark = window.AppTreasuryModule?.CXPIV_ABONO_MARKER || '[cxp:inv_entrada]';
  const invEntradaCredAbonoAbs = (state.tes_abonos_prov || [])
    .filter(
      (ab) =>
        String(ab.proveedorId) === String(provId) &&
        parseFloat(ab.valor) < 0 &&
        String(ab.nota || '').includes(cxpIvMark)
    )
    .reduce((s, ab) => s + Math.abs(parseFloat(ab.valor) || 0), 0);
  const valorInventarioCostoNeto = Math.max(0, valorInventarioCosto);
  const tsMsFromAbonoProv = (ab) => {
    if (ab.fechaHora) {
      const t = Date.parse(ab.fechaHora);
      if (!Number.isNaN(t)) return t;
    }
    const d = ab.fecha ? String(ab.fecha).slice(0, 10) : '';
    if (d) return Date.parse(d + 'T23:59:59.999');
    return NaN;
  };
  const tsMsFromMoveProv = (m) => {
    if (!m || m.fecha == null || m.fecha === '') return NaN;
    const s = String(m.fecha);
    if (s.length <= 10) return Date.parse(s + 'T12:00:00');
    return Date.parse(s);
  };
  const abonosPosProv = (state.tes_abonos_prov || []).filter(
    (ab) => String(ab.proveedorId) === String(provId) && parseFloat(ab.valor) > 0,
  );
  let corteMsUltimoAbono = null;
  let fechaUltimoAbonoLabel = null;
  if (abonosPosProv.length) {
    let bestAb = null;
    let bestMs = -Infinity;
    abonosPosProv.forEach((ab) => {
      const t = tsMsFromAbonoProv(ab);
    ...
    if (bestAb) {
      corteMsUltimoAbono = bestMs;
      fechaUltimoAbonoLabel = bestAb.fechaHora
        ? String(bestAb.fechaHora).replace('T', ' ').slice(0, 19)
        : bestAb.fecha
          ? String(bestAb.fecha).slice(0, 10)
          : null;
    }
  }
  ...
  (state.stock_moves_ventas || []).forEach((m) => {
    ...
    if (!art || !esCred(art.tituloMercancia) || !art.proveedorId) return;
    if (String(art.proveedorId) !== String(provId)) return;
    const uds =
      typeof window.unidadesVentaPosAbs === 'function'
        ? window.unidadesVentaPosAbs(m)
    ...
    if (corteMsUltimoAbono == null) {
      vendidoDesdeUltimoAbono += uds * cost;
      unidadesVendidasDesdeUltimoAbono += uds;
    } else {
      const tm = tsMsFromMoveProv(m);
      if (!Number.isNaN(tm) && tm > corteMsUltimoAbono) {
        vendidoDesdeUltimoAbono += uds * cost;
        unidadesVendidasDesdeUltimoAbono += uds;
      }
    }
  ...
  const vendidoDesdeUltimoAbonoN = Math.max(0, vendidoDesdeUltimoAbono);
  const uVdaN = Math.max(0, unidadesVendidasDesdeUltimoAbono);
  ...
  (state.inv_ajustes||[]).forEach(aj=>{
    ...
    if(!artAj||!esCred(artAj.tituloMercancia)||!artAj.proveedorId)return;
  ...
  const refOperativaTotal = valorInventarioCostoNeto + costoVN + ajusteUnidadesCosto;
  ...
  const cxpIdsEspejoDevInv = new Set(
    (state.tes_devoluciones_prov||[]).filter(dv => String(dv.proveedorId)===String(provId)).map(dv => 'cxp-devolucion-'+String(dv.id))
  );
  const devolucionesNcCxpSinEspejoInv = (state.tes_cxp_movimientos||[]).filter(r =>
    String(r.proveedorId)===String(provId) && r.naturaleza==='credito' && r.tipo==='nota_credito' && !cxpIdsEspejoDevInv.has(String(r.id))
  ...
  const topePagoRespaldadoVentas = Math.min(saldoOperativoEstimado, costoVN);
  const pendienteSinRespaldoVentaCosto = Math.max(0, saldoOperativoEstimado - costoVN);
  let cxpCargo = 0;
  let cxpCredito = 0;
  (state.tes_cxp_movimientos || [])
  ...
  const vendidoDesdeUltimoAbonoAlcanzaSaldo = vendidoDesdeUltimoAbonoN + 1e-6 >= saldo;
  return {
    ...
    vendidoDesdeUltimoAbono: vendidoDesdeUltimoAbonoN,
    unidadesVendidasDesdeUltimoAbono: uVdaN,
...
function renderTesPagosProv() {
  ...
  const provTieneActividad = (d) =>
    (d.compromisoReconocido||0) > 0 || d.saldo > 0 || Math.abs(d.abonos||0) > 1e-9 || (d.valorInventarioCosto||0) > 0 || (d.costoVendidoHist||0) > 0 || Math.abs(d.ajusteUnidadesCosto||0) > 1e-9;
  ...
  el.innerHTML = `
    ...
    ${provConDeuda.length === 0 ? `
      <div class="empty-state">
        ...
        const saldoNumFb = parseFloat(p.saldo) || 0;
        const vendDesdeAbFb = parseFloat(p.vendidoDesdeUltimoAbono) || 0;
        const colorVAbFb = vendDesdeAbFb + 1e-6 >= saldoNumFb ? 'var(--green)' : 'var(--yellow)';
        const fechaUltFb = p.fechaUltimoAbonoLabel
          ? String(p.fechaUltimoAbonoLabel).replace(/</g, '&lt;')
          : '— sin abonos positivos · histórico completo';
        ...
        <div class="card" style="margin-bottom:12px">
          ...
          <div style="margin-bottom:12px;padding:10px 12px;border-radius:8px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1)">
            <div style="font-size:9px;font-weight:700;text-transform:uppercase;color:var(--text2);margin-bottom:6px">Resumen (calcDeudaProveedor · modo fallback)</div>
            <div style="display:flex;flex-wrap:wrap;gap:8px 12px;font-size:12px;line-height:1.45">
              <span><span style="color:var(--text2)">ref.</span> <b>${fmt(p.refOperativaTotal||0)}</b></span>
              <span>·</span>
              <span><span style="color:var(--text2)">pend. est.</span> <b style="color:var(--yellow)">${fmt(p.saldoOperativoEstimado||0)}</b></span>
              <span>·</span>
              <span><span style="color:var(--text2)">inv.</span> <b>${fmt(p.valorInventarioCosto||0)}</b></span>
              <span>·</span>
              <span><span style="color:var(--text2)">vendido total</span> <b>${fmt(p.costoVendidoHist||0)}</b></span>
              <span>·</span>
              <span><span style="color:var(--text2)">💰 vend. últ. abono</span> <b style="color:${colorVAbFb}">${fmt(vendDesdeAbFb)}</b></span>
            </div>
            <div style="font-size:10px;color:var(--text2);margin-top:6px">Último abono: <b style="color:var(--text1)">${fechaUltFb}</b> · Carga <code>treasury-module.js</code> para la vista completa (facturas + CXP).</div>
          </div>
...
function openAbonoProvModal(provId='', provNombre='') {
  ...
  const opts=[];
  (state.usu_proveedores||[]).forEach(p=>{const d=calcDeudaProveedor(p.id);if(d.saldo>0)opts.push({id:p.id,nombre:p.nombre,saldo:d.saldo,vendUltAb:Math.max(0,parseFloat(d.vendidoDesdeUltimoAbono)||0)});});
  if(provId && !opts.find(o=>o.id===provId)) {
    const d = calcDeudaProveedor(provId);
    opts.push({ id: provId, nombre: provNombre || 'Proveedor', saldo: d.saldo, vendUltAb: Math.max(0, parseFloat(d.vendidoDesdeUltimoAbono) || 0) });
  }
  ...
  openModal(`
    ...
    <div class="form-group"><label class="form-label">PROVEEDOR *</label>
      <select class="form-control" id="ab-prov-sel" onchange="updateSaldoPendiente()">
        ...
        ${opts.map(o => {
          ...
          return `<option value="${o.id}" data-saldo="${o.saldo}" data-vend-ult-ab="${o.vendUltAb ?? 0}" data-nombre="${nm}" ${o.id===provId?'selected':''}>${o.nombre} · Saldo: ${fmt(o.saldo)}</option>`;
        }).join('')}
      </select>
    </div>
    ...
    <div id="ab-vendido-sug-wrap" style="display:none;background:rgba(0,229,180,.07);border:1px solid rgba(0,229,180,.22);border-radius:8px;padding:10px;margin-bottom:12px;font-size:12px;line-height:1.45">
      <div><span style="color:var(--text2)">Vendido a costo desde el último abono:</span> <span id="ab-vendido-ult-val" style="font-weight:800;color:var(--accent)"></span></div>
      <div style="font-size:10px;color:var(--text2);margin-top:4px">Monto sugerido: <b id="ab-sugerido-pago-val"></b> (= mín(vendido, saldo))</div>
...
function validateAbono() {
  ...
  const excedeSaldo = valor > saldo + 1e-6;
  if (warn) {
    if (excedeSaldo) {
      warn.style.display = 'block';
      warn.textContent = `El abono (${fmt(valor)}) supera el saldo pendiente (${fmt(saldo)}).`;
    } else {
  ...
  if (warnVend) {
    if (!excedeSaldo && valor > vendRef + 1e-6 && valor > 1e-9) {
      ...
      warnVend.innerHTML = `Parte del abono (<b>${fmt(diff)}</b>) supera lo vendido desde el último abono (${fmt(vendRef)}). Estarías usando <b>recursos propios</b> para cubrir esa diferencia.`;
    } else {
...
function abonoUsarMontoVendidoSugerido() {
  if (window.AppTreasuryModule?.abonoUsarMontoVendidoSugerido) {
    return window.AppTreasuryModule.abonoUsarMontoVendidoSugerido({ fmt, state });
  }
  const sel = document.getElementById('ab-prov-sel');
  const opt = sel?.options[sel.selectedIndex];
  const saldo = parseFloat(opt?.getAttribute('data-saldo') || 0);
...
async function guardarAbonoProv() {
  if (window.AppTreasuryModule?.guardarAbonoProv) {
    return window.AppTreasuryModule.guardarAbonoProv({ state, uid, dbId, today, showLoadingOverlay, supabaseClient, saveRecord, closeModal, renderTesPagosProv, notify, fmt, renderTesCajas });
  }
  const sel = document.getElementById('ab-prov-sel');
  const opt = sel?.options[sel.selectedIndex];
  const provId = sel?.value;
...
function verAbonosProv(provId, provNombre) {
  ...
  openModal(`
    ...
    <div style="font-size:11px;color:var(--text2);margin-bottom:10px"><b>Desglose:</b> en stock ${fmt(d.valorInventarioCosto||0)}${
      (d.invEntradaCredAbonoDedup || 0) > 0.5 ? ` · reg. créd. inv. ${fmt(d.invEntradaCredAbonoDedup)} (en abonos)` : ''
    } · vendido POS ${fmt(d.costoVendidoHist||0)} · inv. ajustes neto (info) ${fmt(d.ajustesInvNeto || 0)}</div>
...
async function eliminarAbonoProv(id) {
  if (window.AppTreasuryModule?.eliminarAbonoProv) {
    return window.AppTreasuryModule.eliminarAbonoProv({ state, id, confirm, supabaseClient, saveRecord, renderTesPagosProv, notify, renderTesCajas });
  }
...
/** Cajas y libro de movimientos: implementación única en `treasury-module.js` (turno, arqueo, buckets, sesión). */
function renderTesCajas() {
  if (window.AppTreasuryModule?.renderTesCajas) {
    return window.AppTreasuryModule.renderTesCajas({
      state,
      fmt,
      openModal,
      formatDate,
  ...
  if (el) {
    el.innerHTML =
      '<div class="card" style="padding:20px;color:var(--text2)">Carga <b>treasury-module.js</b> para administrar cajas (turno, arqueo y cierres).</div>';
  }
}
...
function openCajaModal() {
  ...
  notify('warning', '⚠️', 'Módulo tesorería', 'Carga treasury-module.js para crear cajas.', { duration: 4000 });
}

function saveCaja() {
  if (window.AppTreasuryModule?.saveCaja) {
    return window.AppTreasuryModule.saveCaja({
      state,
      uid,
      dbId,
      saveRecord,
...
function movCaja(cajaId, tipo) {
  ...
  notify('warning', '⚠️', 'Módulo tesorería', 'Carga treasury-module.js para movimientos de caja.', { duration: 4000 });
}

function saveMovCaja(cajaId, tipo) {
  if (window.AppTreasuryModule?.saveMovCaja) {
    return window.AppTreasuryModule.saveMovCaja({
      state,
      cajaId,
      tipo,
      uid,
...
function renderTesDinero() {
  if (window.AppTreasuryModule?.renderTesDinero) {
    return window.AppTreasuryModule.renderTesDinero({ state, formatDate, fmt, today });
  }
  const el = document.getElementById('tes_dinero-content');
  if (el) {
    el.innerHTML =
      '<div class="card" style="padding:20px;color:var(--text2)">Carga <b>treasury-module.js</b> para el libro de movimientos con filtros por fecha.</div>';
  }
}
...
async function syncPosVentaInventoryFromDb(v, f) {
  ...
  try {
    ...
    const { data: movesRows, error: mvErr } = await supabaseClient
      .from('stock_moves')
      .select(`id, product_id, bodega_id, ${qtyCol}`)
      .eq('tipo', 'venta_pos')
      .eq('documento_id', id);
