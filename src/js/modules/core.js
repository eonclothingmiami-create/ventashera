...
/** Sin esto, `calcDeudaProveedor` en core no debe usarse para KPIs (buildAlerts, etc.). */
function isTreasuryCalcReady() {
  return !!(window.AppTreasuryModule && typeof window.AppTreasuryModule.calcDeudaProveedor === 'function');
}
...
/** Deuda por proveedor: fuente de verdad en `treasury-module.js` → `calcDeudaProveedor`. Si falta el módulo, devuelve ceros con `_erpTreasuryStub` (no usar para decisiones). */
...
function buildAlerts() {
  ...
  // --- Deudas a proveedores: solo si AppTreasuryModule está cargado (evita “todo OK” con stub en ceros) ---
  if (!isTreasuryCalcReady()) {
    treasuryFn('calcDeudaProveedor');
    alerts.push({
      type: 'urgent',
      icon: '🏧',
      title: 'Tesorería no cargada',
      desc: 'Falta AppTreasuryModule (treasury-module.js). No se calculan alertas de deuda a proveedores; Pagos proveedores y listas tesorería pueden fallar hasta recargar.',
