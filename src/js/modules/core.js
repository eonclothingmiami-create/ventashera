...
${(window.AppRepository?.SUPABASE_URL || (window.DROPI_SYNC_ENDPOINT || '').trim()) ? `
  ...
  <div style="font-weight: bold; color: var(--text1);">📦 Dropi al guardar (catálogo visible)</div>
</div>` : ''}
${(window.AppRepository?.SUPABASE_URL || (window.RAPPI_SYNC_ENDPOINT || '').trim()) ? `
  ...
  <div style="font-weight: bold; color: var(--text1);">🛵 Rappi al guardar (catálogo visible)</div>
</div>` : ''}
...
function renderTesDinero(){
  ...
  const t = today();
  const movs=[...(state.tes_movimientos||[])].filter(m=>String(m.fecha||'').slice(0,10)===t).reverse();
...
async function repararStockProductosPosMasivo(opts) {
  ...
  try {
    ...
    if (typeof renderHistorial === 'function') renderHistorial();
    if (document.getElementById('art-tbody') && typeof renderArticulosList === 'function') renderArticulosList();
    if (document.getElementById('tes_pagos_prov-content') && typeof renderTesPagosProv === 'function') renderTesPagosProv();
  } finally {
