...
async function loadState() {
  ...
  try {
    ...
    if(cajas&&cajas.length>0) {
      ...
      // #region agent log
      (function () {
        const ids = (state.cajas || []).map((c) => c.id);
        ...
        fetch('http://127.0.0.1:7397/ingest/fdd15a6c-f2bb-4e50-9cda-32cc3a03f3ff', {
          ...
          body: JSON.stringify({
            ...
            runId: 'cajas-debug',
            hypothesisId: 'H1',
            location: 'core.js:loadState',
            message: 'cajas loaded from Supabase',
            data: { branch: 'from_db', apiRowCount: cajas.length, stateCount: state.cajas.length, uniqueIds: unique, ids, nombres: state.cajas.map((c) => c.nombre) },
            timestamp: Date.now()
          })
        }).catch(function () {});
      })();
      ...
      fetch('http://127.0.0.1:7397/ingest/fdd15a6c-f2bb-4e50-9cda-32cc3a03f3ff', {
        ...
        body: JSON.stringify({
          ...
          hypothesisId: 'H2',
          location: 'core.js:loadState',
          message: 'cajas empty from API; keeping initial state',
          data: { branch: 'initial_only', stateCount: (state.cajas || []).length, ids: (state.cajas || []).map((c) => c.id) },
          timestamp: Date.now()
        })
      }).catch(function () {});
      // #endregion
