...
/** URL Edge Function dropi-sync-product (Dropi dropshipping). */
function getDropiSyncEndpoint() {
  const custom = (window.DROPI_SYNC_ENDPOINT || '').trim();
  if (custom) return custom;
  const base = window.AppRepository?.SUPABASE_URL;
  if (base) return String(base).replace(/\/$/, '') + '/functions/v1/dropi-sync-product';
  return '';
}
...
${(window.AppRepository?.SUPABASE_URL || (window.DROPI_SYNC_ENDPOINT || '').trim()) ? `
  ...
  <div style="font-size:11px;color:var(--text2);line-height:1.45;">Si marcas <strong>Mostrar en el Catálogo Web</strong> y guardas, el producto se envía a Dropi (no hace falta crear “tienda” tipo WooCommerce en el panel de Dropi: tu ERP usa la API con token en servidor). Edge Function: <code style="font-size:9px;">dropi-sync-product</code>.</div>
</div>` : ''}
...
async function saveArticulo(existingId) {
    ...
    try {
        ...
        renderArticulos();
        showLoadingOverlay('hide');
        notify('success', '✅', 'Guardado', `${refID} guardado.${catalogVisibleBool ? '' : ' Oculto en catálogo web (el sitio debe filtrar visible=true).'}${mlNote}${metaNote}${dropiNote}`);
