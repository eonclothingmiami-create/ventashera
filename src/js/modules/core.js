...
let posFormState = { 
  ...
  comprobante: '', cedula: '', cliente: '', telefono: '', metodo: 'efectivo', cuenta: '', applyIva: false, applyFlete: false, flete: 0,
  tipoPago: 'contado',
...
function getPinterestCatalogSyncEndpoint() {
  const custom = (window.PINTEREST_CATALOG_SYNC_ENDPOINT || window.PINTEREST_SYNC_ENDPOINT || '').trim();
  if (custom) return custom;
  const base = window.AppRepository?.SUPABASE_URL;
  if (base) {
  ...
  try {
    ...
    if (typeof console !== 'undefined' && console.info) {
      console.info(
        ...
        pinRes?.itemsProcessed || pinRes?.itemId || pinRes?.batchId || pinRes?.message || ''
      );
    }
    ...
    if (pinRes && pinRes.dryRun) {
      ...
      return ' · Pinterest: modo prueba (token OAuth o refresh+app en Supabase + URL base)';
    }
    if (pinRes && pinRes.ok) {
      const n = pinRes.itemsProcessed != null ? String(pinRes.itemsProcessed) : '';
      if (n) return ` · Pinterest: ${n} ítem(s)`;
      return pinRes.itemId ? ` · Pinterest: ${pinRes.itemId}` : ' · Pinterest: sincronizado';
    }
...
const COLLECTION_MAP = {
  ...
  'tes_ajustes_unidades_prov': { table:'tes_ajustes_unidades_prov', mapFn:(d)=>({id:d.id,proveedor_id:d.proveedorId,articulo_id:d.articuloId||null,delta_unidades:parseFloat(d.deltaUnidades)||0,nota:d.nota||'',fecha_hora:d.fechaHora||new Date().toISOString()}) },
  'ventas_catalogo': { table:'ventas_catalogo', mapFn:(d)=>({id:d.id,reference:d.reference||'',estado_pago:d.estadoPago||'pendiente',canal_pago:d.canalPago||null,catalog_type:d.catalogType||null,cliente_nombre:d.clienteNombre||'',cliente_email:d.clienteEmail||'',cliente_telefono:d.clienteTelefono||'',cliente_documento_tipo:d.clienteDocumentoTipo||'CC',cliente_documento:d.clienteDocumento||'',envio_departamento:d.envioDepartamento||'',envio_ciudad:d.envioCiudad||'',envio_direccion:d.envioDireccion||'',items:Array.isArray(d.items)?d.items:[],totales:(d.totales&&typeof d.totales==='object')?d.totales:{},amount_cop:Number(d.amountCop)||0,proveedor_ref:d.proveedorRef||null,pagado_at:d.pagadoAt||null,pos_factura_id:d.posFacturaId||null}) },
...
async function procesarVentaPOS(opts) {
  ...
  if (canalV === 'local') {
    ...
    if (tpLoc === 'contraentrega') {
      ...
      const okLoc = ['nequi', 'daviplata', 'transferencia', 'tarjeta_debito', 'tarjeta_credito', 'efectivo'];
...
/** Envía a Pinterest un solo lote (fullSync en servidor) para prendas visibles con imagen y URL base. */
async function bulkPinterestCatalogSyncVisibleArticles() {
  ...
  if (typeof window.requestPinterestCatalogSync !== 'function') {
    notify('danger', '📌', 'Pinterest', 'pinterest-catalog.js no cargó.', { duration: 5000 });
    return;
  }
  const list = (state.articulos || []).filter((a) => normalizeVisibleFlag(a.mostrarEnWeb));
  if (!list.length) {
  ...
  if (
    !confirm(
      `Se enviará a Pinterest un lote con hasta ${list.length} prenda(s) visibles (el servidor filtra las que tengan imagen HTTPS y URL pública). ¿Continuar?`,
    )
  )
    return;
  showLoadingOverlay('connecting');
  try {
    ...
    if (pinRes && pinRes.dryRun) {
      notify(
        ...
        '📌',
        'Pinterest',
        pinRes.message || 'Modo prueba — configura token y URL en Supabase.',
        { duration: 10000 },
      );
      return;
    }
    ...
    notify(
      ...
      '📌',
      'Pinterest (lote)',
      `Procesados: ${n}${pinRes && pinRes.skipped != null ? ` · omitidos sin datos: ${pinRes.skipped}` : ''}`,
      { duration: 12000 },
    );
...
${(window.AppRepository?.SUPABASE_URL || (window.PINTEREST_CATALOG_SYNC_ENDPOINT || '').trim() || (window.PINTEREST_SYNC_ENDPOINT || '').trim()) ? `
  ...
  <label style="display: flex; align-items: center; gap: 10px; cursor: pointer; color: var(--text1); font-weight: bold;">
    ...
    📌 Sincronizar con Pinterest (catálogo) al guardar
  </label>
  <div style="font-size:10px;color:var(--text2);margin-top:6px;margin-left:28px;line-height:1.35;">OAuth (repo <code style="font-size:9px;">pinterest/api-quickstart</code>): scopes <code style="font-size:9px;">catalogs:read</code> + <code style="font-size:9px;">catalogs:write</code>. Secrets: <code style="font-size:9px;">PINTEREST_PRODUCT_BASE_URL</code> y <code style="font-size:9px;">PINTEREST_ACCESS_TOKEN</code> <em>o</em> <code style="font-size:9px;">PINTEREST_REFRESH_TOKEN</code> + <code style="font-size:9px;">PINTEREST_APP_ID</code> + <code style="font-size:9px;">PINTEREST_APP_SECRET</code>. Opcionales: país/idioma/marca/ad account. Píxel de conversiones va en el catálogo web (<code style="font-size:9px;">PINTEREST_TAG_ID</code>). Deploy: <code style="font-size:9px;">pinterest-catalog-sync</code> <code style="font-size:9px;">--no-verify-jwt</code>.</div>
</div>` : ''}
...
async function saveArticulo(existingId) {
    ...
    try {
        ...
        const pinterestNote = await postSavePinterestCatalogIntegration(productId, catalogVisibleBool);
...
async function saveAjusteInv() {
  ...
  try {
    ...
    if (tipo === 'devolucion' && product.tituloMercancia === 'credito' && product.proveedorId) {
      ...
      const valorCosto = (parseFloat(product.precioCompra)||0) * cant;
      const devId = dbId();
      const notaDv = `Devolución inventario · ${product.nombre||artId} · ${motivo}`;
      const fh = new Date().toISOString();
      await supabaseClient.from('tes_devoluciones_prov').insert({
...
function calcDeudaProveedor(provId) {
  ...
  (state.stock_moves_ventas||[]).forEach(m=>{
    ...
    if(!art||art.tituloMercancia!=='credito'||!art.proveedorId)return;
    if(String(art.proveedorId)!==String(provId))return;
  ...
  (state.inv_ajustes||[]).forEach(aj=>{
    ...
    if(!artAj||artAj.tituloMercancia!=='credito'||!artAj.proveedorId)return;
    if(String(artAj.proveedorId)!==String(provId))return;
  ...
  const refOperativaTotal = valorInventarioCosto + costoVN + ajusteUnidadesCosto;
  ...
  const devolucionesDeuda = (state.tes_devoluciones_prov||[]).filter(dv => String(dv.proveedorId) === String(provId)).reduce((s, dv) => s + (parseFloat(dv.valorCosto)||0), 0);
  const saldoLibro = Math.max(0, compromisoReconocido - abonos);
  const saldo = Math.max(0, refOperativaTotal - abonos - devolucionesDeuda);
  return { valorInventarioCosto, costoVendidoHist: costoVN, unidadesVendidasHist: uVN, ajusteUnidadesCosto, refOperativaTotal, compromisoReconocido, compromisoTotal: compromisoReconocido, deudaBruta: refOperativaTotal, abonos, devolucionesDeuda, saldo, saldoLibro, articulos, ajustesSalidaCosto, ajustesSalidaUds, ajustesEntradaCosto, ajustesEntradaUds };
}
...
function renderTesPagosProv() {
  ...
  const provTieneActividad = (d) =>
    (d.compromisoReconocido||0) > 0 || d.saldo > 0 || (d.abonos||0) > 0 || (d.valorInventarioCosto||0) > 0 || (d.costoVendidoHist||0) > 0 || Math.abs(d.ajusteUnidadesCosto||0) > 1e-9;
...
function openAbonoProvModal(provId='', provNombre='') {
  ...
  if(provId && !opts.find(o=>o.id===provId)) {
    ...
    opts.push({ id: provId, nombre: provNombre || 'Proveedor', saldo: d.saldo });
  }
  ...
  openModal(`
    ...
    <div class="form-row">
      ...
      <div class="form-group"><label class="form-label">PAGO DEL ABONO *</label>
        <select class="form-control" id="ab-medio-caja">
          <option value="efectivo">💵 Efectivo → bucket efectivo</option>
          <option value="transferencia">🏦 Transferencia → bucket transferencia</option>
        </select>
      </div>
    </div>
    <div class="form-row">
...
function verLibroProveedorModal(provId, provNombre) {
  if (window.AppTreasuryModule?.verLibroProveedorModal) {
    return window.AppTreasuryModule.verLibroProveedorModal({ state, provId, provNombre, fmt, formatDate, openModal });
  }
  notify('warning', '⚠️', 'Módulo tesorería', 'Carga treasury-module.js para ver el libro.', { duration: 4000 });
}

function quitarCreditoArticuloProveedorFromPagos(artId) {
  if (window.AppTreasuryModule?.quitarCreditoArticuloProveedor) {
    return window.AppTreasuryModule.quitarCreditoArticuloProveedor({
      state,
      artId,
      confirm,
      supabaseClient,
  ...
  notify('warning', '⚠️', 'Módulo tesorería', 'Carga treasury-module.js para corregir deuda.', { duration: 4000 });
}

function openAjusteUnidadesProvModal(provId, artId) {
  if (window.AppTreasuryModule?.openAjusteUnidadesProvModal) {
    return window.AppTreasuryModule.openAjusteUnidadesProvModal({ state, provId, artId, fmt, openModal, notify });
  }
  notify('warning', '⚠️', 'Módulo tesorería', 'Carga treasury-module.js para ajustar unidades.', { duration: 4000 });
}

function guardarAjusteUnidadesProv(provId, artId) {
  if (window.AppTreasuryModule?.guardarAjusteUnidadesProv) {
    return window.AppTreasuryModule.guardarAjusteUnidadesProv({
      state,
      provId,
      artId,
      uid,
  ...
  notify('warning', '⚠️', 'Módulo tesorería', 'Carga treasury-module.js.', { duration: 4000 });
}

function eliminarAjusteUnidadesProv(id, provId, artId) {
  if (window.AppTreasuryModule?.eliminarAjusteUnidadesProv) {
    return window.AppTreasuryModule.eliminarAjusteUnidadesProv({
      state,
      id,
      confirm,
      supabaseClient,
...
function renderVentasCatalogo(){
  ...
  if (window.AppVentasCatalogoModule?.renderVentasCatalogo) {
    return window.AppVentasCatalogoModule.renderVentasCatalogo({
      state, fmt, openModal, saveRecord, notify, renderVentasCatalogo
    });
  }
