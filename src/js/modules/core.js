...
async function loadState() {
  ...
  try {
    const [
      ...
      Promise.resolve({data:[]}), // ventas: se cargan paginadas abajo (misma razón que tes_movimientos)
    ...
    /** Misma regla: la meta del dashboard y el historial suman `ventas`; sin paginar, solo se ve un subconjunto (p. ej. 1000) y la cifra queda desfasada respecto a `tes_movimientos` / caja. */
    let ventas = [];
    try {
