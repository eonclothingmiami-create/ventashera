...
function refreshFalabellaMaquetadorValidation() {
  ...
  if (profile !== 'falabella') {
    ...
    if (badge) {
      ...
      badge.style.background = 'rgba(0,229,180,0.22)';
    }
    return;
  }
  ...
  if (errEl) {
    errEl.innerHTML = errs.length
      ? `<div style="opacity:.9">Overrides parciales: el servidor auto-completa lo vacío.</div>` +
        errs
          .slice(0, 6)
  ...
  if (badge) {
    badge.textContent = errs.length ? 'Overrides parciales' : 'Overrides OK';
    badge.style.background = errs.length ? 'rgba(255,180,80,0.22)' : 'rgba(0,229,180,0.22)';
  }
...
async function postSaveFalabellaIntegration(productId) {
  ...
  if (!want) return { note: '' };
  try {
    // Modo Mercado Libre: solo productId. El Edge auto-mapea categoría/attrs/color/talla/marca.
    // Overrides opcionales si el usuario llenó el panel avanzado Falabella.
    let falExtra;
    const profile = document.getElementById('m-art-channel-profile')?.value || 'generic';
    const hasAdvanced =
      profile === 'falabella' &&
      window.FalabellaMaquetador &&
      String(document.getElementById('m-fal-primary-cat')?.value || '').trim();
    if (hasAdvanced && window.FalabellaMaquetador.buildFalabellaPayload) {
      const draft = window.FalabellaMaquetador.collectDraftFromDom();
      const artRow = {
        id: productId,
...
async function reenviarFalabellaFeedModal() {
  ...
  const hasAdvanced =
    ...
    window.FalabellaMaquetador &&
    (String(document.getElementById('m-fal-primary-cat')?.value || '').trim() ||
      artSt?.falabellaProductDataJson?.falabellaDraft?.primaryCategoryId);
  if (hasAdvanced && window.FalabellaMaquetador) {
    if (artSt?.falabellaProductDataJson?.falabellaDraft) {
...
function checkBadges(){const active=(state.ventas||[]).filter(v=>ventaCuentaParaTotales(v));const despachos=active.filter(v=>canonCanalVenta(v)!=='vitrina');const ymB=calendarYearMonthLocal();const metaProg=tesVentaPosMetricsForMonth(state.tes_movimientos||[],ymB).ingresosCOP||0;const pct=state.meta>0?metaProg/state.meta:0;if(active.length>=1)checkAndAwardBadge('primera_venta');if(despachos.length>=20)checkAndAwardBadge('v20');if(despachos.length>=50)checkAndAwardBadge('v50');if(despachos.length>=100)checkAndAwardBadge('v100');if(despachos.length>=150)checkAndAwardBadge('v150');if(pct>=0.25)checkAndAwardBadge('meta25');if(pct>=0.50)checkAndAwardBadge('meta50');if(pct>=0.75)checkAndAwardBadge('meta75');if(pct>=1.00)checkAndAwardBadge('meta100');if(metaProg>=40000000)checkAndAwardBadge('super');if(active.some(v=>(parseFloat(v.valor)||0)>=500000))checkAndAwardBadge('gran_venta');const hoy=today();const todayCanals=new Set(active.filter(v=>v.fecha===hoy).map(v=>canonCanalVenta(v)));if(todayCanals.has('vitrina')&&todayCanals.has('local')&&todayCanals.has('inter'))checkAndAwardBadge('multicanal')}
...
function renderDashboard(){
  ...
  /** Una sola fuente para la barra de meta: ingresos clase venta_pos en caja (misma base que trazabilidad). */
  const metaProgCop=tesMes.ingresosCOP||0;
  const metaGoal=parseFloat(state.meta)||0;
  const pct=metaGoal>0?Math.min(100,(metaProgCop/metaGoal)*100):0;
  const faltaMeta=Math.max(0,metaGoal-metaProgCop);
  const g=state.game||{};
    ...
    <div class="rpg-header">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:12px">
        <div>
          ...
          <div style="font-family:Syne;font-size:22px;font-weight:800">${fmt(metaProgCop)} <span style="font-size:13px;color:var(--text2);font-weight:400">de ${fmt(metaGoal)}</span></div>
          ...
          <div style="font-size:9px;color:var(--text2);margin-top:6px;max-width:520px;line-height:1.35;opacity:.88">Progreso según <b>ingresos POS en caja</b> (${ymMes}). Las tarjetas por canal son facturación en tabla ventas.</div>
        </div>
  ...
  <div style="font-size:10px;color:var(--text2);margin:-4px 0 16px;opacity:.9">Facturación por canal · ${ymMes} (tabla ventas): vitrina ${fmt(vm.vitrineTotal||0)} + local ${fmt(vm.localTotal||0)} + inter ${fmt(vm.interTotal||0)} + otros ${fmt(vm.otrosTotal||0)} = <b>${fmt(vm.totalCOP)}</b></div>
  ...
  <div class="card">
    ...
    <div style="font-size:11px;color:var(--text2);margin:-6px 0 10px">Ingresos POS mes (caja): <b>${fmt(metaProgCop)}</b> · tickets ~${tesMes.ticketsUnicos} · facturas mes en tabla: <b>${resumenMesCal.length}</b> · <b>${fmt(totalMesCal)}</b></div>
        ...
        <div style="max-height: 75vh; overflow-y: auto; padding-right: 10px; text-align:left;">
            ...
            <div class="form-row">
                <div class="form-group">
                    <label class="form-label">Perfil avanzado (opcional)</label>
                    <select class="form-control" id="m-art-channel-profile" onchange="onMaquetadorChannelProfileChange()">
                        <option value="generic">Automático (recomendado)</option>
                        <option value="falabella">Overrides manuales Falabella</option>
                    </select>
                    <p style="font-size:10px;color:var(--text2);margin:6px 0 0;line-height:1.35;">Con el checkbox Falabella basta: el servidor mapea categoría, color, talla y attrs. Usa «Overrides» solo si necesitas forzar un ID o marca.</p>
                </div>
            </div>
            <div id="falabella-requisitos-wrap" style="display:none;padding:14px;border-radius:10px;border:1px solid rgba(100,80,200,0.35);background:rgba(100,80,200,0.06);margin-bottom:16px;">
                <div style="font-weight:800;color:var(--accent);margin-bottom:10px;">Overrides Falabella (opcional)</div>
                <p style="font-size:10px;color:var(--text2);margin:0 0 10px;line-height:1.35;">Vacío = auto-mapa en servidor (categoría ERP→3199 baño, color básico, talla, ProductData). Solo completa si quieres forzar valores.</p>
                <div class="form-row">
