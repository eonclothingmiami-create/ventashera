...
/** Pinterest Catalog / Shopping → Edge Function pinterest-catalog-sync */
function getPinterestSyncEndpoint() {
  const custom = (window.PINTEREST_SYNC_ENDPOINT || '').trim();
  if (custom) return custom;
  const base = window.AppRepository?.SUPABASE_URL;
  if (base) {
...
async function postSaveMercadoLibreIntegration(productId, catalogVisibleBool) {
  ...
  const want = mlEndpoint && mlChk && mlChk.checked && catalogVisibleBool;
  if (!want) return '';
  try {
    ...
    if (mlRes && mlRes.skipped) return ' · ML: no se llamó al endpoint';
    if (mlRes && mlRes.dryRun) {
      ...
      return ' · ML: modo prueba (configura ML_ACCESS_TOKEN y ML_DEFAULT_CATEGORY_ID_MCO en Supabase)';
    }
    if (mlRes && mlRes.ok) {
      const link = mlRes.permalink || mlRes.itemId || '';
      return link ? ` · ML: ${link}` : ' · ML: publicado';
    }
    return ' · ML: respuesta inesperada';
  } catch (mlErr) {
    console.warn('[Mercado Libre]', mlErr);
    notify('warning', '🛒', 'Mercado Libre', mlErr.message || 'Falló la sincronización', { duration: 8000 });
    return ' · ML: error (consola / Edge Function)';
  }
}
...
async function postSaveMetaCommerceIntegration(productId, catalogVisibleBool) {
  ...
  try {
    ...
    if (metaRes && metaRes.skipped) return ' · Meta: no se llamó al endpoint';
    if (metaRes && metaRes.dryRun) {
      ...
      return ' · Meta: modo prueba (META_ACCESS_TOKEN / META_CATALOG_ID en Supabase)';
    }
    if (metaRes && metaRes.ok) {
      return metaRes.retailerId ? ` · Meta: ${metaRes.retailerId}` : ' · Meta: sincronizado';
    }
    return ' · Meta: respuesta inesperada';
  } catch (metaErr) {
    console.warn('[Meta Commerce]', metaErr);
    notify('warning', '📱', 'Meta Commerce', metaErr.message || 'Falló la sincronización', { duration: 8000 });
    return ' · Meta: error (consola / Edge Function)';
  }
}
 ...
 */
async function postSaveFalabellaIntegration(productId, catalogVisibleBool) {
  ...
  try {
    ...
    if (typeof console !== 'undefined' && console.info) {
      console.info(
        ...
        falRes?.requestId || falRes?.sellerSku || falRes?.message || '',
      );
    }
    ...
    if (falRes && falRes.ok) {
      ...
      return {
        note: falRes.sellerSku ? ` · Falabella: SKU ${falRes.sellerSku}${rid}` : ` · Falabella: enviado${rid}`,
...
async function postSaveGoogleMerchantIntegration(productId, catalogVisibleBool) {
  ...
  try {
    ...
    if (gRes && gRes.skipped) return ' · Google: no se llamó al endpoint';
    if (gRes && gRes.dryRun) {
      notify('warning', '🔍', 'Google Merchant', gRes.message || 'Configura secrets en la Edge Function.', { duration: 9000 });
      return ' · Google: modo prueba (Merchant ID + cuenta de servicio + URL base en Supabase)';
    }
    if (gRes && gRes.ok) {
      return gRes.offerId ? ` · Google: ${gRes.offerId}` : ' · Google: sincronizado';
    }
    return ' · Google: respuesta inesperada';
  } catch (gErr) {
    console.warn('[Google Merchant]', gErr);
    notify('warning', '🔍', 'Google Merchant', gErr.message || 'Falló la sincronización', { duration: 8000 });
    return ' · Google: error (consola / Edge Function)';
  }
}

async function postSavePinterestIntegration(productId, catalogVisibleBool) {
  const pinEndpoint = getPinterestSyncEndpoint();
  const pinChk = document.getElementById('art-sync-pinterest');
  const want =
  ...
  try {
    ...
    if (typeof console !== 'undefined' && console.info) {
      console.info(
        ...
        pinRes?.itemsProcessed || pinRes?.message || ''
      );
    }
    if (pinRes && pinRes.skipped) return ' · Pinterest: no se llamó al endpoint';
    if (pinRes && pinRes.dryRun) {
      notify('warning', '📌', 'Pinterest', pinRes.message || 'Configura PINTEREST_ACCESS_TOKEN y URL base en Supabase.', { duration: 9000 });
      return ' · Pinterest: modo prueba (secrets en Edge Function)';
    }
    if (pinRes && pinRes.ok) {
      const n = pinRes.itemsProcessed != null ? String(pinRes.itemsProcessed) : '';
      return n ? ` · Pinterest: ${n} ítem(s)` : ' · Pinterest: sincronizado';
    }
    return ' · Pinterest: respuesta inesperada';
  } catch (pinErr) {
    console.warn('[Pinterest]', pinErr);
    notify('warning', '📌', 'Pinterest', pinErr.message || 'Falló la sincronización', { duration: 8000 });
    return ' · Pinterest: error (consola / Edge Function)';
  }
}
  ...
  try {
    ...
    try {
      ...
      while (supabaseClient) {
        ...
        if (smErr) { console.warn('stock_moves venta_pos:', smErr.message); break; }
        const chunk = smRows || [];
        chunk.forEach((r) => {
          const qty = r.cantidad ?? r.quantity ?? r.qty ?? r.amount;
          state.stock_moves_ventas.push({
            id: r.id,
            productId: r.product_id,
            cantidad: qty != null ? Number(qty) : 0,
            tipo: r.tipo,
            fecha: r.fecha,
    ...
    } catch (e) { console.warn('stock_moves:', e.message); }
...
/** Rellena desplegables de envío según canal (no repinta el POS). */
function syncPOSShippingUI() {
  ...
  if (canal === 'vitrina') {
    ...
    if (canal === 'local') {
      ...
      if (liqInfo) liqInfo.textContent = '⚡ Liquidación al día siguiente hábil.';
    } else if (canal === 'inter') {
      ...
      if (liqInfo) liqInfo.textContent = `📦 Liquidación en ${state.diasInter} días hábiles.`;
    }
  }
}

/** Último canal elegido en el POS (para activar flete al pasar a inter la primera vez). */
let _lastPosCanalElegido = null;

/** onchange del canal: actualiza estado y repinta para mostrar/ocultar flete (mensajería local / inter). */
function handlePOSShippingUI() {
  const sel = document.getElementById('pos-canal');
  const nuevoCanal = sel ? sel.value : null;
  if (nuevoCanal === 'inter' && _lastPosCanalElegido !== 'inter') {
    posFormState.applyFlete = true;
  }
  if (nuevoCanal) _lastPosCanalElegido = nuevoCanal;
  ...
  if (window.AppPosController?.handleShippingUI) {
    ...
    const canalEl = document.getElementById('pos-canal');
    if (!canalEl) return;
    posFormState.canal = canalEl.value;
  }
  renderPOS();
}
...
async function procesarVentaPOS() {
  ...
  const fleteV = (posFormState.applyFlete && (canalV === 'local' || canalV === 'inter')) ? (posFormState.flete || 0) : 0;
  ...
  if (window.AppPosService?.buildPosDocuments) {
    ...
    flete = (posFormState.applyFlete&&(canal==='local'||canal==='inter'))?(posFormState.flete||0):0;
  ...
  // Actualizar stock en BD + trazabilidad stock_moves + caja (ingreso en venta POS si hay caja abierta)
  if(_sbConnected){
    try {
  ...
  if (window.AppPosRepository?.autoRegisterCustomer) {
    ...
    if(!yaExiste){
      const nc={id:dbId(),tipo:'cliente',tipoId:'CC',cedula:posFormState.cedula||'',nombre:posFormState.cliente,celular:posFormState.telefono||'',whatsapp:posFormState.telefono||'',ciudad:posFormState.ciudad||'',fechaCreacion:fechaActual};
  ...
  posFormState={canal:'vitrina',empresa:'',transportadora:'',guia:'',ciudad:'',cliente:'',telefono:'',metodo:'efectivo',cuenta:'',applyIva:true,applyFlete:false,flete:0,tipoPago:'contado',bodegaId:_keepBod,cajaId:_keepCaja};
  ...
  document.getElementById('articulos-content').innerHTML=`
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;flex-wrap:wrap;gap:8px">
      ...
      <div class="btn-group">
        ...
        ${(window.AppRepository?.SUPABASE_URL || (window.FALABELLA_SYNC_ENDPOINT || '').trim()) ? `<button class="btn btn-secondary btn-sm" onclick="bulkFalabellaSyncVisibleArticles()" title="Un feed ProductCreate por prenda (Seller Center)" style="border-color:rgba(255,120,60,.45);color:#ff7a3d;">🏬 Falabella: enviar visibles</button>` : ''}
        ${(window.AppRepository?.SUPABASE_URL || (window.PINTEREST_SYNC_ENDPOINT || '').trim()) ? `<button class="btn btn-secondary btn-sm" onclick="bulkPinterestCatalogSyncVisibleArticles()" title="Catálogo visible: lote vía Edge Function (Pinterest API)" style="border-color:rgba(230,0,35,.35);color:#e60023;">📌 Pinterest: sincronizar visibles</button>` : ''}
...
/** Envía a Pinterest (un solo lote en servidor) todas las prendas visibles en catálogo web con imagen y URL base configurados. */
async function bulkPinterestCatalogSyncVisibleArticles() {
  const pinEndpoint = getPinterestSyncEndpoint();
  if (!pinEndpoint) {
    notify('warning', '📌', 'Pinterest', 'Sin endpoint — revisa Supabase / repository.js.', { duration: 6000 });
    return;
  }
  if (typeof window.requestPinterestCatalogSync !== 'function') {
    notify('danger', '📌', 'Pinterest', 'pinterest.js no cargó.', { duration: 5000 });
    return;
  }
  const list = (state.articulos || []).filter((a) => normalizeVisibleFlag(a.mostrarEnWeb));
  if (!list.length) {
    notify('warning', '📌', 'Pinterest', 'No hay prendas con «Mostrar en catálogo web» activo.', { duration: 6000 });
    return;
  }
  if (!confirm(`Se enviará a Pinterest un lote con hasta ${list.length} prenda(s) visibles (el servidor filtra las que tengan imagen HTTPS y URL pública). ¿Continuar?`)) return;
  showLoadingOverlay('connecting');
  try {
    const pinRes = await window.requestPinterestCatalogSync(null, { fullSync: true });
    if (pinRes && pinRes.skipped) {
      notify('warning', '📌', 'Pinterest', pinRes.reason || 'No se llamó al endpoint.', { duration: 8000 });
      return;
    }
    if (pinRes && pinRes.dryRun) {
      notify('warning', '📌', 'Pinterest', pinRes.message || 'Modo prueba — configura token y URL en Supabase.', { duration: 10000 });
      return;
    }
    const n = pinRes && pinRes.itemsProcessed != null ? pinRes.itemsProcessed : '—';
    notify('success', '📌', 'Pinterest (lote)', `Procesados: ${n}${pinRes && pinRes.skipped != null ? ` · omitidos sin datos: ${pinRes.skipped}` : ''}`, { duration: 12000 });
    if (typeof console !== 'undefined' && console.info) console.info('[Pinterest lote]', pinRes);
  } catch (e) {
    console.warn('[Pinterest lote]', e);
    notify('danger', '📌', 'Pinterest', (e && e.message) || String(e), { duration: 10000 });
  } finally {
    showLoadingOverlay('hide');
  }
}

/** Envía a Falabella (ProductCreate) cada prenda visible; la Edge Function persiste estado en products. */
async function bulkFalabellaSyncVisibleArticles() {
  const falEndpoint = getFalabellaSyncEndpoint();
  if (!falEndpoint) {
    notify('warning', '🏬', 'Falabella', 'Sin endpoint — revisa Supabase / repository.js.', { duration: 6000 });
    return;
  }
  if (typeof window.requestFalabellaSync !== 'function') {
    notify('danger', '🏬', 'Falabella', 'falabella.js no cargó.', { duration: 5000 });
    return;
  }
  const list = (state.articulos || []).filter((a) => normalizeVisibleFlag(a.mostrarEnWeb));
  if (!list.length) {
    notify('warning', '🏬', 'Falabella', 'No hay prendas con «Mostrar en catálogo web» activo.', { duration: 6000 });
    return;
  }
  if (!confirm(`Se enviará a Falabella ${list.length} prenda(s) visible(s) (un feed por ítem). ¿Continuar?`)) return;
  showLoadingOverlay('connecting');
  let ok = 0;
  let dryRun = 0;
  let failed = 0;
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    const label = a.codigo || a.nombre || a.id;
    try {
      const falRes = await window.requestFalabellaSync(a.id);
      if (falRes && falRes.skipped) {
        failed++;
        lines.push(`${label}: omitido (${falRes.reason || '—'})`);
      } else if (falRes && falRes.dryRun) {
        dryRun++;
        lines.push(`${label}: modo prueba — ${falRes.message || 'configura secrets'}`);
      } else if (falRes && falRes.ok) {
        ok++;
        const idx = state.articulos.findIndex((x) => x.id === a.id);
        if (idx >= 0) {
          state.articulos[idx].falabellaSyncStatus = 'synced';
          state.articulos[idx].falabellaSellerSku = falRes.sellerSku || state.articulos[idx].falabellaSellerSku;
          state.articulos[idx].falabellaFeedRequestId = falRes.requestId || state.articulos[idx].falabellaFeedRequestId;
          state.articulos[idx].falabellaPrimaryCategoryId = falRes.primaryCategoryId || state.articulos[idx].falabellaPrimaryCategoryId;
          state.articulos[idx].falabellaLastError = '';
        }
        lines.push(`${label}: OK ${falRes.requestId || falRes.sellerSku || ''}`);
      } else {
        failed++;
        lines.push(`${label}: respuesta inesperada`);
      }
      ...
      const idx = state.articulos.findIndex((x) => x.id === a.id);
      if (idx >= 0) {
        state.articulos[idx].falabellaSyncStatus = 'error';
        state.articulos[idx].falabellaLastError = (e && e.message) || String(e);
      }
    }
    await delay(700);
  }
  showLoadingOverlay('hide');
  const head = `Enviados OK: ${ok} · Dry-run: ${dryRun} · Error: ${failed}`;
  const detail = lines.length ? '\n' + lines.slice(0, 14).join('\n') + (lines.length > 14 ? '\n…' : '') : '';
  const tone = failed && !ok && !dryRun ? 'danger' : failed || dryRun ? 'warning' : 'success';
  notify(tone, '🏬', 'Falabella (lote)', head + detail, { duration: 16000 });
  console.log('[Falabella lote]', { ok, dryRun, failed, total: list.length });
}
...
${(window.AppRepository?.SUPABASE_URL || (window.MERCADOLIBRE_SYNC_ENDPOINT || '').trim()) ? `
  ...
  <div style="font-size:10px;color:var(--text2);margin-top:6px;margin-left:28px;line-height:1.35;">Solo si la prenda queda visible en el catálogo web. Token ML en Supabase (secrets), no en el navegador. Deploy: <code style="font-size:9px;">mercadolibre-sync-product</code>.</div>
</div>` : ''}
${(window.AppRepository?.SUPABASE_URL || (window.META_COMMERCE_SYNC_ENDPOINT || '').trim()) ? `
  ...
  <label style="display: flex; align-items: center; gap: 10px; cursor: pointer; color: var(--text1); font-weight: bold;">
    ...
    📱 Sincronizar con Meta (Facebook Shops / Instagram) al guardar
  </label>
  <div style="font-size:10px;color:var(--text2);margin-top:6px;margin-left:28px;line-height:1.35;">Catálogo vía Commerce Manager. Secrets <code style="font-size:9px;">META_ACCESS_TOKEN</code>, <code style="font-size:9px;">META_CATALOG_ID</code>; opcional <code style="font-size:9px;">META_PRODUCT_BASE_URL</code>. Deploy: <code style="font-size:9px;">meta-commerce-sync</code>. Ver <code style="font-size:9px;">docs/META_COMMERCE_MANAGER.md</code>.</div>
</div>` : ''}
${(window.AppRepository?.SUPABASE_URL || (window.GOOGLE_MERCHANT_SYNC_ENDPOINT || '').trim()) ? `
  ...
  <label style="display: flex; align-items: center; gap: 10px; cursor: pointer; color: var(--text1); font-weight: bold;">
    ...
    🔍 Sincronizar con Google Merchant (Shopping) al guardar
  </label>
  <div style="font-size:10px;color:var(--text2);margin-top:6px;margin-left:28px;line-height:1.35;">Solo con prenda visible en catálogo web. Secrets <code style="font-size:9px;">GOOGLE_MERCHANT_ID</code>, <code style="font-size:9px;">GOOGLE_SERVICE_ACCOUNT_JSON</code>, <code style="font-size:9px;">GOOGLE_PRODUCT_BASE_URL</code> (HTTPS). Opcionales: <code style="font-size:9px;">GOOGLE_BRAND</code>, <code style="font-size:9px;">GOOGLE_PRODUCT_CATEGORY</code>, envío/peso (<code style="font-size:9px;">GOOGLE_SHIPPING_*</code>). Deploy: <code style="font-size:9px;">google-merchant-sync</code> <code style="font-size:9px;">--no-verify-jwt</code>. Verifica con <code style="font-size:9px;">products.list</code> en API Explorer (puede tardar minutos).</div>
</div>` : ''}
${(window.AppRepository?.SUPABASE_URL || (window.PINTEREST_SYNC_ENDPOINT || '').trim()) ? `
            <div class="form-group" data-integration-channel="pinterest" style="margin-top: 8px; padding: 10px; background: rgba(230,0,35,0.08); border-radius: 8px; border: 1px solid rgba(230,0,35,0.35);">
  <label style="display: flex; align-items: center; gap: 10px; cursor: pointer; color: var(--text1); font-weight: bold;">
    <input type="checkbox" id="art-sync-pinterest" style="width: 18px; height: 18px;">
    📌 Sincronizar catálogo Pinterest al guardar
  </label>
  <div style="font-size:10px;color:var(--text2);margin-top:6px;margin-left:28px;line-height:1.35;">Solo con prenda visible. En Supabase: <code style="font-size:9px;">PINTEREST_ACCESS_TOKEN</code> (OAuth <code style="font-size:9px;">catalogs:read</code>+<code style="font-size:9px;">catalogs:write</code>) y <code style="font-size:9px;">PINTEREST_PRODUCT_BASE_URL</code> = URL pública del catálogo con <code style="font-size:9px;">?ref=</code> al final (ej. <code style="font-size:9px;">https://tudominio.com/mayoristas/?ref=</code>). Eso alimenta el catálogo de Pinterest (Pines de producto / Shopping); la tienda en Pinterest depende de elegibilidad. Deploy: <code style="font-size:9px;">pinterest-catalog-sync</code>.<br><b>Pinterest → Crear Pines en lote / RSS:</b> Edge Function <code style="font-size:9px;">pinterest-product-feed</code> + secrets <code style="font-size:9px;">PINTEREST_RSS_SECRET</code>, <code style="font-size:9px;">PINTEREST_PRODUCT_LINK_BASE</code>; URL pública <code style="font-size:9px;">…/functions/v1/pinterest-product-feed?token=…&amp;format=rss</code> (CSV: <code style="font-size:9px;">format=csv</code>). El dominio del link debe estar reclamado en Pinterest.</div>
</div>` : ''}
${(window.AppRepository?.SUPABASE_URL || (window.DROPI_SYNC_ENDPOINT || '').trim()) ? `
  ...
  <div style="font-weight: bold; color: var(--text1); margin-bottom: 4px;">📦 Dropi (automático)</div>
  <div style="font-size:11px;color:var(--text2);line-height:1.45;">Si marcas <strong>Mostrar en el Catálogo Web</strong> y guardas, el producto se envía a Dropi (no hace falta crear “tienda” tipo WooCommerce en el panel de Dropi: tu ERP usa la API con token en servidor). Edge Function: <code style="font-size:9px;">hera-dropi-sync</code>.</div>
</div>` : ''}
${(window.AppRepository?.SUPABASE_URL || (window.FALABELLA_SYNC_ENDPOINT || '').trim()) ? `
            <div class="form-group" data-integration-channel="falabella" style="margin-top: 8px; padding: 10px; background: rgba(255,120,60,0.08); border-radius: 8px; border: 1px solid rgba(255,120,60,0.35);">
  <label style="display: flex; align-items: center; gap: 10px; cursor: pointer; color: var(--text1); font-weight: bold;">
    <input type="checkbox" id="art-sync-falabella" style="width: 18px; height: 18px;">
    🏬 Enviar a Falabella (Seller Center) al guardar
  </label>
  <div style="font-size:10px;color:var(--text2);margin-top:6px;margin-left:28px;line-height:1.35;">Solo con prenda visible. Categoría: mapa <code style="font-size:9px;">FALABELLA_CATEGORY_MAP_JSON</code> o <code style="font-size:9px;">FALABELLA_PRIMARY_CATEGORY_ID</code>. Estado y errores se guardan en la BD al responder la API.</div>
  ${art ? `
  <div style="font-size:11px;margin-top:10px;padding:10px;background:rgba(0,0,0,.18);border-radius:8px;line-height:1.5;border:1px solid rgba(255,120,60,.2);">
    <div style="font-weight:800;margin-bottom:6px;color:var(--text1);">Estado Falabella</div>
    <div><strong>Estado:</strong> ${art.falabellaSyncStatus ? `<span class="badge ${art.falabellaSyncStatus === 'synced' ? 'badge-ok' : art.falabellaSyncStatus === 'error' ? 'badge-pend' : 'badge-warn'}">${art.falabellaSyncStatus}</span>` : '<span style="color:var(--text2)">—</span>'}</div>
    ${art.falabellaSellerSku ? `<div><strong>SKU:</strong> <code style="font-size:10px">${art.falabellaSellerSku}</code></div>` : ''}
    ${art.falabellaPrimaryCategoryId ? `<div><strong>CategoryId:</strong> <code style="font-size:10px">${art.falabellaPrimaryCategoryId}</code></div>` : ''}
    ${art.falabellaFeedRequestId ? `<div><strong>Feed request:</strong> <code style="font-size:9px;word-break:break-all">${art.falabellaFeedRequestId}</code></div>` : ''}
    ${art.falabellaLastSyncAt ? `<div><strong>Último intento:</strong> ${(function(){ try { return new Date(art.falabellaLastSyncAt).toLocaleString('es-CO'); } catch(_) { return String(art.falabellaLastSyncAt); } })()}</div>` : ''}
    ${art.falabellaLastError ? `<div style="color:var(--red);margin-top:6px;font-size:10px;"><strong>Error:</strong> ${String(art.falabellaLastError).slice(0, 280)}</div>` : ''}
  </div>` : ''}
  <label style="font-size:10px;color:var(--text2);display:block;margin-top:10px;">Atributos <code>ProductData</code> extra (JSON: FeedName → valor)</label>
  <textarea class="form-control" id="m-art-falabella-json" rows="3" style="font-family:DM Mono,monospace;font-size:10px;margin-top:4px;" placeholder='Ej. { "Composicion": "100% Algodón" }'></textarea>
</div>` : ''}
  ...
  setTimeout(() => {
    ...
    const mlChk = document.getElementById('art-sync-mercadolibre');
    if (mlChk) mlChk.checked = true;
    const metaChk = document.getElementById('art-sync-meta-commerce');
    if (metaChk) metaChk.checked = false;
    const googleChk = document.getElementById('art-sync-google-merchant');
    if (googleChk) googleChk.checked = false;
    const pinChk = document.getElementById('art-sync-pinterest');
    if (pinChk) pinChk.checked = true;
    const falChk = document.getElementById('art-sync-falabella');
    if (falChk) falChk.checked = false;
    const falJsonTa = document.getElementById('m-art-falabella-json');
    if (falJsonTa && art && art.falabellaProductDataJson && typeof art.falabellaProductDataJson === 'object') {
      try {
        falJsonTa.value = JSON.stringify(art.falabellaProductDataJson, null, 2);
      } catch (_) {
        falJsonTa.value = '';
      }
    }
...
async function saveArticulo(existingId) {
    ...
    const falJsonEl = document.getElementById('m-art-falabella-json');
    if (falJsonEl && falJsonEl.value.trim()) {
      try {
        const parsed = JSON.parse(falJsonEl.value);
        if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
          throw new Error('Debe ser un objeto JSON { "FeedName": "valor" }');
        }
        falabella_product_data_json = parsed;
      } catch (e) {
        alert('Atributos Falabella (JSON): ' + (e.message || e));
        return;
      }
    }
    ...
    try {
        ...
        const mlNote = await postSaveMercadoLibreIntegration(productId, catalogVisibleBool);
        const metaNote = await postSaveMetaCommerceIntegration(productId, catalogVisibleBool);
        const googleNote = await postSaveGoogleMerchantIntegration(productId, catalogVisibleBool);
        const pinterestNote = await postSavePinterestIntegration(productId, catalogVisibleBool);
        ...
        const falabellaResult = await postSaveFalabellaIntegration(productId, catalogVisibleBool);
        ...
        const artLocal = {
          ...
          falabellaProductDataJson: productData.falabella_product_data_json && typeof productData.falabella_product_data_json === 'object'
            ? productData.falabella_product_data_json : {},
...
function renderInvTrazabilidad(){
  if (window.AppInventoryModule?.renderInvTrazabilidad) {
    return window.AppInventoryModule.renderInvTrazabilidad({ state, formatDate });
  }
...
function openDocModal(collection,tipo,existingId){
  ...
  openModal(`
    ...
    ${collection==='facturas'?`<div class="form-group" style="margin-bottom:12px"><label style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:13px"><input type="checkbox" id="m-doc-apply-iva" checked onchange="updateDocTotal()"> Aplicar IVA (19%) — como en POS</label></div>`:''}
...
function updateDocTotal(){
  ...
  const applyIva=ivaEl?ivaEl.checked:true;
...
async function saveDoc(collection,tipo){
  ...
  const ivaEl=document.getElementById('m-doc-apply-iva');
  const applyIvaF=ivaEl?ivaEl.checked:true;
...
function calcDeudaProveedor(provId) {
  if (window.AppTreasuryModule?.calcDeudaProveedor) {
    return window.AppTreasuryModule.calcDeudaProveedor(state, provId);
  }
  ...
  let ajustesSalidaCosto = 0, ajustesSalidaUds = 0, ajustesEntradaCosto = 0, ajustesEntradaUds = 0;
  (state.inv_ajustes||[]).forEach(aj=>{
    const artAj = (state.articulos||[]).find(a=>String(a.id)===String(aj.articuloId));
    if(!artAj||artAj.tituloMercancia!=='credito')return;
    if(esSinEspecificar){ if(artAj.proveedorId)return; }
    else if(String(artAj.proveedorId)!==String(provId))return;
    const cAj = parseFloat(artAj.precioCompra)||0, qAj = parseFloat(aj.cantidad)||0;
    if(aj.tipo==='salida'){ ajustesSalidaUds+=qAj; ajustesSalidaCosto+=qAj*cAj; }
    else if(aj.tipo==='entrada'){ ajustesEntradaUds+=qAj; ajustesEntradaCosto+=qAj*cAj; }
  });
  ...
  return { valorInventarioCosto, costoVendidoHist: costoVN, unidadesVendidasHist: uVN, refOperativaTotal, compromisoReconocido, compromisoTotal: compromisoReconocido, deudaBruta: refOperativaTotal, abonos, saldo, saldoLibro, articulos, ajustesSalidaCosto, ajustesSalidaUds, ajustesEntradaCosto, ajustesEntradaUds };
}
...
function renderTesPagosProv() {
  ...
  const provConDeuda = (state.usu_proveedores||[]).map(p => {
    ...
    return { ...p, ...d };
  }).filter(p => provTieneActividad(p));

  const dSinProv = calcDeudaProveedor('__sin_proveedor__');
  if(provTieneActividad(dSinProv)) {
    provConDeuda.push({
      ...
      <div class="empty-state">
        ...
        <div class="card" style="margin-bottom:12px">
          ...
          <div style="display:flex;gap:8px;flex-wrap:wrap">
            <button class="btn btn-sm btn-secondary" onclick="openCompromisoProvModal('${p.id}','${p.nombre}')">📥 Compromiso</button>
...
function openCompromisoProvModal(provId = '', provNombre = '') {
  if (window.AppTreasuryModule?.openCompromisoProvModal) {
    return window.AppTreasuryModule.openCompromisoProvModal({ state, provId, provNombre, fmt, openModal, notify, today });
  }
  notify('warning', '⚠️', 'Módulo tesorería', 'Carga treasury-module.js para registrar compromisos.', { duration: 4000 });
}

async function guardarCompromisoProv() {
  if (window.AppTreasuryModule?.guardarCompromisoProv) {
    return window.AppTreasuryModule.guardarCompromisoProv({
...
async function eliminarCompromisoProv(id) {
  if (window.AppTreasuryModule?.eliminarCompromisoProv) {
    return window.AppTreasuryModule.eliminarCompromisoProv({ state, id, confirm, supabaseClient, renderTesPagosProv, notify });
  }
}

function verCompromisosProv(provId, provNombre) {
  if (window.AppTreasuryModule?.verCompromisosProv) {
    return window.AppTreasuryModule.verCompromisosProv({ state, provId, provNombre, fmt, formatDate, openModal });
  }
}

async function importarEstimacionCompromisosProv() {
  if (window.AppTreasuryModule?.importarEstimacionCompromisosProv) {
    return window.AppTreasuryModule.importarEstimacionCompromisosProv({
...
function openAbonoProvModal(provId='', provNombre='') {
  if (window.AppTreasuryModule?.openAbonoProvModal) {
    return window.AppTreasuryModule.openAbonoProvModal({ state, provId, provNombre, fmt, openModal, notify, today });
  }
  const opts=[];
  (state.usu_proveedores||[]).forEach(p=>{const d=calcDeudaProveedor(p.id);if(d.saldo>0)opts.push({id:p.id,nombre:p.nombre,saldo:d.saldo});});
  const dSin=calcDeudaProveedor('__sin_proveedor__');
  if(dSin.saldo>0)opts.push({id:'__sin_proveedor__',nombre:'⚠️ Sin proveedor (crédito)',saldo:dSin.saldo});
  if(provId && !opts.find(o=>o.id===provId)) {
    const d = calcDeudaProveedor(provId);
    opts.push({ id: provId, nombre: provNombre || (provId === '__sin_proveedor__' ? 'Sin proveedor (crédito)' : 'Proveedor'), saldo: d.saldo });
  }

  if(opts.length === 0 && !provId) {
    notify('warning','⚠️','Sin saldo pendiente','No hay proveedores con saldo por pagar (compromiso − abonos).',{duration:4000});
    return;
  }
  ...
  openModal(`
    ...
    <div class="form-group"><label class="form-label">PROVEEDOR *</label>
      <select class="form-control" id="ab-prov-sel" onchange="updateSaldoPendiente()">
        ...
        ${opts.map(o => {
          ...
          return `<option value="${o.id}" data-saldo="${o.saldo}" data-nombre="${nm}" ${o.id===provId?'selected':''}>${o.nombre} · Saldo: ${fmt(o.saldo)}</option>`;
        }).join('')}
      </select>
    </div>
    <div id="ab-saldo-info" style="background:rgba(251,191,36,.1);border:1px solid rgba(251,191,36,.3);border-radius:8px;padding:10px;margin-bottom:12px;font-size:12px;display:none">
      <span style="color:var(--text2)">Saldo pendiente (compromiso − abonos):</span> <span id="ab-saldo-val" style="font-weight:700;color:var(--yellow)"></span>
    </div>
    <div class="form-row">
...
function updateSaldoPendiente() {
  if (window.AppTreasuryModule?.updateSaldoPendiente) {
    return window.AppTreasuryModule.updateSaldoPendiente({ fmt });
  }
...
function validateAbono() {
  if (window.AppTreasuryModule?.validateAbono) {
    return window.AppTreasuryModule.validateAbono({ fmt });
  }
  const sel = document.getElementById('ab-prov-sel');
  const opt = sel?.options[sel.selectedIndex];
  const saldo = parseFloat(opt?.getAttribute('data-saldo')||0);
  ...
  if(valor > saldo && saldo > 0) {
    ...
    warn.textContent = `⚠️ El abono (${fmt(valor)}) supera el saldo (${fmt(saldo)}). Puedes registrar anticipo si aplica.`;
  } else {
...
async function guardarAbonoProv() {
  if (window.AppTreasuryModule?.guardarAbonoProv) {
    return window.AppTreasuryModule.guardarAbonoProv({ state, uid, dbId, today, showLoadingOverlay, supabaseClient, saveRecord, closeModal, renderTesPagosProv, notify, fmt });
  }
  const sel = document.getElementById('ab-prov-sel');
  const opt = sel?.options[sel.selectedIndex];
  ...
  const abono = { id: dbId(), proveedorId: provId, proveedorNombre: provNombre,
  ...
  try {
    ...
    if(cajaAbierta) {
      ...
      const mov = { id: dbId(), cajaId: cajaAbierta.id, tipo: 'egreso', valor,
...
function verAbonosProv(provId, provNombre) {
  ...
  openModal(`
    ...
    <div style="font-size:11px;color:var(--text2);margin-bottom:10px"><b>Desglose:</b> en stock ${fmt(d.valorInventarioCosto||0)} · ya vendido (POS a costo) ${fmt(d.costoVendidoHist||0)} · ajustes salida (hist.) ${fmt(d.ajustesSalidaCosto||0)} (${d.ajustesSalidaUds||0} uds, ya en stock)</div>
...
async function eliminarAbonoProv(id) {
  if (window.AppTreasuryModule?.eliminarAbonoProv) {
    return window.AppTreasuryModule.eliminarAbonoProv({ state, id, confirm, supabaseClient, renderTesPagosProv, notify });
  }
...
function guardarAbrirCaja() {
  if (window.AppTreasuryModule?.guardarAbrirCaja) {
    return window.AppTreasuryModule.guardarAbrirCaja({ state, dbId, uid, saveRecord, closeModal, renderTesCajas, notify, fmt, today });
  }
}

function guardarCierreCaja() {
  if (window.AppTreasuryModule?.guardarCierreCaja) {
    return window.AppTreasuryModule.guardarCierreCaja({ state, dbId, uid, today, saveRecord, closeModal, renderTesCajas, notify, fmt });
  }
}

function verCierresCajaModal(id) {
  if (window.AppTreasuryModule?.verCierresCajaModal) {
    return window.AppTreasuryModule.verCierresCajaModal({ state, id, openModal, fmt, formatDate });
  }
}

function renderTesCajas(){
  if (window.AppTreasuryModule?.renderTesCajas) {
    return window.AppTreasuryModule.renderTesCajas({ state, fmt, openModal, formatDate, today, notify, dbId, uid, saveRecord });
  }
...
function openCajaModal(){
  if (window.AppTreasuryModule?.openCajaModal) {
    return window.AppTreasuryModule.openCajaModal({ openModal });
  }
  openModal(`
...
function saveCaja(){
  if (window.AppTreasuryModule?.saveCaja) {
    return window.AppTreasuryModule.saveCaja({ state, uid, dbId, saveRecord, closeModal, renderTesCajas, today });
  }
  ...
  const saldos=window.AppCajaLogic?.emptySaldos?{...window.AppCajaLogic.emptySaldos(),efectivo:inicial}:{efectivo:inicial,transferencia:0,addi:0,contraentrega:0,tarjeta:0,digital:0,otro:0};
  const caja = {id:dbId(),nombre,saldo:inicial,estado:'abierta',apertura:today(),bodegaIds:[],saldosMetodo:saldos};
...
function cerrarCaja(id){
  if (window.AppTreasuryModule?.cerrarCaja) {
    return window.AppTreasuryModule.cerrarCaja({ state, id, openModal, fmt, today });
  }
...
function abrirCaja(id){
  if (window.AppTreasuryModule?.abrirCaja) {
    return window.AppTreasuryModule.abrirCaja({ state, id, openModal, fmt, notify, today });
  }
  const c=(state.cajas||[]).find(x=>x.id===id);
  if(c){
...
function movCaja(cajaId, tipo) {
  if (window.AppTreasuryModule?.openMovCajaModal) {
    return window.AppTreasuryModule.openMovCajaModal({ state, cajaId, tipo, openModal, fmt, today, notify });
  }
  notify('warning', '⚠️', 'Módulo tesorería', 'No se pudo abrir el movimiento de caja.', { duration: 4000 });
}

function saveMovCaja(cajaId, tipo) {
  if (window.AppTreasuryModule?.saveMovCaja) {
    return window.AppTreasuryModule.saveMovCaja({ state, cajaId, tipo, uid, dbId, today, saveRecord, closeModal, renderTesCajas, notify, fmt });
  }
  ...
  if (!concepto) { notify('warning', '⚠️', 'Concepto', 'Describe el movimiento.', { duration: 3000 }); return; }
  ...
  const bucketEl = document.getElementById('m-mov-bucket');
  const bucket = (bucketEl && bucketEl.value) || (window.AppCajaLogic?.bucketFromMetodoId?.(metodo, state.cfg_metodos_pago)) || 'efectivo';
  const caja = (state.cajas || []).find(c => c.id === cajaId); 
  if (!caja) return;
  window.AppCajaLogic?.normalizeCaja?.(caja);
  if (tipo === 'ingreso') {
    window.AppCajaLogic?.applyDeltaBucket?.(caja, bucket, valor);
  } else {
    const disp = window.AppCajaLogic?.saldoEnBucket?.(caja, bucket) ?? caja.saldo ?? 0;
    if (disp < valor) { notify('warning', '⚠️', 'Saldo insuficiente', `En «${bucket}» hay ${fmt(disp)}.`, { duration: 5000 }); return; }
    window.AppCajaLogic?.applyDeltaBucket?.(caja, bucket, -valor);
  }
  const mov = { id: dbId(), cajaId, tipo, valor, concepto, fecha: today(), metodo, categoria, bucket };
  window.AppCajaLogic?.enrichMovWithSesion?.(state, cajaId, mov, dbId);
  state.tes_movimientos.push(mov);
...
function renderTesDinero(){
  if (window.AppTreasuryModule?.renderTesDinero) {
    return window.AppTreasuryModule.renderTesDinero({ state, formatDate, fmt });
  }
  ...
  document.getElementById('tes_dinero-content').innerHTML=`
    ...
    <div class="table-wrap"><table><thead><tr><th>Fecha</th><th>Caja</th><th>Tipo</th><th>Valor</th><th>Bucket</th><th>Clase</th><th>Concepto</th><th>Método</th></tr></thead><tbody>
    ${movs.map(m=>{const caja=(state.cajas||[]).find(c=>c.id===m.cajaId);return`<tr><td>${formatDate(m.fecha)}</td><td>${caja?.nombre||'—'}</td><td><span class="badge ${m.tipo==='ingreso'?'badge-ok':'badge-pend'}">${m.tipo}</span></td><td style="color:${m.tipo==='ingreso'?'var(--green)':'var(--red)'};font-weight:700">${fmt(m.valor)}</td><td style="font-size:11px">${m.bucket||'—'}</td><td style="font-size:11px;color:var(--text2)">${m.categoria||'—'}</td><td>${m.concepto||'—'}</td><td>${m.metodo||'—'}</td></tr>`}).join('')||'<tr><td colspan="8" style="text-align:center;color:var(--text2);padding:24px">Sin movimientos</td></tr>'}
    </tbody></table></div></div>`;
}

// GENERIC SIMPLE RENDERERS FOR TESORERÍA SUB-MODULES
function renderSimpleCollection(pageId,title,collection,columns){
  if (window.AppTreasuryModule?.renderSimpleCollection) {
    return window.AppTreasuryModule.renderSimpleCollection({ state, pageId, title, collection, columns, fmt });
  }
...
function buildAlerts() {
  if (window.AppGameSystemModule?.buildAlerts) {
    return window.AppGameSystemModule.buildAlerts({ state, fmt, getArticuloStock });
  }
  const alerts = [];
  ...
  if(pend.length > 0) alerts.push({
    ...
    action: "showPage('pendientes')", actionLabel: 'Ir a Cobros'
  });
  ...
  // --- Deudas a proveedores (diaria) — mismo criterio que Pagos proveedores (compromiso − abonos) ---
  const _buildProvList = () => {
    ...
    if(dSin.saldo > 0){
      ...
      list.push({id:'__sin_proveedor__',nombre:'Sin proveedor',saldo:dSin.saldo,diasDeuda:0,artCredito:sinProv});
    }
    return list;
  };
...
async function reintentarSyncCajaInventario(id) {
  ...
  // 1) Reintento inventario (stock_moves) de forma segura.
  try {
    ...
    if (!existing || existing.length === 0) {
      for (let i = 0; i < items.length; i++) {
        ...
        const row = {
          ...
          documento_id: id,
          fecha: v.fecha || today(),
          nota: `${it.nombre || 'Ítem'} · Talla: ${it.talla || '—'}`
        };
        row[qtyCol] = -qty;
    ...
    invOk = false;
    console.warn('reintentarSyncCajaInventario stock_moves:', e.message || e);
  }
...
// ===== SEPARADOS (SHOWROOM) =====
// ===================================================================
function _sepLineItems(v){
  const f=(state.facturas||[]).find(x=>x.id===v.id);
  const items=(f&&Array.isArray(f.items)&&f.items.length)?f.items:(v.items||[]);
  return items.map(i=>`${i.nombre||i.name||'Ítem'}${i.talla?' · T:'+i.talla:''} ×${i.qty||i.cantidad||1}`).join('; ')||'—';
}
...
function renderSeparados(){
  ...
  const rowsHtml = lista.map(v => {
    ...
    <td>${v.telefono||'—'}</td>
    <td style="font-size:11px;color:var(--text2);max-width:220px">${_sepLineItems(v)}</td>
    <td style="color:var(--accent);font-weight:700">${fmt(v.valor)}</td>
    <td><span class="badge ${ent?'badge-ok':'badge-warn'}">${v.estadoEntrega||'Pendiente'}</span></td>
  ...
  }).join('')||'<tr><td colspan="9" style="text-align:center;color:var(--text2);padding:24px">Sin separados</td></tr>';
  ...
  document.getElementById('separados-content').innerHTML = `
    <div style="display:flex;gap:10px;align-items:flex-end;flex-wrap:wrap;margin-bottom:16px;">
      <div class="search-bar" style="flex:1;min-width:180px;max-width:280px;margin:0">
        <span class="search-icon">🔍</span>
        <input type="text" id="sep-search" placeholder="Cliente, tel, ref..." value="${q}" oninput="renderSeparados()">
      </div>
    ...
    <div class="card">
      ...
      <div class="table-wrap"><table>
        <thead><tr><th>Fecha</th><th>Canal</th><th>Ref</th><th>Cliente</th><th>Teléfono</th><th>Artículos</th><th>Total</th><th>Estado</th><th>Acción</th></tr></thead>
