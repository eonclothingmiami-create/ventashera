...
window.requestDropiSync = async function requestDropiSync(productId, extra) {
  ...
  if (data && data.ok === false) {
    ...
    if (/token not issued|401/i.test(detail)) {
      detail +=
        ' — Genera un token nuevo en el panel de Dropi (API / integración) y actualiza el secret DROPI_ACCESS_TOKEN en Supabase → Edge Functions → hera-dropi-sync.';
    }
