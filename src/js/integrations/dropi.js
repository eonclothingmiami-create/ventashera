 ...
 * `AppRepository.SUPABASE_URL` + `/functions/v1/dropi-sync-product`.
 *
 * En Supabase, crea la función `dropi-sync-product` (Deno). Secrets típicos (ajusta nombres
...
(function initDropiEndpoint() {
  ...
  if (base) {
    window.DROPI_SYNC_ENDPOINT = String(base).replace(/\/$/, '') + '/functions/v1/dropi-sync-product';
  }
})();
...
window.requestDropiSync = async function requestDropiSync(productId, extra) {
  ...
  const client = window.AppRepository?.supabaseClient;
  ...
  if (client && typeof client.functions?.invoke === 'function') {
    ...
    if (anon) {
      invokeHeaders.apikey = anon;
      if (!invokeHeaders.Authorization) {
        invokeHeaders.Authorization = 'Bearer ' + anon;
      }
    }
    const { data, error } = await client.functions.invoke('dropi-sync-product', {
      body: payload,
    ...
    if (error) {
      ...
      try {
        ...
        if (ctx && ctx.body) {
          ...
          if (b && (b.error || b.hint)) {
            msg = [b.error, b.hint].filter(Boolean).join(' — ');
          }
        }
      } catch (_) { /* noop */ }
