-- Blindaje: un producto archivado (active=false) no se puede vender por POS.
-- Se comporta como borrado en operación; el historial sigue intacto.
...
begin
  ...
  loop
    ...
    if not exists (
      ...
      where id = v_line.product_id and active = true
    ) then
      raise exception 'El producto fue eliminado y no se puede vender';
    end if;
