-- cancel_pos_sale_v2: tolerar ventas POS antiguas sin stock_moves ligados por documento_id.
--
-- Contexto: 207 de 1.455 facturas POS activas (mar–jul 2026) se registraron antes de
-- que el flujo enlazara stock_moves.documento_id con la factura. La versión anterior
-- levantaba 'no reversible stock movements found' y dejaba esas ventas imposibles de
-- anular, incluso para revertir el dinero en caja.
--
-- Ahora se resuelve el origen del stock en cascada y se informa cuál se usó:
--   documento  → stock_moves ligados por documento_id (exacto, ventas nuevas).
--   referencia → stock_moves con el número de factura y documento_id nulo (46 casos).
--   items      → líneas guardadas en invoices.items (161 casos; mismo comportamiento
--                que tenía el ERP antes de este cambio, pero ahora dentro de la transacción).
-- Si no hay ninguna de las tres, se aborta sin tocar nada.
...
begin
  ...
  if exists (
    ...
    raise exception 'cancel_pos_sale_v2: no hay movimientos ni líneas para revertir el stock';
  end if;
  ...
  loop
    ...
    values (gen_random_uuid(), v_stock.product_id, v_stock.qty_to_restore, v_stock.qty_to_restore, 'anulacion_pos', 'venta_pos',
            ...
            'Anulación POS (' || v_stock_source || ') · ' || v_reason);
  end loop;

  if not v_has_stock then
    raise exception 'cancel_pos_sale_v2: no se pudo resolver ningún producto para devolver stock';
  end if;
  ...
  if not exists (
    ...
    loop
      ...
      values (gen_random_uuid()::text, v_cash.caja_id, 'egreso', v_cash.valor,
              'Anulación ' || v_invoice.number || ' · ' || v_reason,
