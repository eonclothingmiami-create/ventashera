...
- **Pinterest (Shopping / catálogo)**: API por lotes `pinterest-catalog-sync` + script `src/js/integrations/pinterest.js`. Feed CSV programado (URL): `pinterest-catalog-feed`. Claves y URLs: [`docs/PINTEREST_CLAVES_SECRETS.txt`](docs/PINTEREST_CLAVES_SECRETS.txt).
